...
- **BLIND rebuild** (Opus, code-only) — reconstructs the kit from the engine + stripped override. Says
  what the code _does_, with zero knowledge of intent or the real text. Surfaces non-obvious routing.
- **SIGHTED full-context review** — reads the real kit text, the override in full, `kit-status.json`,
  and the board. Says what the override _intends_ and what it deliberately doesn't model.
