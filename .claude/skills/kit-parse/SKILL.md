...
1. **Weapon-state modifiers ARE damage (prior 9).** Reload speed/ratio ▲▼, partial/instant reload,
   ...
   **weapon swap** ("Changes the weapon in use") — they gate SHOT COUNT and the fire profile, which
   gate damage. Before writing "skip", answer: _does this change shots fired or the weapon?_ If yes,
...
1. **Cadence tuple** — rate-of-fire / `reloadFrames` / rolling-reload. _Datamine-unreliable._ (see the
...
2. **A damage line the text gives NO trigger for** — if you had to INVENT the activation (e.g. a bare
   "Deals X% as damage" with no "when …"), flag the invented trigger + cadence. _Kit-silent._
...
3. **Weapon-swap shot economy** — shots-fired per burst / `maxShots` vs `durationSec` for a
   `weaponSwap`. The text rarely states how many swapped shots actually land. _Kit-silent._
...
4. **Stack / currency steady-state average + ramp haircut** (prior 4). _Measured-only_ for the exact
...
5. **Multi-projectile split-vs-merge** (prior 5). _Kit-silent — read popups._
6. **`noFb` per-kit** — default OFF (FB by timing, prior 2); set ONLY with measured FB-OFF evidence.
7. **Hit-Rate → core-rate magnitude** (hard rule 4). _Measured-only._
