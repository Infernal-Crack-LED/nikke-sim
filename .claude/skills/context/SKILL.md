...
`addGauge`/`skillGauge`/`shotGauge`; datamined per-shot table (`data/gauge-per-shot.json`). **Focus bonus**: the CAMERA-FOCUSED unit's CHARGE weapon (SR/RL) generates at THAT UNIT'S OWN full-charge bonus — `chargeMultiplier/100` from `characters.json`, with `gauge-per-shot.json`'s `fullChargeBonus` filling in where that row reads 0 (`raven`). There is **no roster default and no flat pin anywhere** (owner ruling 2026-08-12, completed 2026-08-18: `FOCUS_CHARGE_GEN` and `PENDING_TEAM_ISOLATION` are both deleted). ×2.5 is merely the modal family (250), not a rule: `alice` 3.5×, `scarlet-black-shadow` 1.5×, `cinderella`/`vesti-tactical-upgrade` 2.0×. A unit with no bonus in EITHER column does not full-charge and takes ×1.0 — `pascal` is the only one. Unfocused charge = ×1.0 (`UNFOCUSED_CHARGE_GEN`, measured, `sim.ts:1434`); the ladder itself is in `gaugePerShot()` (`sim.ts:1474`). Non-charge weapons focused ≈ no bonus. Recorded runs pass `cfg.focusSlug`. Gauge locked during FB AND during the chain (stages 1-3), unlocking the instant FB ends (owner rulings 2026-07-13 + 2026-08-04). The synergy-API burstGaugePerShot column was DROPPED as a source.

## 8. Burst rotation state machine [gm §8]

Chain B1→B2→B3 triggers Full Burst. `STAGE_CAST_GAP_FRAMES=30` (sim.ts:141) between stage casts; gauge-full → 30f → B1 (`PRE_B1_GAP_FRAMES`, sim.ts:148); B3 cast → 22f → FB countdown (`FB_PRE_DELAY_FRAMES`, sim.ts:153). FB start sets `fbEndFrame` (sim.ts:1513). **NO post-FB chain-open lock** (owner ruling 2026-08-04): generation resumes the instant FB ends and the next chain opens on gauge-full — the ~3-4s gap to the next chain is natural refill-from-zero, not a delay. The old fixed block (`POST_FB_CHAIN_DELAY_FRAMES=150`) survives ONLY as the opt-in `ENV.ROTMODEL='floor'` A/B arm (constant sim.ts:1501, setter :2988, consumer :3007). **First-ready-with-waiting selection** (sim.ts:3078): inside a timed stage window the chain waits for the stage-filling unit whose cooldown ends SOONEST (tie → leftmost); `B3_LEFTMOST=1` restores strict leftmost (owner ruling 2026-07-21). Full-burst COUNTS are refill/cooldown arithmetic — deterministic run-to-run except boss-transition/chain collisions; graded comps pinned in `scripts/regression.ts`. Burst-cast damage lands BEFORE FB begins (no +50%, no entry auras).
