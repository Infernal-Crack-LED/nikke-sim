...
- an SG PELLET-LANDING question → the UNIGEO **marker detector** (`scripts/unigeo/marker_detect2.py` +
  ...
  template track (crosshair works) and tracking is tuned (avgLanded ≈7.3), but its CORE count
  over-counts perimeter VFX — see its reader-table note.
...
crosshair lock works and tracking is tuned (avgLanded ≈7.3); the core/red count still over-counts
perimeter VFX (red fix outstanding). The validated part is the marker detection.
...
- `read-markers.py` (Python, `scripts/probe/.venv`) — SG pellet-marker counter; wraps the UNIGEO
  ...
  validated _given a disc centre_; crosshair lock works and tracking is tuned (avgLanded ≈7.3), but the
  core/red count over-counts perimeter VFX (red fix TBD).
