...
- an SG PELLET-LANDING question → **`read-markers.py`** (UNIGEO marker counter, ✅ validated — the preferred
  instrument) or `read-pellets.ts` (CV popup survey, ⚑ candidate). **Both already exist and are documented
  in the reader table below — do NOT re-derive a pellet counter from scratch.**
...
| `scripts/probe/read-markers.py <video> --out D` (UNIGEO pellet-marker counter; wraps `marker_detect2.py` + `marker_track.py`)                          | per-shot **pellet landing** from the in-game pellet markers (white/red circles inside the grey aim disc) + radial position relative to the disc centre. Requires disc-centre tracking and per-HR scaling. **SG-only.**            | per-shot `{t_video, landed, cores, radii_white, radii_red}` + radial-distribution JSON; consumable by `scripts/unigeo/w1-fit.py` / `w4-part2-fit.py`                                                               | ✅ **VALIDATED 2026-07-22** on `soda tb control.mov` near HR-ON vs owner count: structural match, core totals 10 vs 11, cadence reproduced, recall ~0.70 with radially unbiased misses. **Preferred instrument for SG landing / UNIGEO geometry questions.** Runnable via `scripts/probe/read-markers.py <video> [--at S --dur S --fps 60 --hr 0 --crop …] --out D` — it wraps `marker_detect2.py` + `marker_track.py` AND supplies the disc-centre tracker (`find_disc`) the raw scripts lack, so it runs on any solo SG recording. → `D/shots.json` `{shots[{t_video,landed,cores,radii_white,radii_red}], summary{totalShots,avgLanded,avgCores}}` + `D/discs.json` (per-frame disc centre/radius/conf). Needs `scripts/probe/.venv` (cv2/numpy). **Use this — do NOT re-derive a pellet counter.** |
...
measured cells), and `scripts/probe/read-markers.py` — the runnable SG pellet-marker counter that wraps
`docs/probes/drawn-geometry/marker_detect2.py` + `marker_track.py` and supplies the disc-centre tracker
(`find_disc`) they lack (machine-reads pellet markers for SG landing/radial distribution).
...
- `read-markers.py` (Python, `scripts/probe/.venv`) — SG pellet-marker counter; wraps the UNIGEO
  `marker_detect2.py` + `marker_track.py` and supplies the disc-centre tracker (`find_disc`) they lack.
  The ✅ validated preferred SG-landing instrument — use this before re-deriving anything.
