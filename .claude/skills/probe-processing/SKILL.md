...
- an SG PELLET-COUNT / LANDING question → **`read-pellets.ts`** — the SG pellet counter: white pellets
  by bright-dot threshold (R,G,B≥210) + circularity, **cores from the red TRIANGULAR hit-markers**
  tight to the crosshair (`--marker-radius 65`), tracked across each shot. Validated avgTotal **7.6**
  on `marciana-solo.MP4` ≈ owner ground truth (7–9 white pellets/shot, ~0 cores). **Do NOT re-derive a
  pellet counter from scratch.** `read-markers.py` (the UNIGEO marker wrapper) is **PARKED WIP** — its
  ring-marker white detection under-counts marciana's pellets and its red-CIRCLE detection catches
  disc-perimeter VFX, not the red-triangle core signal (see its reader-table note).
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `read-pellets.ts <video> --at S --dur S --fps 30 --zoom 2 --out D`                                                                                     | per-shot **pellet counts** from CV dot detection in the damage area (white pellets R,G,B≥210 + circularity; **cores = red triangular hit-markers** tight to the crosshair) + a VLM **timer spine** @1fps + ammo-box-template crosshair tracking. **SG-only.** It does NOT read the ammo digits | `D/pellets.json` — `reads[]`, `shots[]` `{fightT, white, red, total, core, frames, backendAgreement}`, `summary {totalShots, validShots, expectedShots, avgTotal, avgRed}`                                         | ✅ **The SG pellet counter** (per-shot counts). Validated on `marciana-solo.MP4`: avgTotal **7.6** ≈ owner ground truth (7–9 white pellets/shot, ~0 cores), **70 of ~90** shots detected, 58 in the 5–10 bound. ⚠ Its landing AVERAGE still under-reads the lattice ≈**8.45** by ~10% (avgRed 0.19 vs ~0.5) — fine for COUNTING pellets, not yet a landing measurement for U35 (see U35)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `scripts/probe/read-markers.py <video> --out D` (UNIGEO pellet-marker counter; wraps `scripts/unigeo/marker_detect2.py`)                               | per-shot **pellet landing** (intended) from the in-game pellet markers + radial position relative to the disc centre. Requires disc-centre tracking and per-HR scaling. **SG-only.** 🅿 PARKED — see maturity.                                                                                  | per-shot `{t_video, landed, cores, radii_white, radii_red}` + radial-distribution JSON; consumable by `scripts/unigeo/w1-fit.py` / `w4-part2-fit.py`                                                               | 🅿 **PARKED WIP — do NOT use for counts (use `read-pellets.ts`).** The UNIGEO marker DETECTION (`scripts/unigeo/marker_detect2.py`) is ✅ validated for the white ring-markers _given a supplied disc centre_ (`soda tb control.mov`, recall ~0.70), but on `marciana-solo.MP4` it does NOT match the pellets: its ring-marker white detection under-counts (it requires a dark grey ring marciana's pellets lack), and its red-CIRCLE detection catches disc-perimeter **VFX**, not cores — **cores are the red TRIANGULAR hit-markers** (the `read-pellets.ts` signal), which this never detects (owner ground truth: 7–9 white/shot, 0 cores). The wrapper's **ammo-box-template crosshair track + tuned tracking ARE sound** (that locator is shared with `read-pellets.ts`; HoughCircles `find_disc` is only a fallback). Output would be `D/shots.json` `{shots[{t_video,landed,cores,radii_white,radii_red}]}` + `D/discs.json`. Needs `scripts/probe/.venv` (cv2/numpy). To revive: replace `marker_detect2.detect` with bright-dot white detection + red-triangle core detection, keeping the radial-position output (the one thing it adds over `read-pellets.ts`). |
...
template crosshair track (machine-reads pellet markers for SG landing/radial distribution). 🅿 PARKED
WIP for marciana: its ring-marker white detection under-counts the pellets and its red-CIRCLE detection
catches disc-perimeter VFX, not the red-triangle core signal. Use `read-pellets.ts` for SG pellet
counts; read-markers.py's crosshair track is sound and its radial-position output is the revival goal.
...
- `read-pellets.ts` + `count-pellets.py` (+ `ammo-box-template.png`) — **the SG pellet counter** (white
  pellets by bright-dot threshold + circularity; cores from the red triangular hit-markers) + timer spine.
- `read-markers.py` (Python, `scripts/probe/.venv`) — 🅿 PARKED WIP SG pellet-marker counter; wraps the
  UNIGEO `scripts/unigeo/marker_detect2.py` + `marker_track.py` and locates the disc via the proven
  ammo-box template crosshair track (HoughCircles `find_disc` is only a fallback). White ring-marker
  detection is ✅ validated _given a disc centre_ but under-counts marciana's pellets, and its red-circle
  detection catches VFX (cores are the red TRIANGLES) — use `read-pellets.ts` for SG counts.
