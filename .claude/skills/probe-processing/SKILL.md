...
- an SG PELLET-LANDING question → the UNIGEO **marker detector** (`marker_detect2.py` + `marker_track.py`,
  ✅ validated _given a disc centre_) via the `read-markers.py` wrapper, or `read-pellets.ts` (CV popup
  survey, ⚑ candidate). **Both already exist — do NOT re-derive a pellet counter from scratch.** Caveat:
  `read-markers.py`'s automated disc-finder (`find_disc`) is the weak link — see its reader-table note.
...
| `scripts/probe/read-markers.py <video> --out D` (UNIGEO pellet-marker counter; wraps `marker_detect2.py` + `marker_track.py`)                          | per-shot **pellet landing** from the in-game pellet markers (white/red circles inside the grey aim disc) + radial position relative to the disc centre. Requires disc-centre tracking and per-HR scaling. **SG-only.**            | per-shot `{t_video, landed, cores, radii_white, radii_red}` + radial-distribution JSON; consumable by `scripts/unigeo/w1-fit.py` / `w4-part2-fit.py`                                                               | ✅ **VALIDATED 2026-07-22** on `soda tb control.mov` near HR-ON vs owner count: structural match, core totals 10 vs 11, cadence reproduced, recall ~0.70 with radially unbiased misses. **Preferred instrument for SG landing / UNIGEO geometry questions — but the ✅ validation SUPPLIED the disc centre; it covers the marker DETECTION, not the auto disc-finder.** The `scripts/probe/read-markers.py <video> [--at S --dur S --fps 60 --hr 0 --crop …] --out D` wrapper adds a disc-centre finder (`find_disc`) + tracking → `D/shots.json` `{shots[{t_video,landed,cores,radii_white,radii_red}], summary{totalShots,avgLanded,avgCores}}` + `D/discs.json` (per-frame disc centre/radius/conf). Needs `scripts/probe/.venv` (cv2/numpy). ⚠ **`find_disc` (HoughCircles on the translucent disc) is UNVALIDATED — owner-measured 0% usable crosshair; being reworked to reuse `read-pellets.ts`'s proven ammo-box-template crosshair track.** Until then, supply/verify the disc centre — do NOT trust `find_disc` output blindly. |
...
`docs/probes/drawn-geometry/marker_detect2.py` + `marker_track.py` and supplies a disc-centre finder
(`find_disc`) they lack (machine-reads pellet markers for SG landing/radial distribution). ⚠ Its
`find_disc` is unvalidated (owner-measured 0% usable crosshair); the validated part is the marker
detection underneath, which needs a correct disc centre.
...
- `read-markers.py` (Python, `scripts/probe/.venv`) — SG pellet-marker counter; wraps the UNIGEO
  `marker_detect2.py` + `marker_track.py` and supplies a disc-centre finder (`find_disc`) they lack.
  Marker detection is ✅ validated _given a disc centre_; `find_disc` itself is unvalidated (0% usable
  crosshair) — supply/verify the disc centre rather than trusting `find_disc`.
