...
## Stage 9 — per-unit manual-review doc (driver)

Write `scripts/kit-autonomy/manual-review/<slug>.md` — the owner's short-form manual-review artifact. It
consolidates, for a human to eyeball against the real kit:

1. **The real kit** (`data/characters.json` prose).
2. **The blind code-only reconstruction** — what the sim actually IMPLEMENTS, rebuilt blind from the engine +
   stripped override (audit-kit RECONSTRUCT: `npx tsx scripts/blind-rebuild/build-packet.ts <slug>` → spawn the
   `RECONSTRUCT.md` reconstructor on the packet + the loop-safe engine split). This is the single best short-form
   review: the owner reads "what the code does," independent of intent, and checks it against the real kit; the
   `codeDrivenSurprises` are the payload.
   - **Loop-safe engine split (mandatory):** the reconstructor must read the engine as DISTINCT small files —
     split the CURRENT `src/engine/sim.ts` into ~350-line chunks and `src/skills/types.ts` into ~200-line chunks
     under `scripts/blind-rebuild/code-bundle-now/` — NOT the raw files. Reading the raw `sim.ts`/`types.ts`
     (or re-reading any path) triggers LOOP_DETECTED.
   - **Known limitation:** in environments with an aggressive loop-detector (e.g. Qwen), the blind reconstructor
     may STILL loop even on the split. Fall back to the most recent cached reconstruction
     (`scripts/blind-rebuild/reconstructions/<slug>.json`) with a clearly-marked **DRIVER ANNOTATION** correcting
     any line re-encoded since the cache was built, and verify the rest still matches the current code. Label the
     provenance honestly (cache date + what was driver-corrected).
3. **The driver's executive summary** — what's modeled, the verdict, key findings, the residual.
4. **Owner spot-checks** — the systematic-prior-prone lines (scope / duration / trigger) + measurement-gated
   magnitudes + the board reading (and whether it's fit-exposure vs encoding).
...
- Design + decisions of record: `docs/kit-autonomy-decisions.md` (Part I lessons; Part II §10–§13 methodology;
  **§14 red-team revisions are AUTHORITATIVE**; Part III the privaty live run).
- Templates: `scripts/kit-autonomy/{TEST-FAITHFULNESS-REVIEW,BLIND-TEST-WRITER,BLIND-OVERRIDE-WRITER,RECONCILING-JUDGE}.md`.
- Cross-family model routing: `scripts/kit-autonomy/CROSS-FAMILY-PROTOCOL.md` (interop spec) +
  `.claude/skills/kit-autonomy-qwen/SKILL.md` (Qwen adapter/router) +
  `docs/handoffs/2026-07-23-kit-autonomy-model-router-claude.md` (Claude-side work order) +
  `scripts/kit-autonomy/prepare-cross-family-packet.ts` (de-contaminated packet builder).
- Manual-review doc (Stage 9): `scripts/kit-autonomy/manual-review/<slug>.md`; loop-safe engine split
  `scripts/blind-rebuild/code-bundle-now/` (current `sim.ts`/`types.ts` chunked to avoid LOOP_DETECTED).
...
- Reused machinery: `scripts/blind-rebuild/build-packet.ts` (redaction + leak-assertion model);
  `/kit-tdd` (test-writing discipline), `/audit-kit` (triangulation + RECONSTRUCT), `/kit-parse` (blind override authoring).
...
- 2026-07-23 — added Stage 9 (per-unit manual-review doc: blind code-reconstruction + driver exec summary +
  owner spot-checks) with the loop-safe engine-split procedure + the LOOP_DETECTED cached-reconstruction
  fallback; cross-family model-routing references (protocol + Qwen adapter + Claude handoff).
