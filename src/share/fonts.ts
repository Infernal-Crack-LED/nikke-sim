...
//
// NODE ONLY — never import this from a module the web bundle can reach. Import
// it once, BEFORE any canvas rendering (the renderers reference the 'Roboto'
// family via FONT in teamCard.ts). Registration is verified: if any face fails
// to register we throw here, at import time, so a misconfigured host refuses to
// boot instead of serving textless cards.
  ...
  if (!ok) {
    throw new Error(
      `infographics fonts: failed to register ${face} — text would render ` +
        `blank. Check that src/share/assets/fonts/ shipped with the build.`
    );
  }
}
