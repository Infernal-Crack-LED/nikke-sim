...
export function solveDp(model: DollModel, rarity: Rarity): DpTable {
  ...
  for (let L = 0; L <= maxP; L++) {
    cost[L] = new Array(nXp).fill(0);
    tier[L] = new Array(nXp).fill(null);
  }

  for (let L = maxP - 1; L >= 0; L--) {
    const cp = nextCheckpoint(model, L); // proc jump target (> L, already solved)
    for (let xi = nXp - 1; xi >= 0; xi--) {
      // high xp first: a miss only raises xp/level
      const xp = xi * XP_STEP;
      let best = Infinity,
        bestTier: ToolboxTier | null = null;
...
export function simulateDoll(
  ...
  while (L < model.maxPhase && guard++ < 100_000) {
    ...
    cost += model.kitWeight[t];
    exp += model.toolboxExp[t];
    feeds++;
    byTier[t]++;
    if (rng() < procChance(model, rarity, t, L)) {
      L = nextCheckpoint(model, L);
      xp = 0;
    } else {
      const m = applyMiss(model, rarity, L, xp, t);
      L = m.L;
...
export function monteCarlo(
  ...
  for (let i = 0; i < trials; i++) {
    ...
    feeds.push(r.feeds);
    byTier.R += r.byTier.R;
    byTier.SR += r.byTier.SR;
    byTier.SSR += r.byTier.SSR;
  }
...
export function calibrateWeights(
  ...
  for (let pass = 0; pass < 3; pass++) {
    // grid then refine around the best
  ...
  const weights: Record<ToolboxTier, number> = {
    R: 1,
    SR: bestSR,
    SSR: bestSSR,
  };
  const dpStar = solveDp({ ...model, kitWeight: weights }, rarity);
  const c = expectedConsumption(
    { ...model, kitWeight: weights },
    dpStar,
    rarity,
    0,
    0
  ...
  return {
    weights,
    dollsPer1000Mixed: dollsMixed,
    dollsPer1000Pure: dollsPure,
    pureConsumption: { R: c.R, SR: c.SR, SSR: c.SSR },
    bottleneck,
    wasteFrac,
  };
}
...
export function analysePaths(
  ...
  const kitEv = 0.3 * 10 * model.kitWeight.SSR + 0.55 * 20 * model.kitWeight.SR;
  return {
