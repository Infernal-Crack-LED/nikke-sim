...
export interface ValueTableInput {
  ...
  evalSets: (
    sets: string[][],
  ) => Promise<({ teamDamage: number } | null)[]>;
...
export async function buildValueTable(
  ...
  all.forEach((u, i) => {
    ...
    const ref = baseDamage(
      i < supports.length ? dps.slice(0, 2) : stickFor(u),
    );
    if (ref === undefined || !r) {
...
export function enumerateTeams(input: EnumerateInput): EnumeratedTeam[] {
  ...
  const b1s = input.poolB1.filter((s) => input.cooldownOf(s) <= cdShort).map(info);
  ...
  const emit = (
    ...
    heap.push(proxy, units.map((u) => u.slug));
  };
