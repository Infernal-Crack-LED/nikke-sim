...
**PROSE-FREE RUNTIME note (2026-07-16):** the engine no longer parses skill prose at runtime, so every
roster unit now has a file in `src/skills/overrides/` — for the previously-parser-only units it is
**materialized parser output** (note-marked, auto-generated by `scripts/materialize-overrides.ts`,
behavior-identical to the old runtime parse, NOT hand-verified). Promoting a baseline still follows
G1–G6 and simply REPLACES the materialized file. When promoting, meet the new override contract:
all three slots present + the required `unmodeled` field (verbatim skipped kit-text lines per slot —
see the kit-parse SKILL.md output contract). These baselines PREDATE that contract, so add `unmodeled`
during promotion (the audit table in each note lists the SKIPPED lines to copy over).
...
**CONSUMED (kit-parse rollout Wave 1, 2026-07-16):** liter, noir, volume, mana, mari, blanc — their
reviewed hypotheses were merged into fresh AUTHOR-mode kit-parse candidates and promoted to
