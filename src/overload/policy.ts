...
export function simulateTrial(
  ...
  const statsPresent = () =>
    [...tstats].every((k) => piece.some((l) => l && l.key === k));
  while (!statsPresent()) {
  ...
  const tierMet = () =>
    target.every((r) =>
      piece.some((l) => l && l.key === r.key && l.tier >= r.minTier)
    );
  while (!censored && !tierMet()) {
    const locked = [0, 1, 2].map(
      (i) => reqOf(piece[i], target) !== null
    ) as Locked;
    const n = locked.filter(Boolean).length;
  ...
  const establish =
    establishCost(model, maxLockP1) + establishCost(model, maxLockP2);
  return {
    rollsP1,
    resetsP2,
    ops: rollsP1 + resetsP2,
    rerollModules,
...
export interface McSummary {
  ...
  ops: {
    ...
    sd: number;
    se: number;
    pctiles: Record<string, number>;
  }; // total operations (rerolls + value-resets)
...
function histogram(
  ...
  upper: number,
  bins = 12
): Array<{ lo: number; hi: number; count: number; overflow?: boolean }> {
...
function density(
  ...
  upper: number,
  bins = 22
): Array<{ lo: number; hi: number; mid: number; count: number }> {
  const width = Math.max(1, Math.ceil(upper / bins));
...
function newCols(): Cols {
  return { ops: [], p1: [], p2: [], perm: [], temp: [], tl: [], censored: 0 };
}
function push(c: Cols, t: TrialResult) {
  c.ops.push(t.ops);
  c.p1.push(t.rollsP1);
  c.p2.push(t.resetsP2);
...
export function monteCarlo(
  ...
  return summarize(
    c,
    trials,
    target.length === 1 ? analyticSingle(model, target[0]) : undefined
  );
}
...
export function monteCarloBuild(
  ...
  for (let i = 0; i < trials; i++) {
    ...
    for (let j = 0; j < targets.length; j++) {
      const res = simulateTrial(rng, model, targets[j], {
        start: o.starts?.[j],
        fresh: o.fresh,
        cap: o.cap,
        lockPolicy: o.lockPolicy,
      ...
      P1 += res.rollsP1;
      P2 += res.resetsP2;
      P += res.moduleCostPerm;
      T += res.moduleCostTemp;
      L += res.tempLocks;
      c = c || res.censored;
    }
    tot.ops.push(R);
  ...
  const perPiece = per.map((p, j) =>
    summarize(
      p,
      trials,
      targets[j].length === 1 ? analyticSingle(model, targets[j][0]) : undefined
    )
  );
...
export function analyticSingle(
  model: OlProbModel,
  req: { key: Target[number]['key']; minTier: number }
): AnalyticSingle {
  const W = ALL_KEYS.reduce((a, k) => a + model.typeWeights[k], 0);
  const w = model.typeWeights[req.key];
  const g = [
    model.lineCountGates.slot1,
    model.lineCountGates.slot2,
    model.lineCountGates.slot3,
  ];
  ...
  return {
    ...
    expectedP1,
    expectedP2,
    expectedOps: expectedP1 + expectedP2,
