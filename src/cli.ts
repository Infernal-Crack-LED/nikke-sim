...
if (argv[0] === '--coverage') {
  ...
  for (const c of Object.values(data.characters)) {
    ...
    const unmodeled = o.unmodeled
      ? o.unmodeled.skill1.length +
        o.unmodeled.skill2.length +
        o.unmodeled.burst.length
      : 0;
...
const chars = slugs.map((slug) => {
  ...
  if (!c) {
    const suggestions = Object.keys(data.characters)
      .filter((k) => k.includes(slug))
      .slice(0, 5);
    usage(
      `unknown nikke "${slug}"${suggestions.length ? ` — did you mean: ${suggestions.join(', ')}` : ' (use --list to browse)'}`
    );
  }
  return c;
});
...
if (
  element &&
  !ELEMENTS.includes(element[0].toUpperCase() + element.slice(1).toLowerCase())
) {
