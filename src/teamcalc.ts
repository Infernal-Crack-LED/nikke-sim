...
export function canonicalTeamOrder(
  ...
  if (!focus || !sorted.includes(focus)) {
    return sorted;
  }
  const rest = sorted.filter((s) => s !== focus);
  if (rest.length !== sorted.length - 1) {
    return sorted;
...
function isLegal(slugs: string[], chars: Record<string, Char>): boolean {
  if (slugs.length !== 5 || new Set(slugs).size !== 5) {
    return false;
  }
...
function canFormLegalTeam(
  ...
  const slotsFor = (stage: 'I' | 'II'): number[] => {
    ...
    const ways: number[] = [];
    if (cds.some((c) => c <= CD_SHORT)) {
      ways.push(1);
    }
    if (cds.filter((c) => c <= CD_PAIR).length >= 2) {
      ways.push(2);
    }
    return ways;
  };
...
export function locksFeasible(
  ...
  if (locked.length > 5 || new Set(locked).size !== locked.length) {
    return false;
  }
  const c: Record<string, number> = { I: 0, II: 0, III: 0, Λ: 0 };
...
export function assignMustUse(
  ...
  for (const slug of mustUse) {
    if (seen.has(slug) || !chars[slug]) {
      continue;
    }
    seen.add(slug);
    if (locked.some((team) => team.includes(slug))) {
      continue;
    } // already pinned
    let bestT = -1;
    let bestRoom = -1;
    for (let t = 0; t < teamCount; t++) {
      if (locked[t].length >= 5) {
        continue;
      }
      if (!locksFeasible([...locked[t], slug], chars)) {
        continue;
      }
  ...
  const pickTeam = (
    ...
    for (const allowConflict of [false, true]) {
      ...
      for (let t = 0; t < teamCount; t++) {
        ...
        if (!allowConflict && conflicts(t, classes)) {
          continue;
        }
...
export function makeCalc(input: TeamCalcInput) {
  ...
  const constraintsOk = (slugs: string[]): boolean => {
    ...
    for (const { anyOf } of requiredAny) {
      if (!anyOf.some((s) => slugs.includes(s))) {
        return false;
      }
    }
    return true;
  ...
  const evalSets = (sets: string[][]): Promise<(TeamResult | null)[]> =>
  ...
  const focusFinalize = async (team: TeamResult): Promise<TeamResult> => {
    ...
    if (!charge.length) {
      return team;
    } // focus inert — canonical order already final
  ...
  const buildPool = (extraExclude: Set<string>): string[] => {
    ...
    if (meta) {
      ...
      for (const s of byBurst('III')) {
        if (!inPool.has(s) && meta.unitScore(s) >= 0.15) {
          topB3.push(s);
        }
      }
    }
  ...
  const seedTeam = (
    ...
    const coverStage = (stage: 'I' | 'II') => {
      if (team.length >= 5 || stageCovered(team, chars, stage)) {
        return;
      }
      ...
      } else if (pair.length && team.length + 1 <= 5) {
        team.push(pair[0]);
      } else if (cands.length && team.length + 1 <= 5) {
        team.push(cands[0]);
      }
    };
  ...
  const bestTeam = async (opts?: {
    ...
    const pushStart = (t: string[]) => {
      if (!starts.some((s) => key(s) === key(t))) {
        starts.push(t);
      }
    };
    if (opts?.seedsOnly && !extraStarts.length) {
      return null;
    }
    ...
    if (cands.length) {
      ...
      if (legal(legacySeed)) {
        pushStart(legacySeed);
      }
      for (const { set } of ranked) {
        if (starts.length >= ENUM_REFINE_TOP) {
          break;
        }
        pushStart(set);
      }
    ...
    if (extraStarts.length) {
      ...
      for (const [k, r] of seeded.entries()) {
        ...
        const sc = score(r);
        if (sc >= POLISH_SEED_FRAC * floor) {
          climb.push(extraStarts[k]);
        }
        if (sc > bestScore) {
          best = r;
          bestScore = sc;
    ...
    if (opts?.seedsOnly) {
      return best && focusFinalize(best);
    }
        ...
        for (const [i, t] of cur.entries()) {
          for (const u of [...t.slugs].sort(cheapest)) {
            ...
            for (const v of takeBack) {
              ...
              if (!canFormLegalTeam(rest, chars)) {
                continue;
              }
