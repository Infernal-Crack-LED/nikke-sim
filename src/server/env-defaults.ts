...
//
// Why this exists: the render host resolves its TTFs and portraits relative to
// import.meta.url, which is correct from source (tsx/vitest) but NOT from the
// esbuild bundle (dist-server/index.js — one flat file). The server build
// (scripts/build-server.mjs) copies src/infographics/assets/ next to the
// bundle, and the deploy's dist/ ships the portrait thumbs; this module points
// the render host at those copies. From source none of the candidate paths
// exist, nothing is set, and the render host's source-tree defaults apply.
