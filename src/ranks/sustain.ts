...
function charFor(ctx: RanksCtx, slug: string) {
  return (
    ctx.characters[slug] ?? syntheticFor(slug) ?? (NOOP_CHARACTERS as any)[slug]
  );
}
...
export function sustainFor(
  ...
  const deps = {
    ...
    overrides: Object.fromEntries(
      slugs.map((s) => [
        s,
        ctx.deps.overrides[s] ??
          ({ slug: s, skill1: [], skill2: [], burst: [] } as any),
      ])
    ),
  };
...
function evaluateLines(
  ...
  for (const line of lines) {
    ...
    if (line.kind === 'lifesteal') {
      ...
      const windows = procs.map(
        (t) => [t, t + (line.windowSec ?? 10)] as const
      );
      parts.lifesteal +=
        (((pct / 100) * damageInWindows(tl.damage, windows)) / tl.maxHp) * 100;
      continue;
    }
    ...
    procs.forEach((t, i) => {
      const rampSum = line.ramp
        ? line.ramp
            .slice(0, Math.min(i + 1, line.ramp.length))
            .reduce((a, b) => a + b, 0)
        : pct;
...
const PER_UNIT: Record<string, (tl: Timeline, withProfile: boolean) => Parts> =
  {
    ...
    mana: (tl) => {
      const casts = tl.burstCasts
        .filter((c) => c.slug === 'mana')
        .map((c) => c.sec);
      const shots = tl.shots.filter((t) =>
        casts.some((c) => t >= c && t <= c + 10)
      );
      return {
        heal: Math.floor(shots.length / 10) * 2.04 * 5,
        shield: 0,
        lifesteal: 0,
      };
    },
    ...
    pepper: (tl) => {
      ...
      const casts = tl.burstCasts.filter(
        (c) => c.slug === 'pepper' && c.sec >= stackedAt
      );
      return {
        heal: healLB + casts.length * 27.22 * 5,
        shield: 0,
        lifesteal: 0,
      };
    },
