...
//
// Pure and platform-free like the rest of core/ — moved here (2026-07-29) from
// web/src/resources-data.ts so the server-side resources infographic can build
// off the SAME table the web Resource Calculator reads; web/src/resources-data.ts
// now re-exports from here so its existing imports keep working untouched.
...
// ORDER IS LOAD-BEARING: the resources infographic renders one section per
// entry, kraken first — see core/resourcesCard.ts.
...
// Basename a /nikke-icons/ path resolves to for node/icons.ts's loadIcon(name,
// size) — e.g. '/nikke-icons/res_module_fragments.webp' → 'res_module_fragments'.
// Derives from the SAME fragmentIcon string BOSS_TABLES already carries, so the
// server-side icon loader can never name a different file than the web page's
// <img> tags point at.
export function iconBasename(iconPath: string): string {
