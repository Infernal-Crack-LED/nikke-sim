...
the core fraction on auto is whatever the _floored_ reticle geometry gives vs each band's core — it must be
...
_manual_ reticle is **2.9px (2622)** — smaller than even the far core (~16–17px) — so manual trivially
