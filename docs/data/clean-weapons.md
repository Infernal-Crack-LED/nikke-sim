...
reading ~1.4% low against a popup — answered: missing bond bonus in the recording (**answered-questions U33**).
...
exactly), so the error is a near-pure scalar — harmless to the _shape_ of a fight, fatal to the
...
never-burst constraint (S1 block 2 is _"Activates when using Burst Skill"_, S2 is _"Activates
during Full Burst"_), but S1 block 1 does not:
...
- `rei` (SMG/Water, _not_ `rei-ayanami`) — clean, but **not owned**.
- `mica-snow-buddy` — **not clean**: _"Max Ammunition Capacity ▲ 40% continuously"_ raises fire
