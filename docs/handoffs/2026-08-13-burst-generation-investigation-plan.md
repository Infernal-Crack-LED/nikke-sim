...
- **The 22f / 30f / 30f chain ladder and the 10s Full Burst.** Frame-measured; footage-confirmed.
...
for the class, not evidence the class is now empty.
...
that. What remains open is the _focused_ side's per-unit value.
...
**WHY IT MATTERS AND WHY IT CAN HIDE.** The ×2.5 (250 column) is measured twice over. **The 200 and
350 columns are not.** The engine's own comment records that the only measurement ever taken at the
200 column — `cinderella` (RL/Electric) reading ~2.2–3.1× — _contradicts_ 2.0×, which is why
`vesti-tactical-upgrade` was pre-emptively pinned rather than allowed to inherit it. The focused unit
is the top generator on 5 of 9 off-count teams (~30% of team generation), so an error here is large
for exactly one unit per team — and invisible on any team whose focus is a non-charge weapon.

**PREMISES TO RE-DERIVE.**

1. Which units in the nine off-count comps are focused AND carry a charge weapon, and what column
   does each resolve to (250 / 200 / 350 / pinned)?
2. What exactly does the `cinderella` 2.2–3.1× read consist of, and is it a clean per-shot
   measurement or a back-derived one?
...
**DECISION RULE.** If every off-count team's focused charge unit resolves to the **measured 250
column**, this item cannot explain the shortfall — close it and say so. If any resolve to 200/350,
the deliverable is a recording request, not a value change.
...
over-generate. Same question for MG belt rounds. Four of the nine off-count comps seat an SG
(`soda-twinkling-bunny` ×2, `noir`, `naga`, `arcana-fortune-mate`), and MG carriers appear in six.
...
the SG-seated comps and moves nothing on the SG-free ones, that localizes it. It still does **not**
...
4. **Item 3** (focus column) — likely closes as "all 250, cannot explain it".
