...
One owner ask is outstanding (`2026-08-01-OWNER-ASK-shot-count.md`) which would resolve a 3.4× swing on
that headline.
...
| Missing-shot **3.9–6.8%**                      | **MEDIUM, and a FLOOR not a point estimate.** Gate-validated on `marciana` only (5/5 owner shots recovered). `guilty`/`noir` internally consistent, no ground truth. **`isabel` LOW-MEDIUM** — raw vs admissible differ 3.4× on a driver-authored rule. |
| Ammo-arbiter read rate **52–71%**              | **Known limitation.** The atlas was white-only; 72 red glyphs were harvested to get this far. A per-video red harvest would tighten everything.                                                                                                         |
...
**Branch `fix/pellet-reader`: `origin` at `8d75008`, HEAD at `688f4a1`, FOURTEEN commits unpushed.**
...
`scripts/probe/ammo-atlas/`.

## 7. Open, in priority order

1. **The owner ask** — `2026-08-01-OWNER-ASK-shot-count.md`. A hand shot-count on one `isabel` clip
   decides a 3.4× swing on the headline. **Blocked on the owner; do not proceed past it by guessing.**
2. **Per-video red-digit atlas harvest** — no owner time; lifts the 52–71% read rate and tightens every
   missing-shot number. The owner has been asked for a nod.
3. **The unexplained remainder of the cold bias** — after the partial channel, roughly half to
   three-quarters is still unaccounted for, **with no live candidate.** Do not manufacture one; §4 is
   what happens when you do.
4. The 60fps localization instability (2 of 4 windows locked zero frames; `run21`/`run21b` have no
   valid consecutive positions at all). Gate 1 is a whole-video conjunction and cannot see it.
5. `count-pellets.py:692` docstring vs line 699 carry-forward contradiction.
6. The generator's radial envelope — it places every label strictly inside the counting window (884
   labels, r=42.0–157.1) while ~10% of real marks fall outside. No synthetic measurement can see that.
7. Phase 2 steps 4–6, still blocked on the owner's Decision 1; the remaining `/logic-gate` pre-op
