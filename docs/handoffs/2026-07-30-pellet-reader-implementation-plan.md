...
> **⚑ Flagged open item for the OWNER-INVOKED `/logic-gate` pre-op on the Phase 2 design (recorded
> 2026-07-31, not resolved here — do not change the numbers above and do not decide where the gate
> should be measured instead).** The Phase 1.2 labeled set is generated by
> `make-synthetic-pellets.py` from the SAME lifecycle model (`lifecycle_scale()`/
> `lifecycle_alpha()`) that Phase 2's template scoring will be built against — so a candidate that
> merely reproduces the generator's own construction is partly passable against this 0.90/0.80 gate
> by construction, independent of whether the template generalizes to real footage. The mandatory
> held-out real-data check specified above (the ⚠ Honest limit: the 6 owner-counted shots in
> `scripts/tests/fixtures/pellets/groundtruth-f8-11.json` **and** the `docs/probe-data/*-sg-band.json`
> anchors, both required to pass) is already the right guard against this — flagging it here so the
> pre-op explicitly considers whether the 0.90/0.80 gate needs to lean on that held-out check rather
> than the synthetic set alone, not deciding it in this pass.
