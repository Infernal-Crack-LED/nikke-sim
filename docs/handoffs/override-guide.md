...
1. `npx tsx scripts/kit.ts <slug>` — **the single entry point for any per-character claim.** Prints the
   character's data, the flattened weapon spec (incl. reload parts), the RESOLVED skill prose, the
   override's blocks (what the engine runs), its unmodeled text + caveats, and the parser draft (clearly
   labelled NOT what runs). **Cite the `=== skillN text ===` sections — never grep the raw JSON.** The raw
   `role` blob is `--raw`-only because its `description_localkey` fields are BASE-KIT templates: for a
   Treasure unit they are not what the override is synced from and the values differ wildly (`privaty`
   85.79/1089/457.87 raw vs 256.17/1687/1407.64 Treasure). The `treasure` flag in the header tells you
   which applies.
...
| `stageEnter`, `stage: N`             | when the chain REACHES stage N — gauge-full (N=1) or the stage-(N-1) cast (N=2,3) | for "when entering Burst Stage N". Fires one chain step BEFORE the stage-N cast, and also on chains that reach the stage and then expire                                                               |
| `stageCast`, `stage: N`              | when a stage-N burst is CAST by anyone                                            | for "when an ally uses a Burst Skill". One chain step LATER than the same-numbered `stageEnter` — do not substitute one for the other                                                                  |
