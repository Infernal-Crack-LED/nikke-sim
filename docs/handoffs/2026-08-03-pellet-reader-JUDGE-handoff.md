...
3. ⚑ **`debounce_shots` exists TWICE** — in `count-pellets.py` and again at **`read-pellets.ts:349`** —
...
2. **`debounce_shots` minimal fix — ⛔ OWNER-GATED.** `cap_cadence` (~3 LOC) and `resplit` (~10 LOC)
   ...
   **`read-pellets.ts:349` is a second implementation that must change in lockstep**. ⚑ Do not read
