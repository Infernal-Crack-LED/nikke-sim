...
- Non-damage ENEMY-debuff applications (re-applications/refreshes included) credit the caster's
  full datamined `targetPerTrigger` once per application, GENERATE-BY-DEFAULT for every trigger
  shape (`applicationGauge` + `isGeneratingApplication` in `sim.ts` — owner rulings + magnitude +
  scope rulings 2026-08-16; the refresh half rests on owner-trusted community-expert testimony).
  Excluded: per-shot on-bullet riders (`shotFired`/`fullCharge`/`chargeCounter`, anti-double-count), permanent
  auras/999-sentinel statuses (not events), `APPLICATION_NONGEN` = `noah`/`snow-white-heavy-arms`,
  and blocks that also deal damage (already credited per impact). Live footprint:
  `emma-tactical-upgrade`, `eunhwa`/`ludmilla`/`exia` (lastBullet), `rosanna`,
  `sakura-suzuhara`/`signal` (hitCount), `brid-silent-track` (Wind-gated); zero graded exposure by
  mechanism (no pinned comp seats a live-credit unit — slug-set intersection empty). Spec:
...
| `shotFired`              | Every owner trigger pull                                             | 28 override files carry it (file count, post-2026-08-24 fullCharge migration; soda-twinkling-bunny, phantom, mari, …) |
| `fullCharge`             | Every CHARGED pull — charge weapons release only at full charge, so per-pull for them; silent on MG belt rounds / non-charge swap states | 14 units — the audited-slice "Full Charge" kit lines (mint, prika, helm, liberalio, snow-white-heavy-arms, …); the roster tail stays on the shotFired proxy pending per-line verification |
...
| `stageEnter`             | When the chain REACHES stage N — gauge-full (N=1) or the stage-(N-1) cast (N=2,3) | 13 units (cinderella, ein, flora, laplace-ultimate-hero, mast-romantic-maid, maxwell-ordinary-mechanic, mihara-bonding-chain, mint, neon-blue-ocean, rei-ayanami, snow-white-heavy-arms, soda-twinkling-bunny, yukiko) |
...
| `requiresSelfStatus`               | Only while the OWNER carries the NAMED status on themself, opened by a `selfStatus` effect (per-(unit,name) map — no cross-unit read in either direction, unlike the shared boss-status channel) | asuka-wille (`'Annihilation State'`)                                                 |
...
| `resourceGate`                     | Fires only when a named resource pool is within [min,max]. Also the roster's answer to an **"at max stacks" kit gate** — a monotone pool counting the stacking event, read at `min: cap` (laplace's `heroVision`, 2026-08-12), which needs no stack-count block gate | 25 units — soda-twinkling-bunny, laplace, laplace-ultimate-hero, maxwell-ordinary-mechanic, mint, phantom, power, rouge, rupee, rupee-winter-shopper, sin, soda, d, e-h, elegg-boom-and-shock, exia, guillotine, guillotine-winter-slayer, julia, mana, mica-snow-buddy, mihara-bonding-chain, novel, pepper, rumani |
...
| `levelScale` / `levelConst` (effect-level, 2026-08-25)                      | SKILL-LEVEL scaling for values the table lookup cannot resolve. `levelScale: {field: [anchors]}` names the table anchor(s) a DERIVED value came from (a fold of two kit lines, a time-averaged stack ramp, a stack-cap product) and scales it `authored × (Σ anchors@L)/(Σ anchors@10)`; `levelConst: [field]` marks a field VERIFIED level-invariant so it stops warning. Anchors resolve WITHIN ONE SLOT — a cross-slot anchor is inexpressible. An unresolvable anchor is a structural ERROR, and the kind→scalable-fields map is exported by `scale.ts` and imported by the validator (never mirrored). Inert at 10/10/10, where `scaleBlocks` early-returns. | `levelScale`: ada, arcana-fortune-mate, asuka-wille, cinderella, ein, emilia, eve, guillotine-winter-slayer, liberalio, little-mermaid, maiden-ice-rose, mana, mast, mast-romantic-maid, nayuta, neon-vision-eye, sakura-bloom-in-summer, snow-white-heavy-arms, soline-frost-ticket · `levelConst`: ada, arcana-fortune-mate, eve, prika, rapi-red-hood, red-hood, snow-white-heavy-arms |
...
| `flatDamage` (+`delaySec`/`charge`/`chargeMultPct`/`requiresPulls`/`gaugeHits`) | Flat hit (flighted-and-snapshotted / charge-bucket / pull-gated); `gaugeHits` credits `skillGauge` once per sub-hit on sequential multi-hit riders without expanding the damage instance | asuka-wille, rapi-red-hood, snow-white, cinderella-crystal-wave, nayuta, red-hood, ada, zwei                                  |
...
| `copyResource`                                                              | Snapshot one named pool into another at the firing block's apply time, clamped to the target's [min,max] — the stack-mirror cast-snapshot shape: a burst freezes a live stack count into a mirror pool BEFORE its own zeroing spend, and a `perResource` DoT reads the frozen mirror                                                                                                          | mihara-bonding-chain (burst: `mirror` ← `ensnaring`)                                                                          |
...
| `hitsPerShot`               | Base SG/MG pellet/belt-round count per pull            | **36 units** — char-static from `data/characters.json` (sync: `shot_count × muzzle_count`, with the `modernia` carve-out): 28 × 10 (every SG), 8 × 2. **No override sets it**; it is not a `charFixes` field. |
...
| `gauge-fill.py --team`                             | **team-HUD burst-gauge fill traces** inside refill windows (team-sum)    | magenta row-vote lock onto the 134px team charging bar (rows 491–498, x 2477–2610 on all three team recordings); loud refusal outside width bounds; nine artifact flags + second-pass `offCurve` | ✅ fixture-pinned (`scripts/tests/gauge-fill-team.test.ts` + `scripts/tests/fixtures/gauge-fill-team-u8-g-30fps.json`); full instants match the chain ladder 12/12 per video. ⚠ absolute low-fill levels unreliable (owner-ruled artifact); ~1.5s blind spot at every window START; single-step magnitudes unvalidated (gain-pulse) |
| `gauge-fill.py --reflag`                           | re-apply the `offCurve` dominant-curve artifact flag to an already-emitted team trace | pure second pass over a committed `fill-trace-*.json` bundle; idempotent; flags reads inconsistent with the window's dominant monotone curve | ✅ deterministic, pinned by `scripts/tests/probe/habc-classification.test.ts` (asserts pure-additive offCurve additions for the classification arms) |
| `gauge-fill.py --diag`                             | team-only **raw pixel diagnostics** for every frame before state classification (`fill`, `magenta`, `red`, `green` fractions) | records the dark-track arithmetic and colour-mask fractions unconditionally; designed for the opening-window observable where the drained FB bar owns the widget slot | ✅ deterministic raw-pixel arithmetic; validated at generation time by byte-identical reproduction of the default trace; pinned by `scripts/tests/probe/fill-trace-opening.test.ts` |
| `fb-count-matrix.ts --credit-schedule`             | the sim's **per-frame gauge-credit schedule** (frame, unit, amount, kind incl. `fillGauge`) per unlocked window | event tap + engine-data reconstruction, self-validated per run: endpoint vs `gaugeGenerated`, first-30s `DBG_GAUGE` truth, truncated-run diffs (prefix determinism asserted); a `flatDamage` carrying `gaugeHits: N` is credited N times per impact, matched to its effect by (slug, srcSlot, atkPct); LOUDLY voids amounts it cannot reconstruct (e.g. SG spray, a ramp-scaled or ambiguous `gaugeHits` coefficient) | ✅ exhaustively engine-exact on iron sweep (run G) + T5 wind-weak + `misc B3s (run I order)` (SG) + `N5 snowwhite-HA fire` (the `snow-white-heavy-arms` `gaugeHits` seat); pinned by `scripts/tests/battery/credit-schedule.test.ts` |
...
| `fill-trace-compare.ts` opening observable         | the reader-blind opening window: cumulative gauge at first clean read vs the sim schedule's opening, plus raw hold-span diagnostics from `--diag` | pure functions over the committed bundle + `gauge-fill.py --team --diag` `diagHold`; separates "sim under-credits" from "sim credits the same total on a different shape" | ✅ pinned (`scripts/tests/probe/fill-trace-opening.test.ts` reproduces committed results without the recordings) |
| `classify.py`                                      | popup / damage-number **colour class presence** (white normal / orange crit / red core / green heal) in a frame region | deterministic CV colour-mask pixel counting over `crosshair|character|full`; no OCR | deterministic; numeric values still read by eye; useful for colour-class presence/absence |
...
| `gauge-magnitude.ts`                              | a unit's **steady per-pull burst-gauge credit** from a committed solo bar trace, as three estimators: E1 anomaly-aware count-to-fill, E2 telescoping run-mean, E3 run-height linearity | pure arithmetic over committed `docs/probe-data/*-gauge*.json` traces — reads no video. Classifies departing credits first by a hypothesis-blind ±3-render-column rule, then estimates; `SE_pooled` = max(quantization model, empirical scatter, noise-floor inflation) | ✅ deterministic, pinned by `scripts/tests/probe/gauge-magnitude.test.ts` (19 assertions, incl. that the declared candidate value appears only AFTER the post-verdict marker). ⛔ **E2 is gain-SENSITIVE**: an instrument gain `g` biases it by `(g−1)·P` while biasing E1 the OPPOSITE way by `(g−1)·S/m`. `anis-star`'s A3 trace carries an unadjudicated `rawOverTrue = 1.064` — until that is settled on the target footage, an E1/E2 agreement is bounded, not clean (QUEUE N1b) |
