...
| `pullsPerSec`               | Per-unit measured fire-cadence override                | **1 unit** — `jill` (2.5/s, video-measured). `weaponSwap.pullsPerSec` is a live capability (`sim.ts:2426`) with **zero** current users. **Same-weapon swap fallback (2026-08-10):** when `u.swap.weapon` is undefined (the swap doesn't change weapon class) and `u.swap.pullsPerSec` is unset, the swap branch falls back to the unit's own `pullsPerSec` before the weapon-class default (`sim.ts` ~3715) — provably inert for every carrier but `jill` today, since she is the only `pullsPerSec` holder. |
