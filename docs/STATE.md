...
| `SMGQUANT` | **OFF** (SMG 24/s) | `=1` quantizes the SMG fire interval to whole frames → **20.0 rounds/s**, the MEASURED cadence (ammo counter, 2 range bands). 1440 rpm = 2.5 frames at 60 fps; SMG is the only weapon in the roster whose datamined rate isn't a whole frame count, so this is a no-op for every other class. Fixes the whole SMG class (liter 1.208→1.031, chisato 1.154→0.975, quency-escape-queen 1.174→1.046); all 11 measured FB assertions hold in both arms. **Flip is P0-queued but BLOCKED on one unexplained `modernia` MG ammo-spend test failure** — see CLAUDE.md + `docs/probe-runs.md` 2026-07-23 | `SMGRATE=<n>` pins a rate directly | → probe-runs 2026-07-23 |
...
| `gainPierce` | Timed "Gain Pierce" window | ade-agent-bunny, grave, milk-blooming-bunny |
