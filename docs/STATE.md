...
| `hitsPerShot`               | Base SG/MG pellet/belt-round count per pull            | **35 units** — char-static from `data/characters.json` (sync: `shot_count × muzzle_count`, with the `modernia` carve-out): 27 × 10 (every SG), 8 × 2. **No override sets it**; it is not a `charFixes` field. |
