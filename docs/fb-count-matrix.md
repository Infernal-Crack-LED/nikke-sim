...
a large fraction of burst generation" survives the uncertainty even though "it generates 59%" does
not.**
...
> **NOT re-derived: the 9.4s sensitivity pair (74% / 61%) above** — that is hand-derived commentary
> with no mode in the instrument, and it does not reproduce from the published columns, so it is
> left as-authored rather than guessed at. It is directional, not a measurement.
