...
## H-A/H-B/H-C classification on the bar-paint-anchored statistic — branch H-C fires on iron sweep (run G), T5 wind-weak does not close; no cross-comp class (2026-08-15)
...
basis clause and branch pinned there; pre-op APPROVED-WITH-REVISIONS R1–R5, all executed).
...
| :----------------- | -------------: | ---------: | ---------------------: | ----: | -----: | ---------------: | ----------------: | -----------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------------- |
| iron sweep (run G) |             10 |      1.620 |                  0.304 | 1.828 |  1.071 |            0.258 |             4.715 |                                                  4.133 | branch 1: H-C mass present (share of rate 0.238); same-arm H-B band hit demoted to candidate remainder, descriptive only (R4) |
...
occupancy/size. Bridged mass 132.9% / 113.8%. **Cross-comp rule: the branches disagree ⇒
per-comp findings only — no class-level claim from this run.** The prior ~1.7–2.0× logged ratio
was not used; the fresh ρ values are 1.62/1.76.
...
true-FB-end-anchored rate by up to that duration share. Iron's closure residual 0.258 is
marginally over the 0.25 limit — branch 1 does not read the decomposition, but the demoted H-B
remainder carries that residual. T5's ceiling test is vacuous by construction. Any class reading
here is n=2 comps.
...
⛔ **Nothing enacted.** No engine constant, override, snapshot, DECISIONS/STATE/QUEUE change. The
discriminating next measurement per the packet's own map: H-C → source hunting at the
unattributable instants on iron sweep (run G); the T5 arm needs a symmetric-threshold or
per-unit-cadence statistic before it can discriminate anything. `bash scripts/verify.sh` green.
