...
`8621d670`. **Owner-accepted without the `/scientific-method` gate** — the statistic is
arithmetic over two already-committed, already-gated artifacts, with no new measurement and no
enactable surface.

This is the judge-ranked item (2) of the 2026-08-15 H-A/H-B/H-C classification run, and it
addresses the SECOND of the two grounds on which that run's "H-C mass present" stamp was struck
(the C4 noise gate lacking power). It does not address the first ground.

**Statistic.** `rate_corrected = (E − f·Q) / T`, with E = 80 event bins, Q = 429 quiet bins,
T = 16.9667 s (clean-bin-time denominator) — all read from the committed classification
artifact, not recomputed — and f the Wilson 95% one-sided upper false-event rate from the solo
#2 measurement. Threshold = 1.15 × C_ceiling = 1.15 × 3.5935 = 4.1325/s.

**Result: R1 — the excess survives.** Primary (f = 0.55%, 492 quiet bins): correction 2.36
bins, corrected rate **4.576/s**, +10.7% over threshold. Pooled (f = 0.45%, 597 bins):
correction 1.93 bins, corrected rate **4.601/s**, +11.3%. The correction removes ~3% of the
event bins; the R1 outcome holds for any falseRate up to the indifference point **2.304%**,
which is 4.2× the measured input. Shelving would require f ≥ 4.436% — ~8× the measurement.

**Stamp (LOG-class, the limited form the striking judge pre-named):** _H-C-candidate event-rate
excess survives noise correction — observed, not noise-manufactured at any plausible falseRate._
No engine change, no constant change, no classification re-issue.

**Mandatory caveats (all pre-committed in the packet):**

- **The closure residual 0.2579 stands** — iron sweep (run G)'s CLASSIFICATION remains
  MIXED/INCONCLUSIVE. This run produces a detection, not a classification, and cannot void the
  clause (2026-08-15 blind post-op ruling; harness lesson 1).
- **MAR-denominator dependence is the load-bearing caveat.** The excess exists only under the
  clean-bin-time estimator. Under the full-window-duration denominator (23.618 s) the rate is
  3.387/s — 5.7% BELOW the raw ceiling, i.e. no excess at all. Clean-bin-time is the principled
  estimator (events are only observable in clean bins, and nonzero `bridgedMass` proves events
  do occur inside excluded spans), but the assumption is stated as a band, not tested.
- **n = 1 comp.** T5's ceiling was cap-saturated at 30/s by `cinderella-crystal-wave`'s 1126
  credit instants, making its H-C detector vacuous — so the detector is structurally n=1 and
  generality is unknown. A third comp with a non-vacuous ceiling (five slow-firing units, no
  MG/SMG) is the outstanding resolver.
- **Solo→team noise transfer.** f was measured on solo footage; the solo clip is documented
  noisier than the team clip, so it bounds the team noise conservatively.

**Controls.** C1 artifact replay (iron numbers from the committed artifact, cross-checked by
`scripts/tests/probe/habc-classification.test.ts`, 8/8 green); C2 falseRate provenance (read
from `docs/probe-data/anis-star-solo2-gauge.json`, which cleared its own basis floors at 3.3×);
C3 arithmetic (indifference points derived by two independent algebraic paths, identical).

**One packet clause was NOT executed, deliberately (2026-08-17).** §G directed that the
classification artifact's `observedCeilingExcess` field be rewritten to
"observed, noise-correction-surviving". That field is generated by the classifier
(`scripts/probe/fill-trace-compare.ts:2188`) and pinned by the replay test at
`habc-classification.test.ts:184`; hand-editing it would desync the artifact from its own
instrument and attribute this later reading to the classification run. The clause is marked
SUPERSEDED in the packet; the cross-reference lives in this run's artifact (`crossRef`) instead.

---

## 2026-08-17 — AR + SG solo gauge-bar reads (NO new footage): `scarlet`'s AR row validated, and `drake`'s SG per-trigger credit measured NON-CONSTANT — an independent mechanical corroboration of U40

Closes the avenue-3 residue of
`docs/handoffs/2026-08-14-burst-generation-remaining-avenues.md` — SMG and MG were bar-validated
2026-08-15 on owner-delivered footage; **AR and SG, the last two never-bar-checked families, both
read out of recordings ALREADY IN THE TREE.** No recording ask was needed.
Artifact: `docs/probe-data/ar-sg-solo-gauge-2026-08-17.json` (verdict-free); traces
`docs/probe-data/scarlet-ar-solo-gauge-trace.json` + `docs/probe-data/drake-sg-solo-gauge-trace.json`;
instrument `scripts/probe/fill-trace-compare.ts solo-rate` (new subcommand); replay pin
`scripts/tests/probe/solo-rate.test.ts` (6/6 green).

Recordings: `docs/probes/ar-sg-smg/scarlet ar.MP4` (`scarlet` — Scarlet, AR/Electric/Burst III)
and `docs/probes/ar-sg-smg/drake sg.MP4` (`drake` — Drake, SG/Fire/Burst III). Both 1206×2622
@60fps rotation-90, the same post-rotation geometry as the 2026-08-15 clips. Identity confirmed
from each clip's own Battle Records screenshot (one named unit + four `EMPTY SLOT`, Total Battle
Time 03:00) — not from the filename.

**Why a solo Burst III unit is the clean basis.** She makes ZERO full bursts: no Burst I exists to
open the chain. So the bar fills ONCE, monotonically, and then sits full for the rest of the fight
— one uninterrupted generating window with no drain side, no chain zeroing, no FB gauge lock and
no teammate credit to disentangle. Both traces show exactly that shape, which is itself a check on
the standing burst-eligibility fact.

**Bar geometry re-derived independently.** `gauge-fill.py --calib-frame 600` self-calibrated on
the `scarlet` frame set (`crop=400:60:2222:470`) and locked `borderRow 31, x0 252, x1 390` in crop
coordinates = **y501, x2474–2612, 138px absolute** — byte-identical to the solo bar hand-verified
on three DIFFERENT recordings on 2026-08-15. That is an independent re-derivation of the HUD
geometry on a separate recording session, not a reuse of the prior number. (Cropping to the HUD
region also removes the documented mis-lock cause — the bright-sky terrain never enters the frame.
Self-calibration still mis-locked at **158px** on `drake` when the calib frame landed on her full
GREEN bar; re-read with the explicit `--bar` hatch. Always pass `--bar`, or a calib frame taken
mid-FILL.)

**`scarlet` — AR row VALIDATED (datamined `targetPerTrigger` 90 = 0.90 %/shot).** Read per
MAGAZINE, because 0.90 %/shot at 12 shots/s is below the reader's per-trigger resolution (one
0.72pp column per 1.25 shots at 30fps) while the 2.30s reload is a hard plateau. The four
magazine deltas are **20.3 / 19.5 / 18.9 / 19.6 pp** (sd 0.57), over her datamined `max_ammo` 20
= **0.9788 %/shot raw = 0.9199 after the anchor bias**, ratio to column **1.0221 (+2.2%)** —
the same few-percent-hot residual the SMG and MG reads carried. Structure control: the observed
plateau cycle is 4.275s against 20/12 + 2.30s reload = 3.967s, the 0.31s remainder covered by her
datamined `spot_first_delay` + `spot_last_delay` (20 + 20).

**`drake` — the SG per-trigger credit reads 0.847 of the datamined column, and is NOT CONSTANT.**
Her steps resolve individually (9.00 %/trigger is ~12 columns, and 1.5 triggers/s is slow), so
this is a direct per-trigger read: 10 clean intra-magazine steps of **8.0 / 7.9 / 7.3 / 8.0 / 8.7
/ 8.6 / 7.3 / 6.5 / 9.4 / 9.4** raw, mean 8.11 raw = **7.6222 %/trigger after bias**, against a
datamined `targetPerTrigger` 900 = 9.00 %/trigger for all 10 pellets landing → ratio **0.8469**.
Cadence control: mean step gap 0.666s → **1.5015/s vs her datamined 90rpm = 1.5/s**, so the steps
found are one-per-trigger, not merged or split. Magazine control: the first magazine is exactly
9 credited triggers before a hard reload plateau = her datamined `max_ammo` 9.

**The load-bearing statistic is the VARIANCE, not the mean.** Under **per-TRIGGER** crediting
every pull must credit the identical value, so the only admissible step-to-step spread is the
bar's own column quantization — a step is a difference of two column-quantized reads, sd
`(0.72/√12)·√2` = 0.296pp. Observed sd is **0.936pp**: **varianceRatio 10.02**. Per-trigger
constant crediting is refuted directly, on an instrument no prior SG work used. This is an
**independent mechanical corroboration of the U40 owner ruling** (a missed SG pellet generates NO
gauge; SG gauge credits per LANDED pellet), which until now rested on owner testimony with the
method not stated. The mean ratio 0.847 is then the mean landed-pellet fraction over the window —
a per-unit quantity, not a re-derivation of the datamined column.

**Caveats.** n=1 recording and one unit per family; neither is a class-wide claim. `drake` is NOT
among the six units in `docs/probe-data/per-unit-sg-landing.json`, so her 0.847 has no per-unit
landing figure to be scored against (the four measured units' mid-band ratio-to-table spread is
0.92–1.00). Her fill window sits entirely in the boss's opening **mid** band
(`docs/data/range-data.md`: starts mid, near at ~2:27), so this is a single-band read and one fill
