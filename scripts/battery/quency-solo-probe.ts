...
for (const { slug, real } of SOLO) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    acc.push(u.totalDamage);
    pulls = u.pulls;
    bk.normal += u.breakdown.normal;
    bk.skill += u.breakdown.skill;
    bk.burst += u.breakdown.burst;
  }
  const mean = acc.reduce((a, b) => a + b, 0) / N;
  console.log(
    `${slug.padEnd(20)} fb=${(fb / N).toFixed(1)} sim=${(mean / 1e6).toFixed(1)}M real=${(real / 1e6).toFixed(1)}M ratio=${(mean / real).toFixed(3)}  n=${(bk.normal / N / 1e6).toFixed(1)}M s=${(bk.skill / N / 1e6).toFixed(1)}M b=${(bk.burst / N / 1e6).toFixed(1)}M pulls=${pulls}`
  );
}
