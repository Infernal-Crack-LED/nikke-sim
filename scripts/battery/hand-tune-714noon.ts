...
const HT = JSON.parse(
  readFileSync(new URL('../../data/kit-status.json', import.meta.url), 'utf8')
);
const TUNED = new Set<string>(
  Object.entries<any>(HT.units)
    .filter(([, u]) => u.tuned)
    .map(([slug]) => slug)
);
...
const norm = (name: string) =>
  name.replace(/\s*\(treasure\)$/i, '').toLowerCase();
const isSupported = (slug: string) =>
  supportedNames.includes(norm(w.data.characters[slug].name));
...
const b3 = (name: string, target: string, boss: Element): Team => ({
  ...
  target,
  boss,
  slugs: ['little-mermaid', 'crown', target, 'helm'],
});
...
const ab = (
  ...
  {
    name: `${label} · A (with ${target}, focused)`,
    target,
    boss,
    carry,
    pair: label,
...
for (const p of pairs) {
  ...
  const a = carryTot[A.name],
    b = carryTot[B.name];
  console.log(
    `  ${p}: carry ${A.carry} ${(a / 1e6).toFixed(0)}M (with ${A.target}) vs ${(b / 1e6).toFixed(0)}M (Crown) → Δ ${((a - b) / 1e6).toFixed(0)}M (${(((a - b) / b) * 100).toFixed(1)}%). Grade real Δ vs this.`
  );
}
