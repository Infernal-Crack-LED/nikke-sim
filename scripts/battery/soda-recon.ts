...
for (const boss of [
  ...
  const rat = (s: string) => {
    ...
    return (m / REAL[s]).toFixed(2);
  };
  console.log(
    `boss=${String(boss).padEnd(9)} FB=${(fb / N).toFixed(1)}  LM=${rat('little-mermaid')} Crown=${rat('crown')} Helm=${rat('helm')} | Soda=${rat('soda-twinkling-bunny')}`
  );
}
