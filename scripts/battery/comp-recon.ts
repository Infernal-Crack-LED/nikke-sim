...
const CASES: Case[] = [
  {
    team: {
      name: 'rrh control (neutral)',
      slugs: ['little-mermaid', 'crown', 'helm', 'rapi-red-hood'],
    },
    boss: null,
    realFb: 13,
    real: {
      'little-mermaid': 470_516_754,
      crown: 213_881_837,
      helm: 477_724_348,
      'rapi-red-hood': 899_555_690,
    },
  },
  {
    team: {
      name: 'moran control (neutral)',
      slugs: ['helm', 'crown', 'moran', 'snow-white'],
    },
    boss: null,
    realFb: 11,
    real: {
      ...
      crown: 212_278_134,
      moran: 287_972_631,
      'snow-white': 399_610_717,
    },
  },
  {
    team: {
      name: 'nayuta control (boss Water)',
      slugs: ['little-mermaid', 'helm', 'nayuta', 'snow-white'],
    },
    boss: 'Water',
    realFb: 12,
    real: {
      'little-mermaid': 283_576_237,
      helm: 435_678_970,
      nayuta: 544_176_905,
      'snow-white': 348_982_871,
    },
  },
];
...
for (const c of CASES) {
  ...
  for (const slug of c.team.slugs) {
    ...
    console.log(
      `  ${slug.padEnd(16)} sim=${(mean / 1e6).toFixed(1).padStart(6)}M  real=${(rr / 1e6).toFixed(1).padStart(6)}M  ratio=${(mean / rr).toFixed(3)}`
    );
  }
}
