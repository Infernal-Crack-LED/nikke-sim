...
const elementOf = (w: World, slug: string) =>
  w.data.characters[slug].element as Element;
...
export function autoWire(w: World, team: BatteryTeam): string[] {
  ...
  if (has('eunhwa-tactical-upgrade') && !has('emma-tactical-upgrade')) {
    warnings.push(
      'eunhwa-tactical-upgrade has only a duo kit parse but emma-tactical-upgrade is absent'
    );
  }
  ...
  const eff = team.slugs.map((s) =>
    s === 'red-hood'
      ? `${['', 'I', 'II', 'III'][lambda['red-hood'] ?? 3]}`
      : burstOf(w, s)
  );
  const n = (b: string) => eff.filter((x) => x === b).length;
  if (n('I') < 1 || n('II') < 1 || n('III') < 2) {
    warnings.push(
      `rotation-outlier shape (B1 x${n('I')}, B2 x${n('II')}, B3 x${n('III')})`
    );
  }
  return warnings;
}
...
export function pickAdvantageBoss(
  w: World,
  team: BatteryTeam
): { boss: Element; via: string } {
  const probe = runOnce(w, team, null, 1, 1000);
  const isB3 = (u: { burst: string }) => u.burst === 'III' || u.burst === 'Λ';
  const pick =
    probe.units.find((u) => isB3(u) && u.burstCasts > 0) ??
...
const fbStr = (d: Record<number, number>, n: number) =>
  Object.entries(d)
    .sort((a, b) => +a[0] - +b[0])
    .map(([c, k]) => `${c}x${Math.round((100 * k) / n)}%`)
    .join(' ');
...
export function runBattery(
  ...
  const out: Record<
    string,
    { team: BatteryTeam; warnings: string[]; cells: Record<string, CellStats> }
  > = {};
  ...
  for (const team of teams) {
    ...
    const wired = [
      ...
      ...Object.entries(team.lambda ?? {}).map(
        ([s, l]) => `${s}: bursts as B${l}`
      ),
    ];
    ...
    for (const cell of cells) {
      ...
      console.log(
        `    ${cell.label.padEnd(22)} FBs ${fbStr(st.fbDist, nSeeds).padEnd(18)} team ${M(st.teamMean).padStart(6)}M`
      );
    }
    ...
    const derived = opts?.derived ?? [
      { label: `${ref}/${cells[0].label}`, num: cells.length - 1, den: 0 },
    ];
    const header = [
      'unit'.padEnd(26),
    ...
    for (const slug of team.slugs) {
      const cols = cells.map((c) =>
        M(stats[c.label].units[slug].mean).padStart(10)
      );
      const refU = stats[ref].units[slug];
      const sdPct = refU.mean
        ? ((100 * refU.sd) / refU.mean).toFixed(1)
        : '0.0';
      ...
      console.log(
        '    ' +
          [slug.padEnd(26), ...cols, sdPct.padStart(6), ...ratios].join(' ')
      );
    }
  }
  if (process.env.OUT) {
    writeFileSync(
      process.env.OUT,
      JSON.stringify({ nSeeds, batteries: out }, null, 1)
    );
...
export function runRealisticBattery(
  w: World,
  anchors: BatteryTeam[],
  boss: Element | null,
  title: string
...
export function fillRoster(
  ...
  const b1s = orderPool(
    pool.filter((s) => burstOf(w, s) === 'I'),
    B1_PRIORITY
  );
  const b2s = orderPool(
    pool.filter((s) => burstOf(w, s) === 'II'),
    B2_PRIORITY
  );
  const b3s = pool.filter(
    (s) => burstOf(w, s) === 'III' || burstOf(w, s) === 'Λ'
  );
  ...
  const allB1 = orderPool(
    w.roster.filter((s) => burstOf(w, s) === 'I'),
    B1_PRIORITY
  );
  const allB2 = orderPool(
    w.roster.filter((s) => burstOf(w, s) === 'II'),
    B2_PRIORITY
  );
  let reuse1 = 0,
    reuse2 = 0;
  const takeB1 = () =>
    b1s.length ? b1s.shift()! : allB1[reuse1++ % allB1.length];
  const takeB2 = () =>
    b2s.length ? b2s.shift()! : allB2[reuse2++ % allB2.length];
  ...
  const takePair = (arr: string[], s: string) =>
    arr.splice(arr.indexOf(s), 1)[0];
  ...
  while (b3Queue.length || b1s.length || b2s.length) {
    ...
    if (hasEmma && hasEunhwa && b1s.includes('emma-tactical-upgrade')) {
      ...
      const pair = STICKY_B2_PAIRS.find(
        ([x, y]) => b2s.includes(x) && b2s.includes(y)
      );
    ...
    if (!b3Queue.length && (b1s.length || b2s.length)) {
      ...
      teams.push({
        name: `fill ${n}`,
        slugs: rest.slice(0, 5),
        source: 'roster fill (support drain, padded with repeats)',
      });
      break;
    }
  }
