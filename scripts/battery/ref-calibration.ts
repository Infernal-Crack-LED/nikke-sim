...
const adv: Record<Element, Element> = {
  Wind: 'Iron',
  Fire: 'Wind',
  Water: 'Fire',
  Iron: 'Electric',
...
console.log(
  `Neutral fight — boss ${NEUTRAL_BOSS} (all four anchors neutral). Record from each perspective:\n`
);
const neutralByFocus: Record<string, Record<string, number>> = {};
for (const focus of FRAME) {
  const res = simFrame(NEUTRAL_BOSS, focus);
  neutralByFocus[focus] = Object.fromEntries(
    res.units.map((u) => [u.slug, u.totalDamage])
  );
  console.log(
...
console.log(
  '=== per-anchor: NEUTRAL total (grade real vs this) + advantage factor to apply ==='
);
for (const a of FRAME) {
  const neu = neutralByFocus[a][a];
  const advBoss = adv[w.data.characters[a].element as Element] as Element;
  const advTot = simFrame(advBoss, a).units.find(
    (u) => u.slug === a
  )!.totalDamage;
  console.log(
    `  ${a.padEnd(16)} neutral ${(neu / 1e6).toFixed(0).padStart(4)}M   ×${(advTot / neu).toFixed(4)} advantaged (${advBoss}) = ${(advTot / 1e6).toFixed(0)}M`
  );
}
...
console.log(
  '  full-burst distribution: ' +
    Object.entries(fbDist)
      .sort((a, b) => +a[0] - +b[0])
      .map(([c, k]) => `${c}×${Math.round((100 * k) / N)}%`)
      .join(' ')
);
...
for (const a of FRAME) {
  ...
  const sd = Math.sqrt(
    arr.reduce((x, y) => x + (y - mean) ** 2, 0) / arr.length
  );
  cv[a] = (sd / mean) * 100;
  console.log(
    `  ${a.padEnd(16)} mean ${(mean / 1e6).toFixed(0).padStart(4)}M  ± ${cv[a].toFixed(2)}%  (range ${(Math.min(...arr) / 1e6).toFixed(0)}-${(Math.max(...arr) / 1e6).toFixed(0)}M)`
  );
}
...
const targets =
  rfIdx >= 0 && process.argv[rfIdx + 1]
    ? [parseFloat(process.argv[rfIdx + 1])]
    : [2, 1.5, 1];
...
for (const t of targets) {
  ...
  console.log(
    `  ±${t.toFixed(1)}%     ` +
      ns.map((n) => String(n).padStart(10)).join('') +
      '   ' +
      String(Math.max(...ns)).padStart(5)
  );
}
console.log(
...
console.log(
  '    (±50%→3, ±35%→5, ±25%→9, ±20%→14, ±15%→23, ±13%→30 runs). Validating the sim MC'
);
console.log(
  '    to a factor-1.5 needs ~10-12 runs; factor-2, ~6-8 — then trust MC jitter elsewhere.'
);
