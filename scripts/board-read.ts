...
import {
  bandLabel,
  boardStats,
  collectBoardReadings,
  COMPS,
  tempLabel,
} from './lib/board-readings.js';
...
console.log(
  `RATIO = sim / real.  >1 = HOT ▲ (sim OVER-models, fix = REMOVE damage).  <1 = COLD ▼ (sim UNDER-models, fix = ADD damage).`
);
console.log(
  `(NB: solo probe-data recons use the OPPOSITE field 'realOverSim' — >1 = COLD there. Do not conflate.)`
);
console.log(
  `ranked most-stable → least (stability = mean |ratio−1|, lower is better)`
);
console.log(
  `seedSD = mean per-comp Monte-Carlo spread (sd/mean across seeds) — the single-run band a real fight sits in; ⚠ = ≥2% (FB-bimodal / boss-timing-sensitive comp, judge against multi-run avg)\n`
);
console.log(
  '  #  unit                        N   mean   temp    range          MAD    band    seedSD   ratios'
);
console.log(
  '  ─  ──────────────────────────  ──  ─────  ──────  ─────────────  ─────  ──────  ───────  ──────────────────────────'
);
...
console.log(
  `  within ±3% MAD: ${in3}  |  ±5%: ${in5}  |  ±8%: ${in8}  |  worse: ${rows.length - in8}`
);
