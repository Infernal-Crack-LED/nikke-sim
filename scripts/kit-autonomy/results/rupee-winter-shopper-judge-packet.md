# CROSS-FAMILY RECONCILING-JUDGE PACKET — unit rupee-winter-shopper (slug `rupee-winter-shopper`)
...
# SECTION: GROUND TRUTH — the unit's kit prose + base stats (data/characters.json, SL10 values)

```json
{
  "slug": "rupee-winter-shopper",
  ...
  "burstMeta": {
    ...
    "burst_apply_delay": 1,
    "change_burst_step": "Step1"
  },
  "skills": {
    "skill1": "\u25a0 Activates when the last bullet hits the target. Affects all allies. \nDEF \u25b2 19.02% for 5 sec.",
    "skill2": "\u25a0 Activates when an ally uses a Burst Skill. Affects all allies.\nShopping: DEF \u25b2 1.33%. Stacks up to 4 times and lasts for 20 sec.\n\u25a0 Activates if Shopping is at max stacks when Full Burst ends. Affects all allies.\nBurst Gauge filling speed \u25b2 7.9% for 5 sec.",
    "burst": "\u25a0 Affects self.\nAttract: Taunts all enemies for 5 sec.\nRecovers 50.47% of attack damage as HP over 10 sec.\n\u25a0 Affects all allies.\nReload Speed \u25b2 63.17% for 10 sec.\nRe-enters Burst Stage 1."
  },
  "baseStats": {
    "hp": 16500,
    "atk": 400,
...
# SECTION: S2b TEST-FAITHFULNESS REVIEW (claude-fable-5, pre-implementation)
...
```json
{
  ...
  "spec": [
    {
      "slot": "skill1",
      "kitLine": "DEF ▲ 19.02% for 5 sec",
      "disposition": "FAITHFUL",
      "scope": "Generic DEF stat buff, no attack-type scoping. defPct is offensively inert in v1 (self DEF never feeds own damage), but the buff must still be applied for kit completeness and future consumers.",
      "durationSemantics": "durationSec: 5 — wall-clock seconds, refreshed on each re-trigger. NOT rounds.",
      "triggerIdentity": "lastBullet — 'Activates when the last bullet hits the target' is the per-magazine trigger. With ammo 60 and reloadFrames 81 this fires once per magazine cycle, so uptime is periodic, not continuous.",
      "targetSet": "allies (all 5 units including self — 'Affects all allies').",
      "nearestWrongModel": "Dropped entirely as 'defensive, no damage, skip' (the reload-speed/Grave-class misread generalized to DEF), or scoped to self only.",
      "distinguishingAssertion": "Event log contains buffApply {stat:'defPct', value:19.02} events at rupee-winter-shopper's last-bullet/reload boundaries, with target coverage spanning all 5 slots, and expiresFrame - applyFrame == 300 (5s @60fps). Red if zero defPct 19.02 applies exist or targets are self-only.",
      "inertness": "Total damage of every unit must be IDENTICAL with this block removed via withPatchedOverride — defPct is a no-damage stat in v1. If removing it moves any total, the encoding leaked into a damage stat.",
      "evidenceTier": "DATAMINED",
      "loadBearing": true
    },
    {
      "slot": "skill2",
      "kitLine": "Shopping: DEF ▲ 1.33%. Stacks up to 4",
      "disposition": "FAITHFUL",
      "scope": "Generic DEF stat, damage-inert itself — but the STACK STATE is the gate for the skill2 Burst-Gauge line, so the stack bookkeeping is the real payload, not the DEF value.",
      "durationSemantics": "durationSec: 20 per application, maxStacks: 4. Seconds, not rounds. A rotation's burst-cast cluster (B1→B2→B3 within ~2-3s) lands 3-4 stacks well inside one 20s window.",
      "triggerIdentity": "'When an ally uses a Burst Skill' = ANY burst cast by ANY team member at ANY stage — engine encoding is stageEnter for stages 1, 2, and 3 (or an equivalent cast-count resource). It is NOT burstCast (own-burst-only) and NOT fullBurstEnter (once per rotation).",
      "targetSet": "allies (all 5).",
      "nearestWrongModel": "Keyed to fullBurstEnter — 1 stack per rotation instead of 3-4 per rotation chain. Stacks would then crawl toward 4 across ~4 rotations (and only if 20s windows overlap) instead of hitting max inside the FIRST rotation, so the max-stack gate on the gauge line essentially never opens in time.",
      "distinguishingAssertion": "Within a single rotation's burst chain (a ~200-frame span containing the B1/B2/B3 burstCast events), the log shows 3+ buffApply {stat:'defPct', value:1.33, maxStacks:4} events with the stacks field incrementing 1→2→3(→4). Red under nearest-wrong: exactly one apply per rotation.",
      "inertness": "The DEF value itself must move zero damage (same withPatchedOverride removal check as skill1). The stack count 4 must NOT be reachable in a rotation with only 3 burst casts unless the re-enter chain (burst line) produces a 4th cast.",
      "evidenceTier": "DATAMINED",
      "loadBearing": true
    },
    {
      "slot": "skill2",
      "kitLine": "Burst Gauge filling speed ▲ 7.9%",
      "disposition": "FAITHFUL",
      "scope": "Burst-gauge generation rate — stat burstGenPct. This is the kit's one directly rotation-moving line: it compresses time-to-next-FB, so it can change FB counts/timing.",
      "durationSemantics": "durationSec: 5.",
      "triggerIdentity": "fullBurstEnd, GATED on Shopping being at max stacks (4) at that moment — a stack/resource-threshold gate (resourceGate min:4 over a burst-cast-counting pool, or equivalent). Not fullBurstEnter, not ungated.",
      "targetSet": "allies (all 5).",
      "nearestWrongModel": "Two plausible misreads: (a) UNGATED — fires on every fullBurstEnd regardless of stack count, over-crediting gauge every rotation even in comps that can never reach 4 stacks; (b) stat misread as an instant fillGauge pct instead of a 5s filling-SPEED rate.",
      "distinguishingAssertion": "In a comp where a second Burst-I unit exists and the re-enter chain yields 4 casts (4 stacks), a buffApply {stat:'burstGenPct', value:7.9} lands within a frame of fullBurstEnd. In a comp/patch where only 3 stacks accrue by FB end (e.g. re-enter line removed via withPatchedOverride), NO burstGenPct 7.9 apply appears. Green-faithful/red-ungated on the second half.",
      "inertness": "Must NOT fire when stacks < 4 at FB end, and must NOT fire at fullBurstEnter or burst-cast time. No instant gauge jump (no fillGauge-style discontinuity) — only a 5s rate window.",
      "evidenceTier": "DATAMINED",
      "loadBearing": true
    },
    {
      "slot": "burst",
      "kitLine": "Attract: Taunts all enemies for 5 sec",
      "disposition": "UNMODELED",
      "scope": "Enemy-targeting/aggro control — the sim has no aggro model and the partless scope-lock boss deals no modeled damage.",
      "durationSemantics": "5 sec, moot.",
      "triggerIdentity": "burstCast (own burst), moot.",
      "targetSet": "enemy, moot.",
      "nearestWrongModel": "Inventing a damageTakenPct or targetStatus effect out of 'Attract' — the text grants no damage or debuff, only taunt.",
      "distinguishingAssertion": "No damage event, no buffApply, and no targetStatus window is attributable to this line; recorded verbatim in unmodeled.burst.",
      "inertness": "Zero events, zero damage. Anything it emits is an invention.",
      "evidenceTier": "DATAMINED",
      "loadBearing": false
    },
    {
      "slot": "burst",
      "kitLine": "Recovers 50.47% of attack damage as HP",
      "disposition": "FAITHFUL",
      "scope": "Self lifesteal-over-time. No HP pool is modeled, but the engine's heal effect exists precisely to emit recovery events for on-recovery consumers — model as heal to SELF with ticks:10, intervalSec:1 (10s of recovery events) on her burstCast.",
      "durationSemantics": "over 10 sec — a heal-over-time, so ~10 periodic recovery events, not one instant tick.",
      "triggerIdentity": "burstCast (her own burst block).",
      "targetSet": "SELF ONLY — '■ Affects self' heads this sub-block. This is the adversarial crux: the recovery trigger fires on units that RECEIVE the heal.",
      "nearestWrongModel": "Target set widened to allies — which would pump recovery events into any on-recovery consumer teammate (crown's 'when recovery takes effect' engine sits in the standard control fixture) and spuriously move team damage every rotation rupee bursts.",
      "distinguishingAssertion": "Every recovery/heal event sourced from rupee-winter-shopper's burst targets rupee's own slot; specifically ZERO recovery events from her slot target crown. Green-faithful (crown's recovery consumers untouched by rupee), red-nearest-wrong (crown's buff refreshes track rupee's burst timing).",
      "inertness": "Team totals must be unchanged with this heal block removed — rupee has no own on-recovery consumer, and a self-heal must not feed any teammate's recovery trigger.",
      "evidenceTier": "DATAMINED",
      "loadBearing": true
    },
    {
      "slot": "burst",
      "kitLine": "Reload Speed ▲ 63.17% for 10 sec",
      "disposition": "FAITHFUL",
      "scope": "Weapon-state modifier — this IS damage (taxonomy #6): reload speed gates shots fired for the whole team. stat reloadSpeedPct 63.17.",
      "durationSemantics": "durationSec: 10.",
      "triggerIdentity": "burstCast — it sits in her OWN burst block, so it fires ONLY on rotations rupee actually casts. With two Burst-I units in a comp, the B1 selection can go to the other unit; on those rotations this buff must NOT appear.",
      "targetSet": "allies (all 5, '■ Affects all allies').",
      "nearestWrongModel": "Two candidates: (a) skipped as 'defensive/utility, no damage' — the documented reload-mechanic drop that over/under-models shot economy; (b) keyed to fullBurstEnter so it fires every rotation even when the other B1 won the cast.",
      "distinguishingAssertion": "buffApply {stat:'reloadSpeedPct', value:63.17} events exist, land on all 5 slots, and every one coincides (same frame) with a burstCast event whose caster is rupee-winter-shopper — never with a fullBurstStart she didn't burst into. Additionally reload events inside the 10s window show a shortened reload gap vs the 81-frame base (≈50 frames at +63.17%), i.e. carry shot count strictly increases vs a withPatchedOverride copy with this block deleted.",
      "inertness": "No reloadSpeedPct applies on rotations where rupee did not cast her burst; no effect outside the 10s window.",
      "evidenceTier": "DATAMINED",
      "loadBearing": true
    },
    {
      "slot": "burst",
      "kitLine": "Re-enters Burst Stage 1",
      "disposition": "FAITHFUL",
      "scope": "Rotation-machine effect: reenterStage stage:1 — after her cast the rotation stays at stage 1 so ANOTHER eligible Burst-I unit can also cast. This is also the mechanical enabler of the kit's own 4-stack Shopping gate: B1(rupee) → re-entered B1(other) → B2 → B3 = exactly 4 burst casts in one chain.",
      "durationSemantics": "Instantaneous rotation-state effect; no duration.",
      "triggerIdentity": "burstCast (rides her own burst).",
      "targetSet": "Rotation state, not a unit buff.",
      "nearestWrongModel": "Omitted entirely (rotation shows one stage-1 cast per chain, Shopping caps at 3, the skill2 gauge line goes permanently dead), or mis-modeled as burstCdr / a self re-cast.",
      "distinguishingAssertion": "In a comp containing rupee plus one other Burst-I unit, a single rotation chain contains TWO stage-1 burstCast events from two DIFFERENT casters before the fullBurstStart, and the Shopping buffApply stacks field reaches 4 in that chain. Red under omission: one stage-1 cast, stacks max out at 3, and no burstGenPct 7.9 ever fires.",
      "inertness": "Must not grant rupee herself a second cast, and must not alter chain timing beyond permitting the additional stage-1 cast; in a comp with NO other B1 it changes nothing.",
      "evidenceTier": "DATAMINED",
      "loadBearing": true
    }
  ],
  "loadBearingSet": [
    "skill1:DEF ▲ 19.02% for 5 sec (lastBullet, all allies)",
    "skill2:Shopping DEF ▲ 1.33% ×4 stacks, 20 sec (any burst cast)",
    "skill2:Burst Gauge filling speed ▲ 7.9% for 5 sec (fullBurstEnd, max-stack-gated)",
    "burst:Recovers 50.47% of attack damage as HP over 10 sec (SELF heal events)",
    "burst:Reload Speed ▲ 63.17% for 10 sec (burstCast, all allies)",
    "burst:Re-enters Burst Stage 1"
  ],
  "unmodeledVerbatim": {
    "skill1": [],
    "skill2": [],
  ...
  "notes": "Expected shared-prior misreads to reconcile, in priority order. (1) The kit is a coherent machine: 'Re-enters Burst Stage 1' exists to produce the 4th burst cast that maxes Shopping (B1 + re-entered B1 + B2 + B3 = 4), which in turn opens the max-stack gate on the fullBurstEnd gauge buff. Tests that exercise skill2's gauge line in a comp where only 3 casts happen per chain — or that never verify the re-enter actually yields a second stage-1 cast — are asserting a state the faithful model cannot reach, and an UNGATED encoding would pass them; demand the paired positive (4 stacks → buff fires) AND negative (3 stacks → silent) assertions. (2) COMP HAZARD: rupee-winter-shopper is Burst I, and the standard control fixture already fixes liter into the B1 slot — the entire burst block (reload speed, re-enter, heal) is conditional on rupee WINNING the stage-1 selection in a two-B1 comp. Any test must first assert a burstCast event with rupee as caster exists at all; otherwise every burst assertion is vacuously green/red for selection reasons, not modeling reasons. (3) skill2a's trigger is any-ally-any-stage burst cast (stageEnter 1/2/3 or a cast-counting resource), not fullBurstEnter — the under-stacking misread. (4) Reload Speed ▲ is DAMAGE (shot-count gate), not skippable utility, and it is burstCast-keyed, not fullBurstEnter — assert both the buffApply-coincides-with-her-cast property and a real shot-count delta. (5) The self-heal's target set is the trap, not its magnitude: 'Affects self' means recovery events must reach only rupee; an allies-targeted heal silently feeds any on-recovery consumer in the fixture (crown) and moves damage. (6) All magnitudes (19.02 / 1.33×4 / 7.9 / 50.47 / 63.17) are kit-literal → DATAMINED; no ⚑ estimates are needed anywhere, so any calibrated-looking value in the override is a red flag. No leak detected in the redacted methodology. Tooling note: I have no file access in this run — the JSON above is returned inline and was not written to scripts/kit-autonomy/reviews/rupee-winter-shopper.test-review.json.",
  "model": "claude-fable-5"
}

```

# SECTION: S5 BLIND TEST (claude-opus-5) — scripts/kit-autonomy/blind/rupee-winter-shopper.test.ts
# Result vs the DRIVER override (run with .vitest.blind.config.mts): 12 GREEN / 2 documented
# it.skip (taunt — no aggro primitive; self heal — event-unobservable) / 1 RED.
# The RED is the damage-direction assertion in "re-enters Burst Stage 1 so the other Burst I ally
# still casts": the blind test expected teamTotal(with reenter) > teamTotal(without) on its
# burstFirst-patched controlComp fixture, asserting the premise "without the re-entry the fixed
# B1 buffer never casts at all". DRIVER PROBE of that exact fixture shape:
#   WITH reenter:    {"team": 748281936.4, "literCasts": 6, "rwsCasts": 9, "fbs": 5}
#   WITHOUT reenter: {"team": 748697652.9, "literCasts": 6, "rwsCasts": 9, "fbs": 5}
# i.e. liter CASTS 6 TIMES EITHER WAY (the ada/helm-throttled ~36s chains leave rotations where
# rws's 20s CD is down and liter takes stage 1 outright — burstFirst only forces rws ahead when
# she casts), so the reentry's mechanical payload (a second B1 cast IN the same chain) is not
# exercised by that fixture's totals; the measured direction is -0.056% timing noise. The
# MECHANICAL reentry effect is pinned green in BOTH specs: the driver spec RW5 (liter fills stage 1
# in chain 1 exactly one 30f stage-gap after rws, before crown; removed -> no chain-1 liter cast;
# sole-B1 fixture byte-identical no-op) and the blind test's own removal-count assertion
# (reenterRemoved > 0 passed).
...
# SECTION: S6 BLIND OVERRIDE (claude-opus-5) — scripts/kit-autonomy/blind/rupee-winter-shopper.override.json
SEMANTIC DIFF (driver vs S6 blind override) — everything else converges:
  1. skill2 Shopping TRIGGER: driver = THREE stageEnter blocks (stage 1/2/3 -> any ally's burst
     cast, faithful to "when an ALLY uses a Burst Skill") + per-cast pool mirror;
     blind = ONE burstCast block (owner-keyed; blind author flagged this blind: the redacted
     schema did not surface the stageEnter any-ally reading, fires 1x/chain instead of 3-4x).
  2. skill2 gate: driver = fullBurstEnd + resourceGate{shopping >= 4} (the power/rupee-base
     resource-mirror construction for the "Shopping at max stacks" precondition);
     blind = UNGATED fullBurstEnd, precondition carried verbatim in unmodeled.skill2
     (blind author flagged: "no maxStacks-reached block gate exists in the schema" — they
     lacked the resource-pool mirror precedent).
  3. reenterStage target: driver = allies block (awb convention); blind = folded into the self
     heal block (target self). Rotation-state effect — behaviorally identical.
  IDENTICAL: skill1 lastBullet->allies defPct 19.02/5s; heal self ticks:10 intervalSec:1;
  reenterStage stage:1; reloadSpeedPct 63.17/10s allies on burstCast; taunt UNMODELED verbatim.
...
```json
{
  ...
  "unmodeled": {
    "skill1": [],
    "skill2": [
      "Activates if Shopping is at max stacks when Full Burst ends. (max-stack precondition on the Burst Gauge filling speed rider is not gated \u2014 no maxStacks-reached block gate exists in the schema)"
    ],
    "burst": [
      "Attract: Taunts all enemies for 5 sec."
    ]
  ...
  "caveats": [
    "\u2691 skill2 Shopping stack trigger: kit says 'when an ally uses a Burst Skill' \u2014 modeled as burstCast on ANY ally (the engine's burstCast trigger is owner-cast-keyed). If the engine keys burstCast to the OWNER only, this UNDER-fires by ~3-4x per rotation; a stageEnter-per-stage triple-block would over/under-fire differently. Unmeasured.",
    "\u2691 skill2 max-stacks precondition on the Burst Gauge rider is NOT gated: with 3 burst casts per rotation the 4-stack cap (20s duration) is reached during rotation 2 and holds thereafter, so the rider is modeled as always-on from the first fullBurstEnd. This OVER-credits the first ~1-2 rotations' gauge.",
    "\u2691 Burst Gauge filling speed \u25b27.9% mapped to burstGenPct (the unit's own gauge contribution scaler) applied to all allies. Whether the engine's burstGenPct is per-unit-contribution or a global fill-rate scaler determines whether this is faithful.",
    "defPct is documented inert in v1 (self DEF does not affect own damage). Both DEF lines are kept for kit completeness / a future consumer, per the HARD RULE that DEF/HP lines are not skippable \u2014 they may drive a teammate's DEF-scaling conversion.",
    "The burst heal is modeled as 10 recovery events over 10 sec (ticks:10) so on-recovery consumers (e.g. a Crown-style 'when recovery takes effect' partner) stay refreshed; no HP amount is modeled. The 50.47%-of-attack-damage magnitude is not representable.",
    "Taunt is skipped: the v1 boss deals no damage and there is no aggro model, so a taunt has no damage-relevant channel."
  ],
  "note": "PARSER BASELINE (HYPOTHESIS \u2014 NOT a validated model). Every \u2691 below is an UNMEASURED estimate; hand-tune + record against a real fight before trusting any number. Rupee: Winter Shopper is an AR/Electric/Defender Burst I. Her kit carries NO damage effects at all \u2014 it is entirely DEF, heal/taunt, reload speed, burst-gauge, and a Burst-Stage-1 re-entry. The two damage-relevant channels are (a) burst Reload Speed \u25b263.17%/10s to all allies, which is a weapon-state modifier and therefore IS damage (it gates shots fired \u2014 HARD RULE 6), and (b) the reenterStage:1 rotation effect, which lets a second Burst I unit also cast at stage 1 and can materially change the rotation. The Shopping stack and Burst Gauge filling speed lines feed rotation via gauge rate. Both DEF lines (S1 last-bullet 19.02%/5s, S2 Shopping 1.33%\u00d74/20s) are modeled as defPct even though defPct is inert in v1 \u2014 DEF lines are never auto-skipped (tandem/cross-unit rule), and dropping them would erase the kit from the file. All three slots authored; skipped lines are listed verbatim in `unmodeled`.",
  "hasPierce": false
}
```

# SECTION: DRIVER IMPLEMENTATION — scripts/tests/units/rupee-winter-shopper.test.ts (7/7 GREEN)
...
# SECTION: DRIVER IMPLEMENTATION — src/skills/overrides/rupee-winter-shopper.json

```json
{
  "note": "Rupee: Winter Shopper (slug rupee-winter-shopper, aka rws) \u2014 AR / Defender / Electric / Burst I (cd 20s), Tetra; ammo 60, reloadFrames 81, normalMult 13.65, rate_of_fire 720, hitsPerShot 1, critRate 15 / critDamage 150. FROM-SCRATCH kit-autonomy build 2026-08-05 (no prior override / kit-status row; simSupported was false). EXACT SLUG: `rupee-winter-shopper` \u2014 the Electric B1 Defender, NOT `rupee` (AR/Iron B2 Attacker \u2014 an entirely different kit; the base variant landed 2026-08-04). A shopping-spree tank/enabler: her OWN damage is only her AR spray (every kit line is a buff, a heal event, or the rotation machine \u2014 she has no skill/burst damage instances), and her team value is (a) a last-bullet team DEF window, (b) Shopping DEF stacks that GATE a post-Full-Burst burst-gauge-fill-speed window, and (c) a burst that taunts (unmodeled), self-heals (event-only), speeds every ally's reload, and RE-ENTERS Burst Stage 1 so a second B1 also casts. || FULL-KIT AUDIT (every line accounted for): S1 'Winter Premium' \u2014 'Activates when the last bullet hits the target. Affects all allies.' (L1) 'DEF \u25b2 19.02% for 5 sec' = IMPLEMENTED lastBullet \u2192 allies \u2192 defPct 19.02 / durationSec 5. lastBullet fires when her magazine dries out (sim.ts fire-dry path); 'hits the target' == the 60th shot of each magazine (v1 has no miss model, clay precedent). defPct has NO consumer in sim.ts (v1: self DEF moves no damage \u2014 the Endurance-cube stat comment), so the line is FAITHFUL-AND-INERT: modeled for kit-SSOT completeness + future consumers, pinned at the event level by the spec. 'Affects all allies' INCLUDES self (no except-self clause; the mica-snow-buddy all-allies convention). S2 'VIP Gift' \u2014 (L2a) 'Activates when an ally uses a Burst Skill. Affects all allies. Shopping: DEF \u25b2 1.33%. Stacks up to 4 times and lasts for 20 sec' = IMPLEMENTED as THREE stageEnter blocks (stage 1/2/3 \u2014 a burst cast by ANY unit at ANY stage; NOT burstCast which is owner-only, NOT fullBurstEnter which is once per rotation) \u2192 allies \u2192 defPct 1.33, maxStacks 4, durationSec 20. The three blocks share one buff key (caster+slot+stat+value), so they MERGE into a single Shopping instance whose stacks accrue across the chain's casts \u2014 exactly the per-cast stack mechanic. The same trigger also feeds the GATE MIRROR: +1 to the owner's `shopping` resource pool per ally burst cast (the power/rupee-base resource-mirror construction \u2014 the engine has no buff-stack gate primitive, so L2b's 'Shopping at max stacks' condition reads the pool via resourceGate {min:4}). Pool-vs-buff divergence disclosed in caveat 2. (L2b) 'Activates if Shopping is at max stacks when Full Burst ends. Affects all allies. Burst Gauge filling speed \u25b2 7.9% for 5 sec' = IMPLEMENTED fullBurstEnd + resourceGate {shopping \u2265 4} \u2192 allies \u2192 burstGenPct 7.9 / durationSec 5 \u2014 NOT fullBurstEnter (the kit says 'when Full Burst ENDS' \u2014 the post-FB refill window, the awb-S2 precedent for the identical sentence), NOT an instant fillGauge (it is a 5s filling-SPEED rate). burstGenPct scales each holder's gauge contribution (sim.ts energy term) \u2014 the kit's 'Burst Gauge filling speed' mechanic. Gate is LOAD-BEARING: an ungated block over-fires on every FB end before Shopping completes (the spec's first-FB-end silence pin). BURST 'Shopaholic Date' \u2014 (B1) 'Affects self. Attract: Taunts all enemies for 5 sec' = UNMODELED (verbatim below): the engine has no taunt primitive and the v1 partless boss deals no damage and has no target choice, so enemy target-lock moves no damage (defensive; the label taunt-immunity precedent). (B2) 'Recovers 50.47% of attack damage as HP over 10 sec' = IMPLEMENTED burstCast \u2192 self \u2192 heal ticks:10 intervalSec:1 \u2014 a heal-over-time ('over 10 sec'): 10 recovery events over 10s (engine design: no HP amount modeled; the avistar self-HoT encoding). SELF-targeted ('\u25a0 Affects self' heads the sub-block), so the recovery events reach only HER \u2014 she carries no 'recovery' trigger and no teammate consumer is fed (a self heal, NOT a team heal \u2014 spec RW7 pins the target set through crown's recovery-consumer silence). Offensively inert; modeled for kit-SSOT completeness + correct recovery-event emission. The 'of attack damage' basis (lifesteal-vs-ATK reading) is moot: no HP pool exists to receive an amount. (B3) 'Affects all allies. Reload Speed \u25b2 63.17% for 10 sec' = IMPLEMENTED burstCast \u2192 allies \u2192 reloadSpeedPct 63.17 / durationSec 10. DAMAGE-RELEVANT (taxonomy: weapon-state modifier): reload speed shortens the reload gap \u2192 more shots for the whole team during the window; keyed to burstCast (her OWN casts \u2014 she is the sole caster of her burst; in a double-B1 comp a chain won by the other B1 grants nothing, the clay own-cast convention). (B4) 'Re-enters Burst Stage 1' = IMPLEMENTED burstCast \u2192 allies \u2192 reenterStage stage:1 \u2014 the rotation HOLDS stage 1 after her cast so a second eligible B1 also casts this chain (the Tia/awb/avistar/chime stage-hold primitive; behaviorally live only when a 2nd B1 is present, else the stage advances \u2014 the real-kit no-op case). Datamined burstMeta confirms it (use_burst_skill/change_burst_step = Step1). The re-entry is also the kit's OWN 4th cast: B1(rws) \u2192 re-entered B1 \u2192 B2 \u2192 B3 = 4 casts/chain = Shopping caps in one chain = the L2b gate opens \u2014 the machine closes. || EVIDENCE TIER: all live values are kit-text-literal (19.02/5s, 1.33\u00d74/20s, 7.9/5s, 50.47/10s, 63.17/10s); the only estimated quantities are the cadence tuple and the caveat-2 divergence bound. TIER 2 (stack mechanics + status-gate via resource mirror + reenterStage meta-defining rotation-machine + burstCast-vs-fullBurstEnd timing). Faithful>fit; measured>fudge. Kit-autonomy gauntlet 2026-08-05.",
  "caveats": [
  ...
  "unmodeled": {
    ...
    "skill2": [],
    "burst": [
      "Attract: Taunts all enemies for 5 sec. (UNMODELED \u2014 no engine taunt primitive; defensive: the v1 partless boss deals no damage and has no target choice, so enemy target-lock moves no damage. The label taunt-immunity precedent.)"
    ]
  },
  "skill1": [
    {
