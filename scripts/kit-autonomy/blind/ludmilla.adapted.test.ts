...
type LooseEvent = { kind: string; stat?: string; value?: number; targetSlug?: string };
...
function setBlocks(ov: LooseOv, slot: Slot, blocks: LooseBlock[]): void {
  const s = ov[slot];
  if (s && !Array.isArray(s) && Array.isArray((s as { blocks?: unknown }).blocks)) {
...
function run(ov?: unknown): { events: SimEvent[]; tot: Record<string, number> } {
...
function zeroDamage(b: LooseBlock): LooseBlock {
  const effects = Array.isArray(b.effects) ? (b.effects as Record<string, unknown>[]) : [];
  return { ...b, effects: effects.map((e) => ('atkPct' in e ? { ...e, atkPct: 0 } : e)) };
}

function buffApplies(events: SimEvent[], stat: string, value: number): LooseEvent[] {
...
const S1_FUDGE: LooseBlock = {
  ...
  target: { kind: 'enemy' },
  effects: [{ kind: 'buff', stat: 'damageTakenPct', value: 8.4, durationSec: 10 }],
};
...
const S2_FUDGE: LooseBlock = {
  ...
  target: { kind: 'enemy' },
  effects: [{ kind: 'buff', stat: 'damageTakenPct', value: 57.86, durationSec: 15 }],
};
...
const BURST_REF_CORE = run(patch((ov) => setBlocks(ov, 'burst', [NUKE({ core: true })])));
const BURST_REF_NOFB = run(patch((ov) => setBlocks(ov, 'burst', [NUKE({ noFb: true })])));
...
describe('ludmilla skill2 - Full-Burst-enter taunt + self damage reduction', () => {
  ...
  it('does not invert the SELF damage reduction into a boss damage-taken debuff', () => {
    ...
    expect(buffApplies(SHIPPED.events, 'damageTakenPct', 57.86)).toHaveLength(0);
  });
...
describe('ludmilla burst - 163.1% of final ATK', () => {
  ...
  it('does not core-strike (the kit text never says core strike damage)', () => {
    ...
    expect(rel(BURST_REF_CORE.tot[SLUG], BURST_REF.tot[SLUG])).toBeGreaterThan(1e-6);
    expect(rel(SHIPPED.tot[SLUG], BURST_REF_CORE.tot[SLUG])).toBeGreaterThan(1e-6);
  });
