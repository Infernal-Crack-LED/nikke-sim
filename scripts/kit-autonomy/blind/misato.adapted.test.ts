// ADAPTED S5 blind spec for `misato` — the S5 test (blind/misato.test.ts, claude-opus-5) with
// its ONE wiring RECON_ERROR accommodated; every ASSERTION INTENT is preserved verbatim.
//   RE1 `misatoHits` filtered damage events by `d.srcSlot === misatoIdx` (a numeric slot
//       index) — damage events carry identity in `slug` and category in `bucket`; `srcSlot`
//       is a kit-line STRING ('normal'|'skill1'|…), so the count came back 0 and the
//       cadence test failed vacuously. Fixed to `slug === 'misato' && bucket === 'normal'`
//       (hitsPerShot 1 → one normal damage event per landed round = the 60-hit counter input).
// Everything else passed UNADAPTED against the shipped driver override, including the two
// fixture-risk probes the blind author built in: misato DOES cast her burst inside
// controlComp('misato') despite liter also being Burst I (non-vacuity probe green), and the
// S1-b heal IS board-inert at scope lock because the leftmost-ally stand-in resolves to liter
// (no recovery trigger) — exactly the engine's documented alliesLowestHp convention.
//
...
function findIn(ov: unknown, slot: Slot, pred: (b: BlockLike) => boolean): BlockLike {
  return must(slotBlocks(ov, slot).find(pred), `no ${slot} block matching the spec predicate`);
}
...
function findAnySlot(
  ...
  for (const slot of ['skill1', 'skill2', 'burst'] as Slot[]) {
    ...
    if (block) return { block, slot };
  }
  throw new Error('misato spec: no block anywhere matching the spec predicate');
}

function dropIn(ov: unknown, slot: Slot, pred: (b: BlockLike) => boolean): void {
  const blocks = slotBlocks(ov, slot);
  const i = blocks.findIndex(pred);
  if (i < 0) throw new Error(`misato spec: nothing to drop in ${slot}`);
  blocks.splice(i, 1);
}
...
const buffApplies = (evs: SimEvent[]) =>
  evs.filter((e) => (e as { kind: string }).kind === 'buffApply') as unknown as BuffApplyEv[];
const damages = (evs: SimEvent[]) =>
  evs.filter((e) => (e as { kind: string }).kind === 'damage') as unknown as DamageEv[];
const burstCasts = (evs: SimEvent[]) =>
  evs.filter((e) => (e as { kind: string }).kind === 'burstCast');

function comp(patched?: unknown): Opts {
  const base = controlComp('misato', true) as unknown as Record<string, unknown>;
  if (!patched) return base as unknown as Opts;
  const overrides = {
...
function run(opts: Opts) {
  ...
  const res = runComp({ ...o, onEvent, cfg: { ...cfg, onEvent } } as unknown as Opts);
  return { res, evs };
}
...
const misatoIdx = must(smApplies[0], 'no Shooting Manual application observed at all').casterIdx;
const misatoHits = damages(control.evs).filter(
  (d) => d.slug === "misato" && d.bucket === "normal",
).length;
