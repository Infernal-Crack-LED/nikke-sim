...
// blind/quiry.test.ts. ONE mechanical correction — the onEvent sink was only wired when the
// comp exposed a cfg key (controlComp does not), so every event read ran on an empty log.
// Assertion INTENT unchanged; see manual-review + the S7 judge packet for the reconciliation.
/**
...
const runQuiry = (overrides?: Record<string, unknown>) => {
  ...
  // ADAPTED (driver, mechanical, 2026-08-04): controlComp exposes NO cfg key, so the pristine
  // conditional never wired the sink and the event log stayed empty; wire it unconditionally.
  opts.cfg = { ...((src.cfg as object | undefined) ?? {}), onEvent: sink };
