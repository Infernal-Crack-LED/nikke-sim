...
// blind/mary.test.ts. Two structural corrections to blind-writer assumptions that were
...
//      at scripts/tests/lib/harness.ts).
//   2. the whole-unit-inertness test compared `srcSlot` (a string union 'normal'|'skill1'|…)
//      against the caster's numeric slot index; corrected to filter damage events by
//      `slug === 'mary'` — same intent (mary must deal ONLY normal-bucket damage).
/**
...
describe('mary — blind kit spec', () => {
  ...
  describe('skill2 — "entering Full Burst" -> Incoming healing 23.78% / 15 sec', () => {
    it('is not smuggled in as some other stat', () => {
      ...
      // (Adapted: loops the HEAL_MAGNITUDES constant the blind author declared for exactly
      // this check — none of the three heal percentages may appear as a mary-granted stat.)
      for (const e of maryBuffs) {
        for (const m of HEAL_MAGNITUDES) {
          expect(Math.abs(Number(e.value) - m)).toBeGreaterThan(1e-9);
        }
      }
    });
  ...
  describe('whole-unit inertness', () => {
    it('mary\u2019s kit adds no skill or burst damage', () => {
      ...
      // ever carry her slot. (Adapted: damage events carry `slug`, not a caster slot index.)
