 ...
 *
 * DRIVER ADAPTATION NOTE (ether.adapted.test.ts): ONE change vs the raw blind file — the
 * casterOf() helper now reads unitIdx first (damage/burstCast events carry the owner there;
 * the raw helper's srcSlot/casterIdx reads never matched a damage event, which would have
 * masked every rider assertion regardless of the model) — plus the run() event-bus
 * wiring (controlComp exposes no cfg key; onEvent must be threaded via opts.cfg). No
 * assertion was touched by either change. Trigger reading CONVERGED: the
 * shipped override carries the interval:13 auto-cast this test asserts (the driver's initial
 * fullBurstEnter derivation was overturned — see the override note).
 *
...
function run(overrides?: Record<string, any>) {
  ...
  // ADAPTATION 2 (driver, 2026-08-05): controlComp() returns NO cfg key, so the blind
  // wiring (opts.onEvent / opts.cfg?.onEvent) never reached runSim and the bus stayed empty.
  // CompOptions threads SimConfig fields through `cfg` — set it explicitly.
...
// ADAPTATION (driver, 2026-08-05): damage events carry their owner in `unitIdx` (the
// srcSlot field is a STRING slot name, and casterIdx exists only on buffApply events), so
// unitIdx is read FIRST. Same semantics the blind author intended ('the owning slot'); no
// assertion was touched.
const casterOf = (e: AnyEv): number => {
  const a = e as any;
  if (typeof a.unitIdx === 'number') return a.unitIdx;
