...
/* ------------------------------- helpers -------------------------------------- */
const nearV = (a: any, b: number) => typeof a === 'number' && Math.abs(a - b) < 0.005
...
const mapFx = (o: any, pred: (e: any) => boolean, fn: (e: any, b: any) => void) => {
  for (const b of [].concat(o.skill1 ?? [], o.skill2 ?? [], o.burst ?? [])) for (const e of b.effects ?? []) if (pred(e)) { patchHits++; fn(e, b) }
}

interface Run { res: any; events: any[]; patched: number }

function run(mutate?: (o: any) => void): Run {
  patchHits = 0
  const events: any[] = []
  const base: any = { slugs: ['liter', SLUG, 'ada'], bossElement: 'Fire', focusSlug: 'ada' }
  const opts: any = {
    ...base,
    cfg: { ...(base.cfg ?? {}), onEvent: (e: any) => { events.push({ ...e, i: events.length }) } },
  }
  if (mutate) opts.overrides = { ...(base.overrides ?? {}), [SLUG]: withPatchedOverride(SLUG, mutate) }
...
const applies = (r: Run, stat: string, pred?: (e: any) => boolean) =>
  r.events.filter((e) => e.kind === 'buffApply' && e.stat === stat && (!pred || pred(e)))
const removes = (r: Run, stat: string, pred?: (e: any) => boolean) =>
  r.events.filter((e) => e.kind === 'buffRemove' && e.stat === stat && (!pred || pred(e)))
...
const SNAP_GENERIC = run((o) => mapFx(o, (e) => isBuff(e, 'normalAttackPct'), (e) => { e.stat = 'attackDamagePct' }))
...
const CASTER_FLAT_MAX = run((o) =>
  mapFx(o, (e) => isBuff(e, 'casterAtkPct'), (e) => {
    e.value = 39
    e.maxStacks = 1
    delete e.rampSec
...
const NO_PRECIOUS = run((o) =>
  dropFx(o, (e) => isBuff(e, 'atkPct') && (nearV(e.value, 2.49) || nearV(e.value, 4.98) || nearV(e.value, 7.47))),
)

// nearest-wrong: 'when Full Burst ends' read as 'when entering Full Burst'
const S1A_FB_ENTER = run((o) => {
  for (const b of [].concat(o.skill1 ?? [], o.skill2 ?? [], o.burst ?? [])) {
    if ((b.effects ?? []).some((e: any) => isBuff(e, 'casterAtkPct'))) { patchHits++; b.trigger = { kind: 'fullBurstEnter' } }
  }
})
...
const PELLET_PROXY = run((o) =>
  mapFx(o, (e) => isBuff(e, 'pelletCountFlat'), (e) => { e.value = 10 * (e.value ?? 1); e.stat = 'normalAttackPct' }),
)

// nearest-wrong: the 554.4% burst hit unmodeled
const NO_NUKE = run((o) => dropFx(o, (e) => e.kind === 'flatDamage' && nearV(e.atkPct, 554.4)))

/* ---- identify her slot from arcana-fortune-mate-only magnitudes (blind-safe) --- */
const ARCANA_SIG = (e: any) =>
  e.kind === 'buffApply' && e.casterIdx != null && (
...
const arcanaCasts = BASE.events.filter((e) => e.kind === 'burstCast' && idxOf(e) === arcanaIdx)
const shotsOf = (r: Run) => r.events.filter((e) => e.kind === 'shot' && idxOf(e) === arcanaIdx).length
...
describe('arcana-fortune-mate / fixture validity (non-vacuity)', () => {
  it('she is identifiable, casts her own burst, and the team reaches Full Burst', () => {
    ...
    expect(BASE.events.filter((e) => e.kind === 'fullBurstEnd').length).toBeGreaterThan(0)
  })

  it('exercises BOTH sides of the Making-Memories gate (in-window and out-of-window shots)', () => {
    const firstCast = arcanaCasts[0]
    const inFb = BASE.events.filter((e) => e.kind === 'damage' && idxOf(e) === arcanaIdx && e.inFullBurst === true)
    const outFb = BASE.events.filter((e) => e.kind === 'damage' && idxOf(e) === arcanaIdx && e.inFullBurst === false)
...
describe('S1-b / S1-c  Snapshots of Youth: Normal Attack Damage Multiplier +10%, wiped at FB end', () => {
  it('is modeled and load-bearing, and moves nobody else (inertness)', () => {
    ...
    for (const m of MATES) expect(unitDmg(NO_SNAP, m)).toEqual(unitDmg(BASE, m))
  })
...
describe('S2-a  normal-attack thresholds while in Making Memories (2 / 4 / 6)', () => {
  it('the whole family is gated on HER burst - nothing fires before her first cast', () => {
    ...
    const precious = applies(BASE, 'atkPct', (e) => nearV(e.value, 2.49) || nearV(e.value, 4.98) || nearV(e.value, 7.47))
  ...
  it('4x: Happy Memories is +1 pellet per stack, <=3, load-bearing, and gauge/teammate-inert', () => {
    ...
    for (const m of MATES) expect(unitDmg(NO_PELLET, m)).toEqual(unitDmg(BASE, m))
  })
  ...
  it('6x: Precious Moments is a SELF ATK% stack of 2.49 (<=3), load-bearing', () => {
    ...
    const ap = applies(BASE, 'atkPct', (e) => nearV(e.value, 2.49) || nearV(e.value, 4.98) || nearV(e.value, 7.47))
    ...
    for (const m of MATES) expect(unitDmg(NO_PRECIOUS, m)).toEqual(unitDmg(BASE, m))
  })
...
describe('S2-b  Burst use: all shotgun-wielding allies (except self) Attack Damage +55% / 10s', () => {
  it('never lands on a non-shotgun ally, and never on self (inertness in this comp)', () => {
    ...
    expect(applies(BASE, 'attackDamagePct', (e) => nearV(e.value, 55)).length).toBe(0)
  })
...
describe('burst  Making Memories self-mode + 554.4% burst hit', () => {
  it('Critical Rate +20.09% lands once per cast, on her only, and expires', () => {
    ...
    expect(removes(BASE, 'critRatePct', (e) => nearV(e.value, 20.09)).length).toBeGreaterThan(0)
  })
  ...
  it('Attack Damage +29.99% lands once per cast, on her only, and expires', () => {
    ...
    expect(ap.length).toBeLessThanOrEqual(arcanaCasts.length)
    expect(new Set(ap.map((e) => e.targetIdx))).toEqual(new Set([arcanaIdx]))
    expect(removes(BASE, 'attackDamagePct', (e) => nearV(e.value, 29.99)).length).toBeGreaterThan(0)
  })
  ...
  it('the 554.4% hit is modeled, lands once per cast, and is Full-Burst-exempt by timing', () => {
    ...
    const mine = BASE.events.filter((e) => e.kind === 'damage' && idxOf(e) === arcanaIdx)
    const byBucket = new Map<string, any[]>()
    for (const d of mine) byBucket.set(d.bucket, [...(byBucket.get(d.bucket) ?? []), d])
    // heuristic, bucket-name-agnostic: the burst hit is the bucket with exactly one event per cast
    const nuke = [...byBucket.values()].find((list) => list.length === arcanaCasts.length)
