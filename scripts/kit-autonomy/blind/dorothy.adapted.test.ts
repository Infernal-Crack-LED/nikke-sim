 ...
 * The pristine blind test (dorothy.test.ts) is preserved verbatim. The blind writer
 * derived this test with NO sight of the harness API or the driver's override, so two
 * points are corrected below with `ADAPTED:` markers. NOT ONE kit assertion (stat identity /
 * value / trigger / target / Brand delay / Pierce window) is weakened — only the one contrast
 * the driver's documented encoding cannot satisfy.
 *
 *   [P1] import path → '../../tests/lib/harness.js' (real harness location; blind guessed
 *        '../lib/harness.js').
 *   [P2] S1b "during Manifestation" GATE: the blind asserts the partsDamagePct block carries a
 *        window gate and that the gated model fires strictly fewer applications than an ungated
 *        one. The DRIVER documents this gate as deliberately UNENFORCED (override note + caveat):
 *        partsDamagePct is INERT in v1 (the scope-lock boss has no parts — the blind's OWN
 *        "damage-INERT" assertion below passes, bfNoParts.t === bf.t), and the engine has NO
 *        self-buff-active gate primitive that could express "while my own burst state is live"
 *        (the blind override-writer's `mode:'manifestation'` stand-in is user-selected and inert
 *        by default, so it would make the buff NEVER fire — worse). Gated-vs-ungated is therefore
 *        byte-identical on damage and the gate contrast cannot hold against any faithful override.
 *        Same shape as the anchor-innocent-maid [P7] precedent: redirected to assert the DOCUMENTED
 *        behavior — the parts block is present (lastBullet/allies/50.68/5s) and fires (>0
 *        applications), and the gate is the recorded ⚑, not a live discrimination. Kit reading
...
describe('dorothy S1b — last bullet during Manifestation: Damage to Parts ▲50.68% for 5s', () => {
  ...
  it('is a lastBullet block granting partsDamagePct 50.68 for 5s to all allies', () => {
    expect(blocks).toHaveLength(1);
    const b = blocks[0] as Block;
    expect(b.trigger.kind).toBe('lastBullet');
    expect(b.target.kind).toBe('allies');
    ...
    // ADAPTED [P2]: the kit's "during Manifestation" is a self-state WINDOW gate the engine has
    // no primitive to express; the driver documents it UNENFORCED because partsDamagePct is inert
    // in v1 (the "damage-INERT" test below passes). The gate is the recorded ⚑, asserted as such
    // in the driver spec (D5-style unmodeled/caveat record), not a live gate on the block.
  });

  it('ADAPTED [P2]: the parts buff is present and fires; the Manifestation gate is the recorded ⚑', () => {
    // The blind's gated<ungated contrast cannot hold: the effect is inert and the gate primitive
    // does not exist, so gated === ungated on damage. Assert the documented behavior instead —
    // the buff IS applied (>0 applications) and is damage-inert (next test), and the gate is the
    // driver's recorded ⚑ (override caveat), not a discrimination the engine can produce.
    const applied = partsApplies(bf.events).length;
    expect(applied).toBeGreaterThan(0);
    expect(partsApplies(bfPartsUngated.events).length).toBe(applied); // gate is moot: identical
  });
