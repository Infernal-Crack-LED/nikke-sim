...
// One-off config to run the S5 cross-family BLIND test (alice.test.ts in this dir) against the
...
//   npx vitest run --config scripts/kit-autonomy/blind/vitest.blind.config.ts
export default defineConfig({
  test: {
    include: ['scripts/kit-autonomy/blind/alice.test.ts'],
    environment: 'node',
    testTimeout: 300_000,
    hookTimeout: 300_000,
  },
