...
describe('sugar kit faithfulness', () => {
  ...
  describe('skill2', () => {
    it('self critRate buff applies on every Full Burst start', async () => {
      ...
      const fbFrames = events
        .filter((e) => e.kind === 'fullBurstStart')
        .map((e) => (e as any).frame);
      ...
      expect(crits.every((c) => fbFrames.includes((c as any).frame))).toBe(
        true
      );
          ...
          sugar: withPatchedOverride('sugar', (ov) => {
            ov.skill2!.blocks = ov.skill2!.blocks.filter(
              (b: any) =>
                !b.effects.some(
                  (eff: any) =>
                    eff.kind === 'buff' && eff.stat === 'critRatePct'
                )
            );
          }),
        },
      ...
      expect(
        buffApplies(noCrit.events, 'critRatePct', 13.02, 'sugar').length
      ).toBe(0);
    });
    ...
    it('SG maxAmmo buff applies to sugar (self, SG) on every Full Burst start', async () => {
      ...
      expect(ammos.every((a) => fbFrames.includes((a as any).frame))).toBe(
        true
      );

      // Inertness: non-SG allies must not receive this buff.
      const otherAmmo = events.filter(
        (ev) =>
          ev.kind === 'buffApply' &&
        ...
        overrides: {
          sugar: withPatchedOverride('sugar', (ov) => {
            ov.skill2!.blocks = ov.skill2!.blocks.filter(
              (b: any) =>
                !b.effects.some(
                  (eff: any) => eff.kind === 'buff' && eff.stat === 'maxAmmoPct'
                )
            );
          }),
        },
      ...
      expect(
        buffApplies(noAmmo.events, 'maxAmmoPct', 83.8, 'sugar').length
      ).toBe(0);
    });
  });
  ...
  describe('burst', () => {
    it('self attackSpeed and hitRate apply on sugar burst cast', async () => {
      ...
      const sugarBurstFrames = events
        .filter(
          (ev) => ev.kind === 'burstCast' && (ev as any).casterIdx === sugarIdx
        )
      ...
      if (sugarBurstFrames.length > 0) {
        ...
        expect(
          as.every((a) => sugarBurstFrames.includes((a as any).frame))
        ).toBe(true);
        expect(
          hr.every((h) => sugarBurstFrames.includes((h as any).frame))
        ).toBe(true);
      }

      // Inertness: other allies must not receive these self burst buffs.
      const otherAs = events.filter(
        (ev) =>
          ev.kind === 'buffApply' &&
          (ev as any).stat === 'attackSpeedPct' &&
          (ev as any).targetSlug !== 'sugar'
      );
      const otherHr = events.filter(
        (ev) =>
          ev.kind === 'buffApply' &&
          (ev as any).stat === 'hitRatePct' &&
          (ev as any).targetSlug !== 'sugar'
      );
          ...
          sugar: withPatchedOverride('sugar', (ov) => {
            ov.burst!.blocks = ov.burst!.blocks.filter(
              (b: any) =>
                !b.effects.some(
                  (eff: any) =>
                    eff.kind === 'buff' && eff.stat === 'attackSpeedPct'
                )
            );
          }),
        },
      ...
      expect(
        buffApplies(noAs.events, 'attackSpeedPct', 66, 'sugar').length
      ).toBe(0);
      expect(unitOf(noAs.res, 'sugar').totalDamage).toBeLessThan(
        unitOf(res, 'sugar').totalDamage
      );
        ...
        overrides: {
          sugar: withPatchedOverride('sugar', (ov) => {
            ov.burst!.blocks = ov.burst!.blocks.filter(
              (b: any) =>
                !b.effects.some(
                  (eff: any) => eff.kind === 'buff' && eff.stat === 'hitRatePct'
                )
            );
          }),
        },
      });
      expect(buffApplies(noHr.events, 'hitRatePct', 33, 'sugar').length).toBe(
        0
      );
    });
  });
