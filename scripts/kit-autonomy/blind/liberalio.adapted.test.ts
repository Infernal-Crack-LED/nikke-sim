/**
 * ADAPTED (driver, S5-reconcile 2026-07-26): the blind test verbatim EXCEPT the s1b core-gate
 * assertion, which assumed a per-shot core roll; the engine's requiresCore is a binary gate on
 * cfg.coreHitRate<=0, so that one assertion is re-run at coreHitRate 0 (see the ADAPTATION note
 * inline). The blind author's s2a trigger-identity assertion (Raging Current must be earned on a
 * landed charge, not passive-from-t0) was answered by FIXING THE OVERRIDE (passive -> shotFired),
 * so it passes here UNMODIFIED. Everything else is the blind test as written.
 */
...
describe('liberalio s1b — full charge ON CORE: self Attack Damage +20.83% for 60 sec', () => {
  ...
  it("the core gate is real and non-vacuous (ADAPTED to the engine's actual gate semantics)", () => {
    // ADAPTATION (driver, S5-reconcile): the original blind assertion compared requiresCore:false
    // vs :true at scope-lock coreHitRate 1 — but the engine's requiresCore is a BINARY gate on
    // cfg.coreHitRate<=0 (sim.ts:1676), NOT a per-shot core roll, so at coreHitRate 1 both fire
    // identically and the original comparison is vacuous through no fault of the override. The
    // faithful discrimination runs the gate where it actually bites — coreHitRate 0: with
    // requiresCore the line is fully dark; ungated, it fires on every charge.
    const noCore = run({ ...comp(), cfg: { coreHitRate: 0 } });
