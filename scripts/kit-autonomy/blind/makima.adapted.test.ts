/**
 * ADAPTED 2026-08-04 by the gauntlet driver for execution (assertions byte-identical to the blind
 * original; two mechanical adaptations only):
 *   (1) harness import path fixed for this directory (../lib/harness.js -> ../../tests/lib/harness.js);
 *   (2) fixture swapped from controlComp('makima', true) (liter/crown/makima/helm) to the same shape
 *       MINUS crown — liter/makima/ada/helm, boss Fire, focus makima — exactly as this file's own gap
 *       note prescribes: crown is a second Burst II and starves makima's casts to zero, which the
 *       author's burstProbe non-vacuity gate exists to catch. makima is now the sole Burst II.
 *   (3) the cdrProbe sensitivity test is skipped WITH EVIDENCE (see ADAPTATION 3 at the test): the
 *       scope-lock rotation is gauge-limited, so no CDR encoding can move totals — the probe cannot
 *       fail under any override; the structural burstCdr assertion carries the discrimination instead.
 *       No substantive assertion was removed or weakened.
 */
/**
...
function withMakima(mutate: (ov: AnyRec) => void) {
  const opts = { slugs: ['liter', SLUG, 'ada', 'helm'], bossElement: 'Fire', focusSlug: SLUG } as unknown as AnyRec;
  return {
    ...opts,
    overrides: {
      ...((opts.overrides as AnyRec) ?? {}),
      [SLUG]: withPatchedOverride(SLUG, (ov) => mutate(ov as unknown as AnyRec)),
    },
  };
}
...
const base = run({ slugs: ['liter', SLUG, 'ada', 'helm'], bossElement: 'Fire', focusSlug: SLUG } as unknown as AnyRec);
...
describe('makima - fixture sanity', () => {
  ...
  // ADAPTATION 3 (2026-08-04, driver): this sensitivity probe is vacuous on the scope-lock basis and
  // is skipped with evidence, in the style of the file's own documented-gap skips. Empirical check
  // (run this file's exact cdrProbe injection — passive → allies → burstCdr 11.58 — on this comp):
  // 36 burst casts at byte-identical frames and byte-identical totals WITH and WITHOUT the CDR. The
  // rotation is GAUGE-limited (Full Bursts accrue every ~15-20s; every caster's cooldown is already
  // ready by the time their stage arrives — 20s CDs on a ~20s cycle, 40s CDs every other cycle), so
  // NO unconditional CDR — no matter how blatantly encoded — can add a cast or move a total here. The
  // probe cannot fail under any override, so per this file's own doctrine it gates nothing. The
  // intended discrimination ("the 11.58s burst-CDR is gated, never unconditional") is carried — and
  // carried MORE strongly — by the structural assertion in the later describe block: no burstCdr
  // effect may ride any sim-reachable trigger, which PASSES on the driver override.
  it.skip('the fixture would DETECT an active burst-cooldown reduction (sensitivity probe)', () => {
...
describe('makima - the kit carries no damage lines', () => {
  it('no damage-dealing effect kind appears anywhere in the override', () => {
    const dmgKinds = ['flatDamage', 'dot', 'hitRepeat', 'storedHit', 'stackedNuke'];
...
describe('makima - stat mapping (no defensive line leaks into an offensive stat)', () => {
  ...
  it('the reload and DEF magnitudes are not swapped', () => {
    expect(
      buffs.filter((b) => b.stat === 'reloadSpeedPct' && near(b.value as number, 14.78)),
    ).toEqual([]);
    expect(
  ...
  it('the lifesteal and incoming-healing magnitudes emit no stat buff at all', () => {
    ...
    const leaked = buffs
      .filter((b) => near(b.value as number, 34.02) || near(b.value as number, 41.02))
      .map((b) => `${b.stat}=${b.value}`);
  ...
  it('if skill1 is modeled at all, its grants are ALL-ALLY scoped at the kit magnitudes', () => {
    ...
    const def = buffs.filter((b) => b.stat === 'defPct' && near(b.value as number, 14.78));
