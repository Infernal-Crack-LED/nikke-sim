 ...
 *
 * FIXTURE: controlComp('himeno', true) -> liter (B1) / crown (B2) / himeno (carry) / helm (B3).
...
describe('himeno skill2 — Max Ammunition (up) 2 round(s) for 10 sec, snipers only', () => {
  ...
  it('is DAMAGE-BEARING: +2 rounds on a 6-round magazine removes reloads', () => {
    expect(unitOf(control.res, SLUG).totalDamage).toBeGreaterThan(
  ...
  it('flat vs percent: the same 2 read as maxAmmoPct would be ~inert', () => {
    ...
    expect(unitOf(ammoAsPct.res, SLUG).totalDamage).toBeLessThan(
