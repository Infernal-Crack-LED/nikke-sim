...
// ADAPTED (driver, 2026-08-04 — fixture isolation, no assertion weakened): helm's
// full-charge S1 heal + burst heal and crown's hitCount-860 self-heal SATURATE crown's
// on-recovery consumer in BOTH arms, masking sakura's burst HoT (byte-identical totals
// with/without her heal). Strip those three heal sources so her HoT is the ONLY recovery
// stream in the fight — the helm.test.ts H8 isolation pattern.
const helmNoHeal = withPatchedOverride('helm', (ov) => {
  for (const slot of ['skill1', 'burst'] as const) {
    const before = (ov as any)[slot].length;
    (ov as any)[slot] = (ov as any)[slot].filter(
      (b: any) => !b.effects.some((e: any) => e.kind === 'heal'),
    );
    if ((ov as any)[slot].length === before) {
      throw new Error(`helm ${slot} heal block missing — fixture is stale`);
    }
  }
});
