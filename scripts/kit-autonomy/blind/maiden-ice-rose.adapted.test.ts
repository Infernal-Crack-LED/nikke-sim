...
function buffEffects(ov: any, stat: string): any[] {
  return effectsOf(allBlocks(ov)).filter(
...
describe('maiden-ice-rose - skill1 MP economy', () => {
  it('models MP as a pool capped at 12, starting empty', () => {
    expect(
      MP_POOL,
      'kit states MP accumulates to a maximum of 12'
    ).toBeTruthy();
  ...
  it('gains MP on Burst Stage 1 entry, gated to MP === 0', () => {
    ...
    expect(b.resourceGate?.max, 'clause reads: affects self when MP is 0').toBe(
      0
    );
  });
  ...
  it('gains MP on Full Burst entry behind a non-zero MP floor', () => {
    const blocks = (OV.skill1 ?? []).filter(
      (b: any) =>
        b.trigger?.kind === 'fullBurstEnter' && resourceEffects([b]).length > 0
    );
    expect(blocks.length, 'full-burst-entry MP block missing').toBeGreaterThan(
      0
    );
    const b = blocks[0];
    expect(b.target?.kind).toBe('self');
    ...
    expect(
      b.resourceGate?.min,
      'clause reads: affects self when MP is above 1'
    ).toBeGreaterThanOrEqual(1);
  });

  it('spends the whole accumulated pool on her burst', () => {
    const spends = resourceEffects(allBlocks(OV)).filter(
      (e: any) => e.delta < 0
    );
...
describe('maiden-ice-rose - skill1 Max HP stacks on every 6th Full Charge', () => {
  it('is a self 6.34% Max HP buff, 15s, 10 stacks, on a 6-full-charge counter', () => {
    ...
    expect(
      b.trigger?.kind,
      'kit says attacking with Full Charge, not hits/shots'
    ).toBe('chargeCounter');
    expect(triggerCount(b)).toBe(6);
    const e = (b.effects ?? []).find(
      (x: any) => x.kind === 'buff' && x.stat === 'targetMaxHpPct'
    );
  ...
  it('applies repeatedly over 180s and never exceeds 10 stacks', () => {
    ...
    expect(
      evs.length,
      'no self Max HP grant observed - the counter never fires'
    ).toBeGreaterThanOrEqual(5);
    expect(evs.some((e: Ev) => e.maxStacks === 10)).toBe(true);
    expect(Math.max(...evs.map((e: Ev) => e.stacks ?? 1))).toBeLessThanOrEqual(
      10
    );
  });
...
describe('maiden-ice-rose - skill2 on MP replenished (Electric allies except self)', () => {
  ...
  it('targets Electric allies excluding self, on an MP-replenish trigger', () => {
    expect(
      allyBlocks.length,
      '40.9% Electric-ally block missing'
    ).toBeGreaterThan(0);
...
describe('maiden-ice-rose - skill2 on MP used (self, own burst cast)', () => {
  it('is a self block keyed to her own burst cast', () => {
    ...
    expect(
      t === 'burstCast' || (t === 'fullBurstEnter' && b.ownBurstGate === 'cast')
    ).toBe(true);
  });
  ...
  it('applies both self buffs together, as raw percentages', () => {
    ...
    expect(hp.length, 'MP-used self buff never fired').toBeGreaterThanOrEqual(
      1
    );
  ...
  it('fires per OWN burst cast, not per team Full Burst', () => {
    ...
    expect(
      anyFb,
      'fixture does not separate own-cast from team Full Burst'
    ).toBeGreaterThan(own);
  });
});
...
describe('maiden-ice-rose - skill2 547.62% Full Charge rider', () => {
  it('is a per-full-charge enemy hit of 547.62%, with no core strike', () => {
    ...
    expect(blocks.length, '547.62% rider missing from skill2').toBeGreaterThan(
      0
    );
    const b = blocks[0];
    ...
    expect(e.core === true, 'text says damage, not core strike damage').toBe(
      false
    );
  });
  ...
  it('carries a large share of her damage', () => {
    ...
    expect(totals(RIDER0.res)[SLUG]).toBeLessThan(
      totals(BASE.res)[SLUG] * 0.85
    );
...
describe('maiden-ice-rose - burst nuke', () => {
  it('is an enemy-targeted flat hit of at least 1372.8% of final ATK', () => {
    const eff = effectsOf(OV.burst ?? []).filter(
      (e: any) => e.kind === 'flatDamage'
    );
    expect(eff.length, 'burst damage effect missing').toBeGreaterThan(0);
    expect(Math.max(...eff.map((e: any) => e.atkPct))).toBeGreaterThanOrEqual(
      1372.8
    );
    expect(blocksWithKind(OV.burst ?? [], 'flatDamage')[0].target?.kind).toBe(
      'enemy'
    );
  });
...
describe('maiden-ice-rose - elemental-advantage scoping', () => {
  it('encodes both advantage lines on the advantage-gated stat, not generic Damage Up', () => {
    const vals = buffEffects(OV, 'elemAdvantageDamagePct').map(
      (e: any) => e.value
    );
  ...
  it('is damage-inert against the Fire control boss (Electric holds no advantage there)', () => {
    ...
    for (const slug of Object.keys(ta)) {
      expect(
        tb[slug],
        slug + ' moved when an inert advantage buff was zeroed'
      ).toBe(ta[slug]);
    }
  });
});
