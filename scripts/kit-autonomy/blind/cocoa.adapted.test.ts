...
// DRIVER ADAPTATION 2 (fixture re-cut, per the blind author's OWN pre-registered rule — "re-cut
// the fixture, not the assertion"): controlComp('cocoa', true) hard-seats helm, whose S1
// full-charge heal (~every 1.4s) keeps crown's 'when recovery takes effect' buff (7s) at
// ~100% uptime (crown override note: "refreshed to near-permanent"). Under that saturation the
// heal-counterfactual cannot move damage totals, so the author's own sensitivity check dies.
// Re-cut: ada (a heal-free Burst III, 0 heal blocks in her override) replaces helm; the chain
// still completes and crown's consumer is quiet unless cocoa emits recovery. All assertion
// text is unchanged. controlComp is no longer used, so it leaves the import list.
import {
  runComp,
  totals,
  unitOf,
  withPatchedOverride,
} from '../../tests/lib/harness.js'; // DRIVER ADAPTATION (mechanical): blind/ lives two levels deeper than tests/units — no assertion changed.
