 ...
 * fixes only, ZERO assertion changes: (1) the harness import path is re-rooted (blind/ sits
 * under scripts/kit-autonomy/, so ../lib/harness.js -> ../../tests/lib/harness.js);
 * (2) the SECONDS-semantics check reads durationShots with a null-tolerant comparison — this
 * engine's buffApply events carry durationShots as null (not undefined) when unset, and the
 * blind author's claim is exactly "never a round count", which null satisfies. Every other
 * assertion, counterfactual, fixture (controlComp('anchor', true)) and it.skip GAP is
 * exactly as the blind author (claude-opus-5) wrote them.
 */
...
import {
  ...
  totals,
  withPatchedOverride,
} from '../../tests/lib/harness.js'; // path fixed 2026-08-05 (driver, mechanical) — no assertion changed
...
describe('anchor — skill1: DEF ▲ 23.82% for 5 sec (self, on last bullet)', () => {
  ...
  it('carries SECONDS semantics — not a round count, not permanent', () => {
    ...
    for (const e of applies) {
      // MECHANICAL fix 2 2026-08-05 (driver): buffApply events carry durationShots as null
      // (not undefined) when unset — the semantic claim is unchanged: never a round count.
      expect(e.durationShots == null).toBe(true); // "for 5 sec", never "for N round(s)"
