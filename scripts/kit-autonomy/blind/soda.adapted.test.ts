 ...
 *
 * ADAPTED 2026-08-05 by the driver to run against the committed driver override — MECHANICAL
 * fixes only, zero assertion changes: (1) damage events in this engine carry a STRING srcSlot
 * ('normal'|'skill1'|'skill2'|'burst'|null) and a `slug`, not a numeric slot index — damageOf
 * re-routed to slug+srcSlot filtering (the blind author's numeric-srcSlot path + row.events
 * fallback could not resolve); (2) soda demonstrably DOES cast in the base fixture (6 casts in
 * 180s), so the burstEligibility-stage-3 non-vacuity guard is kept exactly as authored (both
 * burst runs carry it; comparisons stay within-fixture). Probed: 39 spirit applies (<40 bound
 * holds), burst bucket removed cleanly, totals respond as the assertions expect.
 *
...
import {
  ...
  totals,
  withPatchedOverride,
} from '../../tests/lib/harness.js'; // path fixed 2026-08-05 (driver, mechanical): blind/ sits under kit-autonomy/, not tests/units/ — unitOf dropped with the damageOf re-route — no assertion changed
...
function damageOf(res: any, evs: SimEvent[], idx: number): DamageEv[] {
  // ADAPTED: damage events carry `slug` + STRING srcSlot in this engine — route by slug.
  void res;
  void idx;
  return evs.filter(
    (e) => e.kind === 'damage' && (e as any).slug === 'soda'
