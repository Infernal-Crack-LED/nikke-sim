...
const near = (a: number, b: number, eps = 0.5) => Math.abs((a ?? NaN) - b) <= eps
const compose = (...ms: Mut[]): Mut => (o) => { for (const m of ms) m(o) }

/** drop the 'no other Burst 1 allies' gate -> the My Own Star branch goes live in a liter comp */
const ungateNoB1: Mut = (o) => { for (const b of blocksOf(o)) if (b.formation === 'noB1') delete b.formation }
const zeroBuff = (stat: string, value?: number): Mut => (o) =>
  eachEffect(o, (e) => {
    if (e.kind === 'buff' && e.stat === stat && (value === undefined || near(e.value, value))) e.value = 0
  })
const zeroRider: Mut = (o) => eachEffect(o, (e) => { if (e.kind === 'flatDamage' && near(e.atkPct, 120.13, 5)) e.atkPct = 0 })
const zeroStars: Mut = (o) => eachEffect(o, (e) => { if (e.kind === 'dot') e.atkPct = 0 })
const zeroCdr: Mut = (o) => eachEffect(o, (e) => { if (e.kind === 'burstCdr') e.seconds = 0 })
const cdrOncePerBattle: Mut = (o) => eachEffect(o, (e) => { if (e.kind === 'burstCdr') e.oncePerBattle = true })
const casterAtkToOwnAtk: Mut = (o) => eachEffect(o, (e) => { if (e.kind === 'buff' && e.stat === 'casterAtkPct') e.stat = 'atkPct' })
const dropHeals: Mut = (o) => {
  const bs = blocksOf(o)
  for (const b of bs) b.effects = (b.effects ?? []).filter((e: any) => e.kind !== 'heal')
...
const takeBurstFirst: Mut = (o) => {
  blocksOf(o).push({ slot: 'skill1', trigger: { kind: 'passive' }, target: { kind: 'self' }, effects: [{ kind: 'burstFirst' }] })
}

/** the committed override, captured through the patch clone (never mutated) */
let OVR: any = null
withPatchedOverride(SLUG, (o: any) => { OVR = JSON.parse(JSON.stringify(o)) })
const allEffects = (): any[] => { const out: any[] = []; eachEffect(OVR, (e) => out.push(e)); return out }

// ---------------------------------------------------------------- run helper
function go(comp: any, mutate?: Mut) {
  const evs: Ev[] = []
  const opts: any = { ...comp, cfg: { ...(comp.cfg ?? {}), onEvent: (e: Ev) => evs.push(e) } }
  if (mutate) {
...
// ---------------------------------------------------------------- event selectors
const ownerOf = (e: Ev): number | undefined => e.unitIdx ?? e.srcIdx ?? e.slotIdx ?? e.idx
const tOf = (e: Ev): number | undefined =>
  e.t ?? e.time ?? e.sec ?? (typeof e.frame === 'number' ? e.frame / 60 : undefined)
const amtOf = (e: Ev): number => e.amount ?? e.damage ?? e.dmg ?? e.value ?? 0
const coreRateOf = (e: Ev): number | undefined => e.coreRate ?? e.core ?? e.coreRateApplied
const of_ = (evs: Ev[], kind: string) => evs.filter((e) => e.kind === kind)
const anisDamage = (evs: Ev[]) => of_(evs, 'damage').filter((e) => ownerOf(e) === ANIS_IDX)
const sumAnis = (evs: Ev[]) => anisDamage(evs).reduce((a, e) => a + amtOf(e), 0)
const sumTeam = (evs: Ev[]) => of_(evs, 'damage').reduce((a, e) => a + amtOf(e), 0)
const sumOthers = (evs: Ev[]) => of_(evs, 'damage').filter((e) => ownerOf(e) !== ANIS_IDX).reduce((a, e) => a + amtOf(e), 0)
const fbCount = (evs: Ev[]) => of_(evs, 'fullBurstStart').length
const anisCasts = (evs: Ev[]) => of_(evs, 'burstCast').filter((e) => ownerOf(e) === ANIS_IDX || e.casterIdx === ANIS_IDX).length
...
const applies = (evs: Ev[], stat: string, value?: number) =>
  of_(evs, 'buffApply').filter(
    (e) => e.stat === stat && e.targetIdx !== null && e.targetIdx !== undefined && (value === undefined || near(e.value, value)),
  )
const targetsOf = (evs: Ev[], stat: string, value?: number) => new Set(applies(evs, stat, value).map((e) => e.targetIdx))
const relDiff = (a: number, b: number) => Math.abs(a - b) / Math.max(1, Math.abs(a))

// ---------------------------------------------------------------- hoisted runs (17 x 180 s)
const BASE = go(controlComp(SLUG, true))
const NO_RIDER = go(controlComp(SLUG, true), zeroRider)
const NO_EXPL = go(controlComp(SLUG, true), zeroBuff('projectileExplosionPct', 92.03))
const NO_ATKDMG = go(controlComp(SLUG, true), zeroBuff('attackDamagePct', 34))

const UNGATED = go(controlComp(SLUG, true), ungateNoB1)
const UNGATED_NO_MOS = go(controlComp(SLUG, true), compose(ungateNoB1, zeroBuff('atkPct', 40.01)))
const UNGATED_NO_CDR = go(controlComp(SLUG, true), compose(ungateNoB1, zeroCdr))
const UNGATED_CDR_ONCE = go(controlComp(SLUG, true), compose(ungateNoB1, cdrOncePerBattle))
const UNGATED_SELFSCALE = go(controlComp(SLUG, true), compose(ungateNoB1, casterAtkToOwnAtk))
...
const CAST_NO_STARS = go(controlComp(SLUG, true), compose(takeBurstFirst, zeroStars))
const CAST_NO_CHARGE = go(controlComp(SLUG, true), compose(takeBurstFirst, zeroBuff('chargeSpeedPct')))
const CAST_NO_DEF = go(controlComp(SLUG, true), compose(takeBurstFirst, zeroBuff('defPct', 55.01)))
const CAST_NO_MAXHP = go(controlComp(SLUG, true), compose(takeBurstFirst, zeroBuff('casterMaxHpPct', 15.02)))
const CAST_UNGATED = go(controlComp(SLUG, true), compose(takeBurstFirst, ungateNoB1))
...
describe('anis-star :: harness + patch-channel contract', () => {
  it('the fixture runs, anis deals damage, and events expose owner + time', () => {
    ...
    expect(of_(BASE.evs, 'damage').some((e) => ownerOf(e) !== undefined)).toBe(true)
    expect(of_(BASE.evs, 'damage').some((e) => tOf(e) !== undefined)).toBe(true)
...
describe('anis-star :: S1-b  squad-formation branch (My Own Star vs Everyone Star)', () => {
  ...
  it('the -7.48 s Burst-Skill cooldown is gated too, and buys Full Bursts when live', () => {
    ...
    expect(fbCount(UNGATED_NO_CDR.evs)).toBeLessThanOrEqual(fbCount(UNGATED.evs))
  })
...
describe('anis-star :: S1-c  Full Charge rider, 120.13% of final ATK', () => {
  ...
  const shots = of_(BASE.evs, 'shot').filter((e) => ownerOf(e) === ANIS_IDX).length
  ...
  it('takes no +30% range bonus and no core bucket (rider convention)', () => {
    const normals = anisDamage(BASE.evs).filter((e) => e.srcSlot !== 'skill1' && e.srcSlot !== 'burst')
  ...
  it('is a self-damage line only -- teammates are byte-identical without it', () => {
    expect(relDiff(sumOthers(BASE.evs), sumOthers(NO_RIDER.evs))).toBeLessThan(1e-9)
  })
})
...
describe('anis-star :: S2-a  FB-enter ATK +35.01% OF THE CASTER ATK (My Own Star gated)', () => {
  ...
  it('lasts 10 s (not permanent, not the full rotation)', () => {
    ...
    const rm = of_(UNGATED.evs, 'buffRemove').find(
      (e) => e.stat === 'casterAtkPct' && e.targetIdx === a.targetIdx && (tOf(e) ?? 0) > (tOf(a) ?? 0),
    )
  ...
  it('scales off the CASTER ATK, not each ally own ATK', () => {
    ...
    expect(relDiff(sumTeam(UNGATED.evs), sumTeam(UNGATED_SELFSCALE.evs))).toBeGreaterThan(1e-3)
  })
})
...
describe('anis-star :: S2-b  Everyone-Star full-charge heal (tandem line, NOT defensive-skip)', () => {
  it('the heal drives crown on-recovery consumer -- removing it costs the team buffs', () => {
    ...
    expect(of_(NH_BASE.evs, 'buffApply').length).toBeGreaterThan(of_(NH_NOHEAL.evs, 'buffApply').length)
  })

  it('...and moves real damage through that consumer', () => {
    expect(relDiff(sumTeam(NH_BASE.evs), sumTeam(NH_NOHEAL.evs))).toBeGreaterThan(1e-6)
  })

  it('repeats per full charge (not a one-shot at battle start)', () => {
    const shots = of_(NH_BASE.evs, 'shot').filter((e) => ownerOf(e) === ANIS_IDX).length
    expect(shots).toBeGreaterThan(10)
    // a one-shot heal could not produce a large buffApply delta; require it to scale
    expect(of_(NH_BASE.evs, 'buffApply').length - of_(NH_NOHEAL.evs, 'buffApply').length).toBeGreaterThan(5)
  })
})
...
describe('anis-star :: S2-c  Projectile Explosion Damage +92.03% for 10 s on FB enter', () => {
  ...
  it('lasts 10 s, not the whole fight', () => {
    const a = applies(BASE.evs, 'projectileExplosionPct', 92.03).filter((e) => e.targetIdx === ANIS_IDX)[0]
    const rm = of_(BASE.evs, 'buffRemove').find(
      (e) => e.stat === 'projectileExplosionPct' && e.targetIdx === ANIS_IDX && (tOf(e) ?? 0) > (tOf(a) ?? 0),
    )
    expect(rm).toBeTruthy()
    expect((tOf(rm) ?? 0) - (tOf(a) ?? 0)).toBeCloseTo(10, 1)
  })
  ...
  it('feeds anis RL damage and is inert on the non-RL teammates', () => {
    ...
    expect(relDiff(sumOthers(BASE.evs), sumOthers(NO_EXPL.evs))).toBeLessThan(1e-9)
  })
})
...
describe('anis-star :: S2-d  Attack Damage +34% to ALL allies for 10 s on FB enter', () => {
  ...
  it('lasts 10 s', () => {
    const a = applies(BASE.evs, 'attackDamagePct', 34).filter((e) => e.targetIdx === ANIS_IDX)[0]
    const rm = of_(BASE.evs, 'buffRemove').find(
      (e) => e.stat === 'attackDamagePct' && e.targetIdx === ANIS_IDX && (tOf(e) ?? 0) > (tOf(a) ?? 0),
    )
    expect(rm).toBeTruthy()
    expect((tOf(rm) ?? 0) - (tOf(a) ?? 0)).toBeCloseTo(10, 1)
  })
...
describe('anis-star :: burst  Shooting Stars 40.01% every 0.25 s for 10 s', () => {
  ...
  it('emits ~40 star hits per cast, not one lump and not a 1 s cadence', () => {
    ...
    const ticks = anisDamage(CAST.evs).filter((e) => e.srcSlot === 'burst').length
...
describe('anis-star :: burst  charge time fixed at 0.7 s for 10 s', () => {
  it('is encoded as a charge-speed window of the right size (1.00 s -> 0.7 s = +42.86%)', () => {
    ...
    const asSpeed = eff.some((e) => e.kind === 'buff' && e.stat === 'chargeSpeedPct' && e.value >= 40 && e.value <= 46)
  ...
  it('raises anis shot count inside the burst window', () => {
    const shots = (r: any) => of_(r.evs, 'shot').filter((e: Ev) => ownerOf(e) === ANIS_IDX).length
...
describe('anis-star :: burst  DEF +55.01% / Max HP +15.02% (kit-complete, offensively inert)', () => {
  it('DEF +55.01% is recorded and moves no damage (v1: self DEF does not feed damage)', () => {
    expect(allEffects().some((e) => e.kind === 'buff' && e.stat === 'defPct' && near(e.value, 55.01))).toBe(true)
    expect(relDiff(sumTeam(CAST.evs), sumTeam(CAST_NO_DEF.evs))).toBeLessThan(1e-9)
  })
  ...
  it('Everyone-Star Max HP +15.02% of caster max HP reaches all allies and stays inert', () => {
    ...
    expect(relDiff(sumTeam(CAST.evs), sumTeam(CAST_NO_MAXHP.evs))).toBeLessThan(1e-9)
  })
})
...
describe('anis-star :: burst  My Own Star Attack Damage +35.2% (self, gated)', () => {
  ...
  it('...and applies to SELF only once the gate is lifted', () => {
    ...
    expect(new Set(applies(CAST_UNGATED.evs, 'attackDamagePct', 34).map((e) => e.targetIdx)).size).toBe(4)
  })
})
