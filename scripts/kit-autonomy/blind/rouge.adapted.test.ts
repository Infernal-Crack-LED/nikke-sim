...
// blind/rouge.test.ts. THREE structural corrections to blind-writer assumptions that were
// unverifiable from the redacted packet — assertion INTENT unchanged:
//   (1) onEvent must be wired through CompOptions.cfg.onEvent, not opts.onEvent (the harness reads
//       cfg.onEvent; the blind guess left the event array empty).
//   (2) buffApply.durationShots is null (not undefined) when a buff has no round budget — engine
//       field type is (number | null). toBeUndefined() -> toBeNull().
//   (3) controlComp slots the carry MID-comp (slot 2 of 4), where selfAndAdjacent sides:2 already
//       covers all four allies and the positional scope is unobservable. Rouge is placed at edge
//       slot 0 (rouge/crown/ada/helm) so selfAndAdjacent(2) = {0,1,2} and widening to all-allies
//       provably adds slot 3 — exactly the discrimination the assertion intends.
...
describe('rouge burst — ATK \u25b2 15.07% of the skill user\u2019s ATK, 10 sec, all allies', () => {
  ...
  it('lasts 10 sec (expiresFrame is ~600 frames past apply), not permanent', () => {
    ...
    expect(e.durationShots).toBeNull(); // seconds, not ROUNDS
  });
