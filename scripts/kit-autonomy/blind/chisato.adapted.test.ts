...
describe('chisato skill1 \u2014 ">25%: Hit Rate \u25b2 22.37%" tier', () => {
  ...
  it('Hit Rate lifts the CORE bucket, not the plain bucket \u2014 \u2691 magnitude unasserted', () => {
    ...
    // SCHEMA ADAPTATION (driver, schema-only): there is NO 'core' damage bucket — the damage
    // event's bucket ∈ {normal,skill,burst} and core-ness is the per-hit `coreRate` field, not a
    // bucket. hitRatePct lifts the core RATE on her normals (engine acrForHR path), so read the max
    // coreRate on chisato's normals instead of summing a non-existent 'core' bucket. Intent
    // preserved: direction-only proof that hitRatePct lifts core (magnitude stays ⚑-unasserted).
    const coreRateOf = (evs: Ev[]) =>
      Math.max(
        0,
        ...chisatoDmg(evs)
...
describe('chisato \u2014 global inertness', () => {
  ...
  it('no weapon-state modifiers: the kit changes no ammo / reload / fire rate', () => {
    ...
    // SCHEMA ADAPTATION (driver, schema-only): filter by chisato as CASTER (casterIdx), not as
    // HOLDER (targetSlug) — controlComp teammates (crown/liter) legitimately buff chisato with
    // weapon-state stats, which a target-based filter wrongly counts. The stated intent is that
    // CHISATO'S OWN kit emits none, so key on her caster slot. Still discriminates: any
    // maxAmmo/reload/fire-rate effect authored into chisato's override would be caught here.
    const CHI = (controlComp(SLUG, true) as any).slugs.indexOf(SLUG);
    const ownBuffs = (evs: Ev[], stat: string) =>
      buffs(evs, stat).filter((e) => (e as any).casterIdx === CHI);
    for (const stat of [
      'maxAmmoPct',
      'maxAmmoFlat',
      'reloadSpeedPct',
      ...
      expect(ownBuffs(baseEvents, stat)).toHaveLength(0);
    }
  });
