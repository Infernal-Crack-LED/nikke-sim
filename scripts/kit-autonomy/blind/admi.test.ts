...
function run(opts: ReturnType<typeof controlComp>) {
  const evs: SimEvent[] = [];
  const res = runComp({ ...opts, cfg: { ...(opts as any).cfg, onEvent: (e: SimEvent) => evs.push(e) } });
  return { res, evs };
}
...
const wrongBurstTarget = run({
  ...
  overrides: {
    admi: withPatchedOverride('admi', (ov) => {
      for (const b of ov.burst!.blocks) b.target = { kind: 'self' };
    }),
  },
});
...
describe('admi — skill1: charge damage multiplier, all allies, counted activation', () => {
  ...
  it('is a MULTIPLIER on base charge damage, not additive charge-damage points', () => {
    ...
    expect(BUFF(wrongChargeStat.evs, 'chargeDamagePct').length).toBeGreaterThan(0);
    expect(totals(wrongChargeStat.res)).not.toEqual(totals(base.res));
  });

  it('affects ALL allies, not just admi (>1 distinct buff target)', () => {
    const targets = new Set(BUFF(base.evs, 'chargeDamageMultPct').map((e) => e.targetSlug));
    ...
    const wrongTargets = new Set(
      BUFF(wrongChargeTarget.evs, 'chargeDamageMultPct').map((e) => e.targetSlug),
    );
...
describe('admi — burst: Reload Speed + Critical Damage, all allies, 10 sec', () => {
  ...
  it('both burst lines affect ALL allies, not just admi', () => {
    const rlTargets = new Set(BUFF(base.evs, 'reloadSpeedPct').map((e) => e.targetSlug));
    const cdTargets = new Set(BUFF(base.evs, 'critDamagePct').map((e) => e.targetSlug));
    ...
    const wrongCd = new Set(BUFF(wrongBurstTarget.evs, 'critDamagePct').map((e) => e.targetSlug));
  ...
  it('the 10 sec windows are time-bounded, not round-bounded or permanent', () => {
    for (const e of [...BUFF(base.evs, 'reloadSpeedPct'), ...BUFF(base.evs, 'critDamagePct')]) {
