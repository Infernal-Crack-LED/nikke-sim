...
const tOf = (ev: Ev): number =>
  ev.t ?? ev.sec ?? ev.time ?? (typeof ev.frame === 'number' ? ev.frame / 60 : NaN)
...
function run(patch?: Patch) {
  ...
  if (patch) {
    opts.overrides = { ...(opts.overrides ?? {}), [SLUG]: withPatchedOverride(SLUG, patch) }
  }
...
function tryRun(patch: Patch) {
  try {
    ...
    return { res: null as any, events: [] as Ev[], unit: null as any, idx: -1, err: String(e) }
  }
}
...
const buffsIn = (o: any, slot: string, stat: string) => {
  ...
  for (const b of o.blocks ?? []) {
    ...
    for (const e of b.effects ?? []) if (e.kind === 'buff' && e.stat === stat) out.push({ block: b, eff: e })
  }
  return out
}
const oneBuff = (o: any, slot: string, stat: string) => {
  const hits = buffsIn(o, slot, stat)
  if (hits.length !== 1) throw new Error(`expected exactly 1 ${slot}/${stat} buff, found ${hits.length}`)
  return hits[0]
}
const riderBlocks = (o: any) =>
  (o.blocks ?? []).filter((b: any) => b.slot === 'skill2' && (b.effects ?? []).some((e: any) => e.kind === 'flatDamage'))
...
const anisBursts = EV.filter((e) => e.kind === 'burstCast' && casterOf(e) === A)
const anisReloads = EV.filter((e) => e.kind === 'reload' && casterOf(e) === A)
const anisShots = EV.filter((e) => e.kind === 'shot' && casterOf(e) === A)
const anisDmg = EV.filter((e) => e.kind === 'damage' && (e.srcSlot ?? casterOf(e)) === A)
...
const applies = (events: Ev[], stat: string, value: number) =>
  events.filter((e) => e.kind === 'buffApply' && e.stat === stat && !isBossDebuff(e) && near(e.value ?? NaN, value))
const reloadsOf = (r: { events: Ev[]; idx: number }) => r.events.filter((e) => e.kind === 'reload' && casterOf(e) === r.idx)
...
describe('anis-sparkling-summer - blind kit spec', () => {
  ...
  it('S1 ATK grant reaches Electric Code allies ONLY - anis alone in this comp', () => {
    ...
    const moved = mates.some(
      (m) => JSON.stringify(unitOf(BASE.res, m)) !== JSON.stringify(unitOf(S1_ALL_ALLIES.res, m)),
    )
  ...
  it('S1 ATK grant runs 10 sec of wall clock', () => {
    const a = applies(EV, 'atkPct', S1_ATK)
    const rm = EV.filter(
      (e) => e.kind === 'buffRemove' && e.stat === 'atkPct' && !isBossDebuff(e) && near(e.value ?? NaN, S1_ATK),
    )
    expect(a.length).toBeGreaterThan(0)
  ...
  it('S2 382.42% rider fires once per MAGAZINE (lastBullet), not once per trigger pull', () => {
    ...
    const rShot = RIDER_ON_SHOT.events.filter(
      (e) => e.kind === 'damage' && (e.srcSlot ?? casterOf(e)) === RIDER_ON_SHOT.idx && bucketOf(e) !== 'normal',
    )
  ...
  it('S2 rider is crit-eligible at the caster sheet rate', () => {
    ...
    const normRate = normals.length ? (normals[0].critRate ?? normals[0].crit) : null
    expect(typeof normRate).toBe('number')
    expect(normRate as number).toBeGreaterThan(0)
    for (const r of riders) expect(near((r.critRate ?? r.crit ?? 0) as number, normRate as number, 0.001)).toBe(true)
  })
  ...
  it('burst Max Ammo -73.92% shrinks the magazine, so reloads (and 382.42% procs) spike inside her window', () => {
    ...
    for (const b of anisBursts) {
      ...
      outW += anisReloads.filter((r) => tOf(r) >= t0 + 12 && tOf(r) < t0 + 22).length
    }
    ...
    expect(reloadsOf(AMMO_UP).length).toBeLessThan(reloadsOf(NO_AMMO_CUT).length)
  })

  it('burst ammo cut also raises the rider count, closing the self-synergy loop', () => {
    const cutlessRiders = NO_AMMO_CUT.events.filter(
      (e) => e.kind === 'damage' && (e.srcSlot ?? casterOf(e)) === NO_AMMO_CUT.idx && bucketOf(e) !== 'normal',
    )
    expect(NO_AMMO_CUT.err).toBeNull()
  ...
  it('burst Elemental Advantage Damage +42.24% is applied but damage-INERT vs the Fire boss', () => {
    ...
    expect(JSON.stringify(totals(NO_ELEM.res))).toBe(JSON.stringify(totals(BASE.res)))
  })
