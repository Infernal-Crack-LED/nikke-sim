...
function run(overrides?: Record<string, unknown>): {
  ...
  const comp = controlComp(SLUG, true) as Comp;
  const events: Ev[] = [];
  comp.onEvent = (ev: SimEvent) => {
    events.push(ev as Ev);
  };
  if (overrides) comp.overrides = { ...(comp.overrides ?? {}), ...overrides };
...
describe('belorta burst — "Charge Speed \u25b2 2.82% for 10 sec" to all allies', () => {
  ...
  it('B2b: target set is ALL allies including self (5 distinct targets, belorta among them)', () => {
    // RED under "allies excludeSelf" (4 targets, no belorta) and under self-only (1 target).
    expect(csTargets.size).toBe(5);
  ...
  it('B2e: trigger identity is her OWN burst cast, not team full-burst entry', () => {
    // One 10 s window per cast, so distinct expiresFrame values == cast count. Belorta contests
    // stage 2 with crown, so a burstCast-keyed buff fires on a STRICT SUBSET of full bursts,
    // while a fullBurstEnter-keyed one fires on every single one.
    expect(csWindows.size).toBeGreaterThan(0);
    expect(csWindows.size).toBeLessThan(fbStarts);
  });
