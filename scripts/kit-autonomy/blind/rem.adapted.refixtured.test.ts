// REM RE-FIXTURED VARIANT — the verbatim blind test (rem.test.ts) uses controlComp('rem', true) = liter/crown/rem/helm,
// where crown (fixed B2 core) out-casts Rem for the shared B2 slot, leaving Rem vacuous (0 burst casts) and failing the
// blind test's OWN first non-vacuity canary. The blind test documents this exact failure as 're-fixture with a non-B2
// support, not an override defect'. This variant performs that sanctioned re-fixture (Rem sole B2; ada the single RL
// ally) to evaluate the SUBSTANTIVE assertions against the driver override. Import path already fixed (adapted).
...
// controlComp dropped from the import: the re-fixture below replaces controlComp('rem', true) with a
// custom comp literal (Rem sole B2), so the helper is no longer referenced in this variant.
import {
  runComp,
  totals,
  withPatchedOverride,
...
function fixture(remOverride?: unknown): Opts {
  // RE-FIXTURED (sanctioned by the blind test's own non-vacuity canary, which FAILED on controlComp('rem',true):
  // crown wins the shared B2 slot every rotation so Rem never casts). Swap to a comp where Rem is the SOLE B2
  // (liter B1 SMG / rem B2 MG / ada B3 RL) so her burst-keyed lines fire; ada is the single RL ally so the RL-scope
  // assertions still discriminate. ASSERTIONS BELOW ARE VERBATIM — only this fixture line changed.
  const opts = {
    slugs: ['liter', 'rem', 'ada'],
    bossElement: 'Fire',
    focusSlug: 'ada',
  } as unknown as Opts & { overrides?: Record<string, unknown> };
  if (!remOverride) {
    return opts;
  }
  return {
