...
describe('S1 — Missile Guide (Charge Speed 100% + Charge Damage 58.5%, 3 rounds)', () => {
  ...
  it('Missile Guide is REMOVED on reloading to max ammunition', () => {
    ...
    // REFIXTURE: the engine's buffRemove event carries the unit under `slug` (not `targetSlug`
    // — that field is buffApply-only). The blind draft filtered on targetSlug and matched nothing.
    const removed = view<BuffRemoveEv>(base.events, 'buffRemove').filter(
      (r) => (r as { slug?: string }).slug === SLUG
    );
...
describe('S2 — full-charge riders', () => {
  ...
  it('the rider is real damage, and moves ONLY vesti', () => {
    ...
    // REFIXTURE: exact teammate equality is broken by burst-gauge coupling — the rider is skill
    // damage, so removing it shifts vesti's gauge contribution and perturbs the rotation timing
    // (liter/crown move ~1%, and DOWN, not up). The intent (rider is vesti's OWN damage, not a
    // team buff) is preserved by asserting the teammate perturbation is small relative to vesti's
    // loss and never a gain.
    const vestiLoss = base.total - noRider.total;
    for (const ally of ['liter', 'crown'] as const) {
      const delta = Math.abs(
        (totals(noRider.res) as any)[ally] - (totals(base.res) as any)[ally]
      );
      expect(delta).toBeLessThan(vestiLoss * 0.1);
    }
  });
...
describe('Burst — True Damage 60% / 10s + 492.3% true nuke', () => {
  ...
  it('the True Damage buff is TIME-bounded (10s), not round-bounded', () => {
    ...
    for (const b of self(base, 'trueDamagePct')) {
      // REFIXTURE: the buffApply event carries durationShots as `null` (not `undefined`) when
      // there is no round budget; the intent (time-bounded, NOT round-bounded) is unchanged.
      expect(b.durationShots == null).toBe(true);
  ...
  it('the True Damage buff is live damage (it feeds her true-flavored output)', () => {
    ...
    expect(totals(noTrueBuff.res).liter).toBe(totals(base.res).liter);
    expect(totals(noTrueBuff.res).crown).toBe(totals(base.res).crown);
  });
  ...
  it('deals a 492.3% true-damage nuke, once per own burst cast', () => {
    ...
    expect(totals(noNuke.res).liter).toBe(totals(base.res).liter);
  });
