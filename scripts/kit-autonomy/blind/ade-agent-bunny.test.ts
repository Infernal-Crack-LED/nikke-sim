...
const teamDmg = (r: any): number => { const t: any = totals(r); return (t.total ?? t.damage ?? t.totalDamage) as number }
...
const PIERCE_OFF = runPatched(o => { o.hasPierce = false; if (o.skills) o.skills.hasPierce = false; return 1 })
...
const S1A_APPLIES = withStat('casterAtkPct', 15.2)
...
describe('S2b — at MAX Spy Lens: gains Pierce + ATK +16% continuously (self)', () => {
  ...
  it('Gains Pierce is carried as the unit-level tag, and removing it never moves teammates', () => {
    const tagged = Boolean(OV.hasPierce ?? OV.skills?.hasPierce)
