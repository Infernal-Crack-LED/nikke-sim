/**
 * DRIVER ADAPTATION (mechanical only — spec assertions preserved byte-identical):
 * A1. FIXTURE: the raw test assumes controlComp('snow-crane', true) gives her a real B2 cast.
 *     FALSE against the engine: controlComp fixes crown (Burst II, 20s CD) at stage II, and
 *     crown wins EVERY stage-II cast — snow-crane casts zero times there, so every burst-keyed
 *     assertion would read an empty set. Replaced with the driver's proven fixture:
 *     liter (B1) / folkwang (B2, forced BARE — her shipped override carries shields/heals) /
 *     snow-crane (B2) / asuka (B3, burst lifesteal patched out so snow-crane is the SOLE
 *     recovery source). Two 40s Burst IIs alternate casts (her cast pattern is real).
 * A2. recipients count 5 -> 4 (the adapted comp is 4 units; the raw "5 units" was also wrong
 *     for controlComp, which is 4).
 * A3. In the TWO heal-strip damage-inert assertions (skill2-a and burst-a), the recovery
 *     CONSUMER (asuka — her S1 converts recovery landings into an atkPct self-buff, the
 *     observer the fixture needs so heal cadence is event-visible at all) is excluded from the
 *     totals equality: stripping a heal channel moves HER damage BY DESIGN through the tandem
 *     channel the line exists for. All other units are still compared exactly. (The raw
 *     premise "no ally in the control comp converts recovery into damage" is false of
 *     controlComp too — crown's S2 is exactly such a consumer.)
 * A4. The skill2-a strip emptied the chargeCounter block's effects[], which the engine's
 *     chargeCounter dispatch (applyEffect(effects[phase])) rejects with a crash. Replaced with
 *     the semantically identical legal strip: remove the block whole.
 */
...
// ---- A1: adapted fixture (see header) --------------------------------------------------------
const scFixture = (): ReturnType<typeof controlComp> => ({
  slugs: ['liter', 'folkwang', 'snow-crane', 'asuka'],
  bossElement: 'Fire',
  focusSlug: 'asuka',
  overrides: {
    folkwang: { slug: 'folkwang', skill1: [], skill2: [], burst: [] } as any,
    asuka: withPatchedOverride('asuka', (ov) => {
...
describe('snow-crane skill1-a — Exclusive Recovery Agreement: Max HP +10% of the USER\'s Max HP, all allies, continuous', () => {
  it('emits one flat-resolved maxHpFlat grant per ally, all with the SAME value (caster-scaled)', () => {
    ...
    // "Affects all allies" — includes self; the adapted comp is 4 units (A2).
...
describe('snow-crane skill2-a — after 3 Full Charge attacks: heal all allies in Exclusive Recovery Agreement status', () => {
  ...
  it('is damage-inert on its own (no ally in the control comp converts recovery into damage)', () => {
    ...
    const stripped = run({
      ...
      overrides: {
        [SLUG]: withPatchedOverride(SLUG, (ov) => {
          // A4: the raw strip emptied the chargeCounter block's effects[] — an illegal shape
          // (the chargeCounter dispatch applies effects[phase]; empty crashes the sim). The
          // semantically identical, legal strip is removing the block whole.
          ov.skill2 = (ov.skill2 ?? []).filter(
            (b) => !(b.trigger?.kind === 'chargeCounter')
          );
        }),
      },
    });
    ...
    const b = totals(stripped.res);
    for (const slug of Object.keys(a)) {
      // A3: asuka is the intentional recovery CONSUMER — stripping the heal channel moves
      // her damage by design (she loses recovery-driven atkPct uptime); compare all others.
      if (slug === 'asuka') continue;
      expect(b[slug]).toBeCloseTo(a[slug], 6);
    }
  });
});
...
describe('snow-crane burst-a — recovers 44.68% of the user\'s final Max HP for all allies', () => {
  it('is an allies-targeted heal fired on her burst cast, and is damage-inert in the control comp', () => {
    ...
    const b = totals(stripped.res);
    for (const slug of Object.keys(a)) {
      // A3: asuka is the intentional recovery CONSUMER — stripping the burst heal moves her
      // damage by design (one fewer recovery landing per snow-crane cast); compare all others.
      if (slug === 'asuka') continue;
      expect(b[slug]).toBeCloseTo(a[slug], 6);
    }
  });
