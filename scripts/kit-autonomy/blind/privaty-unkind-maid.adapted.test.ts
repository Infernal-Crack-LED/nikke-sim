...
// ADAPTED: the unit result row carries .slug, not a slot/idx field — derive the caster/target
// index from the comp slot order (controlComp(SLUG,true) = liter 0 / crown 1 / carry 2 / helm 3).
const selfIdx = base.slugs.indexOf(SLUG);

const evs = BASE.events;
const dmg = evs.filter((e) => e.kind === 'damage');
const mine = dmg.filter((e) => (e as { slug?: string }).slug === SLUG);
const buffs = evs.filter((e) => e.kind === 'buffApply');
const myBuffs = buffs.filter(
  (e) => (e as { targetIdx?: number }).targetIdx === selfIdx
);
  ...
  it('the fixture actually bursts (she is B3; a lone B3 makes ZERO full bursts)', () => {
    ...
    expect(
      casts.length +
        evs.filter(
          (e) =>
            e.kind === 'burstCast' && (e as { slug?: string }).slug === SLUG
        ).length
    ).toBeGreaterThan(0);
  });
...
describe('skill1 — 30-pellet rider, 202.84% of final ATK', () => {
  ...
  it('removing skill1 entirely lowers ONLY her damage — teammates carry no DIRECT buff (inertness)', () => {
    ...
    // ADAPTED (reconciled): the blind draft demanded BYTE-identical teammate totals, but S1's
    // flatDamage rider emits burst gauge per proc (sim.ts skillGauge), so stripping it perturbs
    // Privaty's burst timing and therefore the team Full Burst rotation — a real ~0.01% second-order
    // shift on teammates, NOT a direct team buff. The claim under test is "S1 grants nothing DIRECT
    // to teammates", so assert teammate totals are unchanged to a tight relative tolerance that the
    // gauge-timing coupling stays inside but any real team buff (>=1%) would blow through.
    ...
    for (const slug of Object.keys(baseT)) {
      ...
      const rel = Math.abs(noT[slug] - baseT[slug]) / baseT[slug];
      expect(
        rel,
        `${slug} teammate moved ${(rel * 100).toFixed(3)}% on S1 removal — gauge-timing coupling is ~0.01%, a direct buff would be >=1%`
      ).toBeLessThan(0.005);
    }
  });
});
...
describe('skill2a — Reload Speed ▲20.88% for 2 sec on a >=5-pellet normal attack', () => {
  ...
  it('is SELF-scoped — no teammate ever receives her reload-speed buff', () => {
    ...
    const fromHer = rsAll.filter(
      (e) => (e as { casterIdx?: number }).casterIdx === selfIdx
    );
    for (const b of fromHer) {
      expect((b as { targetIdx?: number }).targetIdx).toBe(selfIdx);
    }
  });
});
...
describe('skill2b — in-Full-Burst 30-pellet: Reload 1 round + ATK ▲11.22% x5 for 2 sec', () => {
  ...
  it('the ATK buff is SELF only — teammates never receive it (inertness)', () => {
    const fromHer = buffs.filter(
      (e) =>
        (e as { casterIdx?: number }).casterIdx === selfIdx &&
        (e as { stat?: string }).stat === 'atkPct' &&
        Math.abs(((e as { value?: number }).value ?? 0) - 11.22) < 0.01
    );
    for (const b of fromHer) {
