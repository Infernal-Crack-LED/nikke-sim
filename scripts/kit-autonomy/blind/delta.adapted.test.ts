 ...
 * anis / ade-agent-bunny precedent):
 ...
 *   3. FIXTURE FIX — the B2 starvation trap (anis precedent): controlComp('delta', true) seats
 *      crown, a second Burst II with a 20s cd, and same-stage selection hands crown EVERY
 *      stage-2 slot (MEASURED on the driver override: 0 delta casts vs 10 crown casts / 5 FBs
 *      in 180s — worse than anis because delta's cd is 40s). Swapped to the sole-B2 comp
 *      ['liter','delta','helm','ada']: liter (B1 20s) opens the chain, delta is the ONLY B2
 *      (casts every FB), helm + ada (both B3 40s) alternate the stage-3 slot. Boss Fire,
 *      focus delta — the same basis variables controlComp used. Consequence, recorded not
 *      hidden: with no co-B2, DELTA_CASTS === FB_STARTS, so the skill2 divergence test takes
 *      the pristine file's own else-branch ("delta took every rotation: the two readings
 *      coincide here and the fixture cannot separate them") — the driver's own spec covers the
 *      divergence with a separate STARVED comp (crown co-seated, delta 0 casts).
 *
 * Written BLIND from the kit prose alone (no sight of the shipped override, no sight of any
 * other author's tests). The pristine header follows.
 *
...
function runDelta(patched?: Rec): Run {
  const events: Ev[] = [];
  // BANNER FIX 3: sole-B2 fixture (controlComp seats crown, a co-B2 that starves delta to
  // 0 casts — measured on the driver override; anis precedent).
  const opts = {
