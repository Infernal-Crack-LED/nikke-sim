...
// blind/mica.test.ts. Structural corrections to blind-writer assumptions that were
...
//   2. the ammo LOAD-BEARING assertion: expected a net recipient-sum INCREASE; against the
//      driver override the grant is live but the magazine extension re-phases the shared
//      Full Burst chain (burst-gauge accrual shift), costing the helm anchor slightly more
//      than liter gains (net −0.008% across recipients {helm, liter}). Adapted to a
//      movement + one-sided-lift assertion (the non-inertia substance); full rationale
//      inline at the assertion, clean all-positive form pinned in the driver spec M4.
/**
...
describe('mica — skill2: "Affects 2 allies with the highest final ATK"', () => {
  ...
  it('the ammo grant is LOAD-BEARING: stripping it MOVES the recipients\u2019 damage', () => {
    ...
    // ADAPTED (driver reconciliation, 2026-08-05): the blind assertion expected a NET
    // recipient-sum INCREASE; against the driver override the grant is live — recipients
    // {helm, liter} read +1.01M / −1.06M — but the magazine extension also shifts burst-
    // gauge accrual, re-phasing the whole Full Burst chain from cast #3 (mica's cast frames
    // drift up to ~150f), and that timing shift costs helm (the 474M anchor) slightly more
    // than liter's extension gains: net −45k (0.008%) across the two recipients. The
    // load-bearing substance — the grant moves recipient damage, it is NOT inert — is
    // preserved here as a movement + one-sided-lift assertion; the clean all-positive form
    // of this check is pinned in the driver spec (scripts/tests/units/mica.test.ts M4,
    // admi fixture, where the recipients are the two B3 carries and BOTH totals rise).
    ...
    expect(sum(ctrlReal.res)).not.toEqual(sum(ctrlNoAmmo.res));
    const tReal = totals(ctrlReal.res);
    const tNoAmmo = totals(ctrlNoAmmo.res);
    expect(
      recipients.some((s) => tReal[s] > tNoAmmo[s]),
      'at least one recipient must gain firing uptime from the extension'
    ).toBe(true);
  });
