...
function run(opts: ReturnType<typeof controlComp>) {
  const events: Ev[] = [];
  const o = { ...opts, cfg: { ...(opts as any).cfg, onEvent: (ev: SimEvent) => events.push(ev as Ev) } };
...
const evOf = (evs: Ev[], kind: string) => evs.filter((e) => (e as any).kind === kind);
...
const chisatoDmg = (evs: Ev[]) =>
  dmg(evs).filter((e) => (e as any).slug === SLUG || (e as any).srcSlug === SLUG);
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.skill1!.blocks) {
          b.effects = b.effects.filter(
            (e: any) => !(e.kind === 'buff' && e.stat === 'atkPct' && Math.abs(e.value - 53.69) < 0.01),
          );
        }
        ov.skill1!.blocks = ov.skill1!.blocks.filter((b: any) => b.effects.length > 0);
      }),
    };
    return o;
  })(),
    ...
    o.overrides = {
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.skill1!.blocks) {
          b.effects = b.effects.filter(
            (e: any) =>
              !(e.kind === 'buff' && e.stat === 'trueDamagePct' && Math.abs(e.value - 48.62) < 0.01),
          );
        }
        ov.skill1!.blocks = ov.skill1!.blocks.filter((b: any) => b.effects.length > 0);
      }),
  ...
  ((): any => {
    ...
    o.overrides = {
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.skill1!.blocks) {
          b.effects = b.effects.filter(
            (e: any) => !(e.kind === 'buff' && e.stat === 'hitRatePct' && Math.abs(e.value - 22.37) < 0.01),
          );
        }
        ov.skill1!.blocks = ov.skill1!.blocks.filter((b: any) => b.effects.length > 0);
      }),
    ...
    o.overrides = {
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.skill2!.blocks) {
          if ((b.trigger as any).kind === 'hitCount') (b.trigger as any).count = 24;
        }
      }),
    };
    return o;
  ...
  ((): any => {
    ...
    o.overrides = {
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.skill2!.blocks) {
          b.effects = b.effects.filter(
            (e: any) => !(e.kind === 'flatDamage' && Math.abs(e.atkPct - 472.18) < 0.01),
          );
        }
        ov.skill2!.blocks = ov.skill2!.blocks.filter((b: any) => b.effects.length > 0);
      }),
    };
    return o;
  })(),
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.burst!.blocks) {
          const hasAtk = b.effects.some(
            (e: any) => e.kind === 'buff' && e.stat === 'atkPct' && Math.abs(e.value - 73.16) < 0.01,
          );
      ...
      [SLUG]: withPatchedOverride(SLUG, (ov) => {
        for (const b of ov.burst!.blocks) {
          for (const e of b.effects as any[]) {
            if (e.kind === 'buff' && e.stat === 'atkPct' && Math.abs(e.value - 73.16) < 0.01) {
...
describe('chisato — fixture non-vacuity', () => {
  it('the control comp actually casts chisato\u2019s burst and enters Full Burst', () => {
    ...
    const casts = evOf(baseEvents, 'burstCast').filter((e) => (e as any).slug === SLUG);
...
describe('chisato skill1 \u2014 "Charges Extrasensory to 100%" at battle start', () => {
  it('the tiered buffs are live from the OPENING of the fight, not ramped in', () => {
    ...
    const firstFrame = Math.min(...atkTier.map((e) => ((e as any).frame ?? 0) as number));
    ...
    for (const stat of ['atkPct', 'trueDamagePct', 'hitRatePct'] as const) {
      const foreign = buffs(baseEvents, stat).filter(
        (e) =>
          ...
          [53.69, 48.62, 22.37].some((v) => Math.abs((e as any).value - v) < 0.01),
      );
    ...
    const noRecharge = run(
      ((): any => {
        ...
        o.overrides = {
          ...
          [SLUG]: withPatchedOverride(SLUG, (ov) => {
            for (const b of ov.burst!.blocks) {
              b.effects = (b.effects as any[]).filter((e) => e.kind !== 'resource');
            }
            ov.burst!.blocks = ov.burst!.blocks.filter((b: any) => b.effects.length > 0);
          }),
        };
        return o;
      })(),
...
describe('chisato skill2 \u2014 "Normal attacks deal true damage for 10 sec" on burst', () => {
  it('fires on chisato\u2019s OWN burst cast, not on any team Full Burst', () => {
    ...
    const casts = evOf(baseEvents, 'burstCast').filter((e) => (e as any).slug === SLUG).length;
...
describe('chisato skill2 \u2014 "after landing 48 normal attack(s)" \u2192 472.18% true', () => {
  it('fires as a repeating hit-count rider, multiple times over 180s', () => {
    ...
    const riders = chisatoDmg(baseEvents).filter(
      (e) => Math.abs(((e as any).mult ?? 0) - 472.18) < 0.5 || (e as any).srcSlot === 'skill2',
    );
  ...
  it('the rider hits the TARGET, not an ally (no ally damage attributed to it)', () => {
    const foreign = dmg(baseEvents).filter(
      (e) => (e as any).srcSlug === SLUG && (e as any).slug && (e as any).slug !== SLUG,
    );
    expect(foreign).toHaveLength(0);
  });
});
...
describe('chisato burst \u2014 "Charges Extrasensory to 100%" + "ATK \u25b2 73.16% for 10 sec"', () => {
  ...
  it('the burst ATK buff is keyed to chisato\u2019s OWN cast, not team FB entry', () => {
    ...
    expect(totals(burstAtkOnFbEnter.res)[SLUG]).toBeGreaterThanOrEqual(baseTotals[SLUG]);
  });
  ...
  it('the buff EXPIRES \u2014 making it permanent strictly raises damage', () => {
    ...
    expect(totals(burstAtkPermanent.res)[SLUG]).toBeGreaterThan(baseTotals[SLUG]);
  });
  ...
  it('no buffRemove is asserted for the 10s window (engine emits none on lapse)', () => {
    const removes = evOf(baseEvents, 'buffRemove').filter(
      (e) => (e as any).targetSlug === SLUG && Math.abs((e as any).value - 73.16) < 0.01,
    );
  ...
  it('the burst recharge tops Extrasensory back to 100 (tiers survive the fight)', () => {
    ...
    const stillBuffed =
      late.length > 0 ||
      selfBuffs(baseEvents, 'atkPct').some((x) => Math.abs((x as any).value - 53.69) < 0.01);
...
describe('chisato \u2014 global inertness', () => {
  ...
  it('no weapon-state modifiers: the kit changes no ammo / reload / fire rate', () => {
    ...
    for (const stat of ['maxAmmoPct', 'maxAmmoFlat', 'reloadSpeedPct', 'fireRatePct', 'attackSpeedPct'] as const) {
