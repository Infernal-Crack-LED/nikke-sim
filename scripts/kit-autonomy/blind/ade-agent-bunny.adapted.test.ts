...
function baseOpts(onEvent?: (ev: Ev) => void): any {
  const o: any = { slugs: ['liter', 'ade-agent-bunny', 'ada', 'helm'], bossElement: 'Fire', focusSlug: 'ade-agent-bunny' }
  if (onEvent) o.cfg = { ...(o.cfg ?? {}), onEvent }
  return o
}
function runPatched(mutate: (o: any) => number): { res: any; hits: number } {
  let hits = 0
  const patched = withPatchedOverride(SLUG, (o: any) => { hits = mutate(o) })
  const o = baseOpts()
...
function readOverride(): any {
  let cap: any = null
  withPatchedOverride(SLUG, (o: any) => { cap = o })
  return cap
}
const teamDmg = (r: any): number => { const t: any = totals(r); return Object.values(t).reduce((a: number, b: any) => a + (b as number), 0) } // harness totals() is a per-slug map; sum it
const uDmg = (r: any, slug: string): number => { const u: any = unitOf(r, slug); return (u.damage ?? u.total ?? u.totalDamage) as number }
// ------------------------------------------------------------------------------

const blocksOf = (o: any): any[] => [...(o.skill1 ?? []), ...(o.skill2 ?? []), ...(o.burst ?? []), ...(o.blocks ?? o.skills?.blocks ?? [])]
function findBuffs(o: any, stat: string, value?: number): any[] {
  const out: any[] = []
  for (const b of blocksOf(o)) {
    for (const e of (b.effects ?? [])) {
      if (e.kind === 'buff' && e.stat === stat && (value === undefined || near(e.value, value))) out.push({ b, e })
    }
  }
  return out
}
...
const S1A_ZERO = runPatched(o => { const f = findBuffs(o, 'casterAtkPct', 15.2); f.forEach(x => { x.e.value = 0 }); return f.length })
const S1A_OWNATK = runPatched(o => { const f = findBuffs(o, 'casterAtkPct', 15.2); f.forEach(x => { x.e.stat = 'atkPct' }); return f.length })
const S1A_SELF = runPatched(o => { const f = findBuffs(o, 'casterAtkPct', 15.2); f.forEach(x => { x.b.target = { kind: 'self' } }); return f.length })

// S2b nearest-wrongs: dead / ungated-from-t0
const S2B_ZERO = runPatched(o => { const f = findBuffs(o, 'atkPct', 16); f.forEach(x => { x.e.value = 0 }); return f.length })
const S2B_UNGATED = runPatched(o => {
  const f = findBuffs(o, 'atkPct', 16)
  f.forEach(x => {
    delete x.b.resourceGate; delete x.b.everyN; delete x.b.everyNOffset; delete x.b.fbGate; delete x.b.requiresCore
    delete x.e.rampSec
...
const PIERCE_ZERO = runPatched(o => { const f = findBuffs(o, 'pierceDamagePct'); f.forEach(x => { x.e.value = 0 }); return f.length })
const PIERCE_OFF = runPatched(o => { o.hasPierce = false; if (o.skills) o.skills.hasPierce = false; for (const blk of blocksOf(o)) blk.effects = (blk.effects ?? []).filter((e: any) => e.kind !== 'gainPierce'); return 1 })

// Burst nearest-wrongs: dead / authored-permanent
const BURST_ZERO = runPatched(o => { const f = findBuffs(o, 'attackDamagePct', 55.04); f.forEach(x => { x.e.value = 0 }); return f.length })
const BURST_LONG = runPatched(o => { const f = findBuffs(o, 'attackDamagePct', 55.04); f.forEach(x => { x.e.durationSec = 60 }); return f.length })

// ---- event-log derivations from the BASE run ---------------------------------
const APPLIES = EV.filter(e => e.kind === 'buffApply' && e.targetIdx !== null && e.targetIdx !== undefined)
const withStat = (stat: string, value: number) => APPLIES.filter(e => e.stat === stat && near(e.value, value))
...
const ADE_CASTS = EV.filter(e => e.kind === 'burstCast' && idxOf(e) === ADE_IDX)
...
const S2B_APPLIES = APPLIES.filter(e => e.stat === 'atkPct' && near(e.value, 16))
...
describe('ade-agent-bunny — fixture non-vacuity', () => {
  ...
  it('no silent drops: the unmodelable Minimum Effective Range lines are recorded, not dropped', () => {
    const audit = JSON.stringify({ unmodeled: OV.unmodeled ?? OV.skills?.unmodeled ?? null, note: OV.note ?? null })
...
describe('S1a — allies ATK +15.2% OF THE SKILL USER ATK, 5s, per full charge', () => {
  ...
  it('trigger is per-full-charge, never a burst/FB key (trigger identity)', () => {
    ...
    expect(['burstCast', 'fullBurstEnter', 'fullBurstEnd', 'passive', 'interval']).not.toContain(b.trigger.kind)
  })
  ...
  it('caster-sourced vs own-ATK is not cosmetic: swapping to atkPct moves the team total', () => {
    ...
    expect(Math.abs(teamDmg(S1A_OWNATK.res) - teamDmg(BASE))).toBeGreaterThan(0)
  })
...
describe('S2a — allies Pierce Damage +18.36%, 5s, per full charge', () => {
  it('is authored as pierceDamagePct at 5s on all allies (stat + duration + target set)', () => {
    ...
    expect(['shotFired', 'hitCount', 'chargeCounter']).toContain(f[0].b.trigger.kind)
  ...
  it('reaches every ally at runtime, not just self', () => {
    ...
    expect(new Set(S2A_APPLIES.map(e => e.targetIdx)).size).toBeGreaterThanOrEqual(4)
  })
...
describe('S2b — at MAX Spy Lens: gains Pierce + ATK +16% continuously (self)', () => {
  ...
  it('the max-Spy-Lens condition is modeled by SOME gate, not shipped live from t=0', () => {
    ...
    const gated = Boolean(b.resourceGate) || typeof e.rampSec === 'number' || b.trigger.kind !== 'passive'
  ...
  it('Gains Pierce is carried as the unit-level tag, and removing it never moves teammates', () => {
    const tagged = Boolean(OV.hasPierce ?? OV.skills?.hasPierce) || JSON.stringify(OV).includes('"gainPierce"')
...
describe('burst — allies Attack Damage +55.04% / Pierce Damage +10.13%, 10s', () => {
  ...
  it('fires exactly once per ADE burst cast, never once per team Full Burst', () => {
    ...
    expect(new Set(BB_APPLIES.map(e => e.targetIdx)).size).toBeGreaterThanOrEqual(4)
