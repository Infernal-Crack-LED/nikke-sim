...
const refs: string[] = [];
const htUnits = Object.entries(ht.units).map(([slug, u]: [string, any]) => ({
  slug,
  ...u,
}));
...
console.log(
  `REFERENCE-GRADE (>=${MIN_TEAMS} teams within +/-3%): ${refs.length ? refs.join(', ') : '(none yet)'}`
);
console.log('\nclosest tuned units (teams / within +/-3%):');
for (const u of htUnits
  .filter((x: any) => x.tuned)
  .sort(
    (a: any, b: any) =>
