...
import type {
  ...
  Piece,
  Line,
} from '../src/overload/model.js';
import { ALL_KEYS } from '../src/overload/model.js';
import {
  monteCarlo,
  monteCarloBuild,
  type McSummary,
} from '../src/overload/policy.js';
...
function parsePiece(spec: string): Target {
  ...
  for (const part of spec.split(',')) {
    ...
    if (t.some((r) => r.key === key)) {
      console.error(`duplicate key ${key} on one piece`);
      process.exit(1);
    }
...
const label = (t: Target) =>
  t.map((r) => `${r.key}≥T${r.minTier}`).join(' + ') || '(empty)';
...
if (hasFlag('--build')) {
  ...
  if (!targets.length || targets.length > 4) {
    console.error('give 1-4 piece specs');
    process.exit(1);
  }
  const { perPiece, total } = monteCarloBuild(model, targets, {
    trials: trialsB,
    seed: seedB,
  });
  console.log(
...
for (const a of argv) {
  ...
  if (target.some((r) => r.key === key)) {
    console.error(
      `duplicate line key ${key} — a piece can't hold the same stat twice`
    );
    process.exit(1);
  }
...
if (!target.length) {
  console.error(
    'usage: npx tsx scripts/ol-sim.ts <key:minTier> [key:minTier ...] [--trials N] [--seed S] [--fresh]'
  );
  process.exit(1);
}
if (target.length > 3) {
  console.error('a piece has at most 3 lines');
  process.exit(1);
}
...
const r = monteCarlo(model, target, {
  trials,
  seed,
  fresh: start ? false : fresh,
  start,
  cap,
});
...
console.log(
  `    phase 1 (stat rerolls) ${r.phase1Rerolls.mean.toFixed(2)}   +   phase 2 (value resets) ${r.phase2Resets.mean.toFixed(2)}`
);
console.log(
  `    median ${r.ops.pctiles.p50}   p80 ${r.ops.pctiles.p80}   p90 ${r.ops.pctiles.p90}   p95 ${r.ops.pctiles.p95}   p99 ${r.ops.pctiles.p99}`
);

console.log(`\n  Expected cost:`);
console.log(
  `    permanent locks : ${r.moduleCostPerm.mean.toFixed(1)} modules`
);
console.log(
  `    temp locks      : ${r.moduleCostTemp.mean.toFixed(1)} modules + ${r.tempLocks.mean.toFixed(0)} temp-locks`
);
...
for (const h of r.histogram) {
  ...
  const range = h.overflow
    ? `${h.lo}+`
    : h.lo === h.hi
      ? `${h.lo}`
      : `${h.lo}-${h.hi}`;
  console.log(
    `    ${range.padStart(11)} │ ${bar} ${((h.count / trials) * 100).toFixed(1)}%`
  );
}
...
if (r.analytic) {
  ...
  console.log(
    `    P(key present) ${(a.pKeyPresent * 100).toFixed(2)}%   P(tier≥) ${(a.pTierGE * 100).toFixed(2)}%`
  );
  console.log(
    `    expected ops  analytic ${a.expectedOps.toFixed(2)} (P1 ${a.expectedP1.toFixed(2)} + P2 ${a.expectedP2.toFixed(2)})   vs   MC ${r.ops.mean.toFixed(2)}`
  );
  console.log(`    (${a.note})`);
}
console.log();
