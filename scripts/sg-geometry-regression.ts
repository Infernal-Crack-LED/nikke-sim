...
console.log(
  '== §2 scale → circle px (3 independent bloom-peak points, proportional, residuals ≤0.7px) =='
);
...
close(
  ...
  28.0,
  0.01,
  'rangeFromCoreDpx is the exact inverse of coreDpx'
);
...
close(
  ...
  BAND_CORE_PX.mid,
  1,
  'AR HR-shrink floor 29px ≈ mid core 28px (independent, ≤1px)'
);
close(
  coreFracGeo(BAND_CORE_PX.mid, 48),
  0.34,
  0.03,
  'base AR core fraction ≈ 0.34 at the 48px accuracy circle'
);
ok(
  coreFracGeo(BAND_CORE_PX.mid, AR_INNER_BOUND_PX) > 0.9,
  'core fraction → ~1 as HR shrinks the bloom to the 29px floor'
);
...
ok(
  coreFracGeo(10, 0) === 1,
  'zero circle (dead zone) → fully covered (guarded)'
);
...
const floor = 1 - s * 100;
ok(
  circleDpxAtHr(75, 0, s, floor) === circleDpx(75),
  'hr=0 leaves the circle unchanged'
);
ok(
  circleDpxAtHr(75, 50, s, floor) < circleDpx(75),
  'positive hr shrinks the circle'
);
ok(
  circleDpxAtHr(75, 1e6, s, floor) >= circleDpx(75) * floor - 1e-9,
  'shrink is floored (never collapses to 0)'
);
...
ok(
  pelletSigma(250, 40, SLOPE, FLOOR) < pelletSigma(250, 0, SLOPE, FLOOR),
  'higher HR shrinks σ'
);
// SG landing reproduces the measured recon shape (spec §3 table), σ_SG at hr=0
const sigSG = pelletSigma(250, 0, SLOPE, FLOOR);
close(
  pelletLandFrac(BAND_SG_HIT_FRAC.near, sigSG),
  0.922,
  0.01,
  'SG landing near ≈ 0.922 (recon 0.888)'
);
close(
  pelletLandFrac(BAND_SG_HIT_FRAC.far, sigSG),
  0.775,
  0.01,
  'SG landing far ≈ 0.775 (recon 0.740)'
);
ok(
  pelletLandFrac(BAND_SG_HIT_FRAC.near, sigSG) >
    pelletLandFrac(BAND_SG_HIT_FRAC.far, sigSG),
...
close(
  pelletCoreFrac(BAND_CORE_PX.near, pelletSigma(75, 0, SLOPE, FLOOR)),
  0.4,
  0.01,
  'AR near core ≈ 0.400 (measured 0.40)'
);
close(
  pelletCoreFrac(BAND_CORE_PX.near, pelletSigma(110, 0, SLOPE, FLOOR)),
  0.25,
  0.02,
  'SMG near core ≈ 0.25 (measured 0.28)'
);
close(
  pelletCoreFrac(BAND_CORE_PX.near, sigSG),
  0.061,
  0.02,
  'SG near core ≈ 0.06 (measured 0.048)'
);
