...
function parseTrigger(header: string, slot: SkillSlot): TriggerDef {
  ...
  if (bossElem) {
    return {
      kind: 'bossElement',
      element: bossElem[1][0].toUpperCase() + bossElem[1].slice(1),
    };
  }
  if (
    /normal attack hits|attack hits|when performing a normal attack/.test(
      clause
    )
  ) {
...
function parseTarget(header: string): { target: TargetDef; warn?: string } {
  ...
  if (lowAtk) {
    const burst = lowAtk[2]
      ? (['I', 'II', 'III'][Number(lowAtk[2]) - 1] as any)
      : undefined;
    return {
      target: {
        kind: 'alliesLowestAtk',
        count: Number(lowAtk[1]),
        ...(burst ? { burst } : {}),
      },
    };
  }
...
function parseEffectLine(line: string): EffectDef | null {
  ...
  if (dmg) {
    const flavor = /as true damage/i.test(l)
      ? 'true'
      : /as distributed damage/i.test(l)
        ? 'distributed'
        : /as sustained damage/i.test(l)
          ? 'sustained'
          : undefined;
    ...
    return flavor
      ? { kind: 'flatDamage', atkPct: Number(dmg[1]), flavor }
      : { kind: 'flatDamage', atkPct: Number(dmg[1]) };
  }
  ...
  const fbExtend = l.match(
    /full burst time(?: duration)?\s*([▲▼])\s*([\d.]+) sec/i
  );
  if (fbExtend) {
    return {
      kind: 'fullBurstExtend',
      seconds: Number(fbExtend[2]) * (fbExtend[1] === '▼' ? -1 : 1),
    };
  }
  ...
  if (buff) {
    ...
    const value = Number(valueStr) * (dir === '▼' ? -1 : 1);
  ...
  for (const rawBlock of cleaned
    ...
    if (escalating) {
      ...
      for (const line of effectLines) {
        ...
        if (swap) {
          const kv = line.match(
            /^damage:\s*([\d.]+)% of (?:the )?(?:final )?atk/i
          )
            ? 'damagePct'
            : line.match(/^full charge damage:\s*([\d.]+)%/i)
              ? 'chargeMultPct'
              : line.match(/^charge time:\s*([\d.]+) sec/i)
                ? 'chargeTimeSec'
                : line.match(/^max ammunition capacity:\s*([\d.]+)/i)
                  ? 'maxAmmo'
                  : line.match(/^duration:\s*([\d.]+) sec/i)
                    ? 'durationSec'
                    : null;
          if (kv) {
          ...
          if (
            /^(snipe mode|additional effects?|reload|(number of )?pellet ?count|number of pellets)/i.test(
              line
            )
          ) {
    ...
    if (trigger.kind === 'unsupported' && real.length) {
      warnings.push(
        `${slot}: unsupported trigger "${trigger.raw}" — its effects are skipped`
      );
      // the whole block is inert at runtime — every line of it is unmodeled
      unmodeled.push(
        header,
        ...effectLines.filter((l) => !BOILERPLATE.test(l))
      );
    } else {
