...
ok(
  'line gates 1 / 0.5 / 0.3',
  model.lineCountGates.slot1 === 1 &&
    model.lineCountGates.slot2 === 0.5 &&
    model.lineCountGates.slot3 === 0.3
);
...
const pT11 = model.tierBands.bands
  .filter((b) => b.hi >= 11)
  .reduce(
    (s, b) => s + b.p * ((b.hi - Math.max(11, b.lo) + 1) / (b.hi - b.lo + 1)),
    0
  );
ok(
  'P(T11+) per line = 5%',
  near(pT11, 0.05, 1e-9),
  `got ${(pT11 * 100).toFixed(3)}%`
);
const pAll3 =
...
ok(
  'P(all 3 lines in one roll) = 15%',
  near(pAll3, 0.15, 1e-9),
  `got ${(pAll3 * 100).toFixed(1)}%`
);
...
const mc = monteCarlo(model, single, {
  trials: 20000,
  seed: 12345,
  fresh: false,
});
...
const t1 = monteCarlo(model, [{ key: 'atk', minTier: 1 }], {
  trials: 8000,
  seed: 3,
}).ops.mean;
const t11 = monteCarlo(model, [{ key: 'atk', minTier: 11 }], {
  trials: 8000,
  seed: 3,
}).ops.mean;
ok(
  'higher tier target costs more (T11 > T1)',
  t11 > t1,
  `T11 ${t11.toFixed(1)} vs T1 ${t1.toFixed(1)}`
);
const two = monteCarlo(
  model,
  [
    { key: 'elem', minTier: 11 },
    { key: 'atk', minTier: 11 },
...
ok(
  'more target lines cost more (2-line > 1-line)',
  two > t11,
  `2-line ${two.toFixed(1)} vs 1-line ${t11.toFixed(1)}`
);

if (fails) {
  console.log(`\noverload regression: ${fails} FAILED`);
  process.exit(1);
}
