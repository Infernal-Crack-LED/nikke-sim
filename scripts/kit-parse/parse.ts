...
function extract(slug: string) {
  ...
  console.log(
    [
      ...
      ``,
      `=== SKILL 1 ===`,
      String(s.skill1 ?? '').trim(),
      ``,
      `=== SKILL 2 ===`,
      String(s.skill2 ?? '').trim(),
      ``,
      `=== BURST ===`,
      String(s.burst ?? '').trim(),
    ].join('\n')
...
function selfcheck(slug: string) {
  ...
  console.log(
    `selfcheck ${slug} in ${team.name} (neutral)  teamFB=${r.fullBursts}`
  );
  if (!u) {
    console.log(`  (${slug} not in reference team — inspect via DBG only)`);
    return;
  }
  const d = u.breakdown;
  console.log(
    `  ${slug}: total=${(u.totalDamage / 1e6).toFixed(1)}M  n=${(d.normal / 1e6).toFixed(1)}M s=${(d.skill / 1e6).toFixed(1)}M b=${(d.burst / 1e6).toFixed(1)}M  pulls=${u.pulls} burstCasts=${u.burstCasts}`
  );
  console.log(
    `  (re-run with DBG_UNIT=${slug} DBG_BUFFS=1 DBG_N=100000 to confirm each block fires over the whole fight)`
  );
}
