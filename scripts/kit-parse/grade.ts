...
const CASES: Case[] = [
  ...
  },
  {
    ...
    team: {
      name: 'jill-control',
      slugs: ['little-mermaid', 'crown', 'jill', 'helm'],
    },
    boss: null,
    realFb: 11,
    real: {
      'little-mermaid': 407_969_363,
      crown: 215_628_547,
      jill: 573_110_084,
      helm: 447_479_339,
    },
...
function grade(slug: string, candFile: string) {
  ...
  console.log(
    `[rotation gate] all-real FB=${A.fb}  candidate FB=${B.fb}  ${rotOk ? 'PASS' : 'FAIL (rotation broke)'}`
  );
  console.log(
    `[whole-team sim-vs-sim] candidate/all-real, PASS if within ±5%:`
  );
  let allPass = rotOk;
...
if (!slug || !cand) {
  console.error('usage: grade.ts <slug> <candFile>');
  process.exit(1);
}
grade(slug, cand);
