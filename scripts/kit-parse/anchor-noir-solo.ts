...
const prepared = [
  {
    skills: resolveSkills(c, loadOverride('noir')),
    extraStats: [] as any[],
    loadout: [] as any[],
  },
];
...
console.log(
  `vs reference ${REFERENCE}: ${drift >= 0 ? '+' : ''}${drift.toFixed(4)}%`
);
if (Math.abs(drift) > 0.5) {
  console.error(
    'ANCHOR GUARD FAILED (>0.5%) — pause and escalate to owner before promoting'
  );
  process.exit(1);
}
