...
const readings = JSON.parse(
  readFileSync(
    new URL('../docs/probe-data/clean-weapons-readings.json', import.meta.url),
    'utf8'
  )
...
const arm = Number(process.env.SMGRATE)
  ? `SMGRATE=${process.env.SMGRATE} (revert)`
  : 'default (SMG 20/s, frame-quantized)';
console.log(
...
console.log(
  '  unit           wpn   n        sim        real   sim/real          spread'
);
...
for (const slug of order) {
  ...
  if (!obs?.length) {
    console.log(
      `  ${slug.padEnd(14)} ${c.weapon.padEnd(4)}  —   ${fmtM(sim[slug]).padStart(9)}      (no reading yet)`
    );
    continue;
  }
  ...
  const spread =
    obs.length > 1
      ? `±${(((Math.max(...obs) - Math.min(...obs)) / 2 / mean(obs)) * 100).toFixed(1)}%`
      : '—';
