...
if (flags.times && flags.times !== 'true') {
  ...
  console.log(
    `wrote ${written.length} frames to ${out}  (region=${flags.region ?? 'full'}, zoom=${zoom}): ${times.join(', ')}`
  );
  process.exit(0);
}
...
console.log(
  `wrote ${outfile.replace('%03d', '001..')}  (region=${flags.region ?? 'full'}, fps=${fps || 'n/a'}, dur=${dur || 'n/a'})`
);
