...
if (!focus) {
  console.error(
    'usage: hit-values.ts <focus> <slot1..slotN> [--boss <Element>] [--focus <slug>]'
  );
  process.exit(1);
}
...
console.log(
  `base crit +${(critBonus * 100).toFixed(0)}% (x${(1 + critBonus).toFixed(3)} on major=1) · base core +${(coreBonus * 100).toFixed(0)}% · values are NON-crit NON-core, per single instance\n`
);
console.log(
  'hit type'.padEnd(16) +
    'coef%'.padStart(8) +
    '  n' +
    'base min'.padStart(13) +
    'base max'.padStart(13) +
    '  crit(min–max)'.padStart(24)
);
for (const b of table.bands) {
  console.log(
    b.cat.padEnd(16) +
      b.coefPct.toFixed(1).padStart(8) +
      `  ${String(b.n).padStart(3)}` +
      b.baseLo.toLocaleString().padStart(13) +
      b.baseHi.toLocaleString().padStart(13) +
...
console.log(
  'NOTE: heals are NOT here (they pop over the CHARACTER, not the crosshair). Core popups = base x' +
