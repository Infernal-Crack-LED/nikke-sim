...
if (!slug) {
  console.error(
    'usage: npx tsx scripts/probe/dot-crit.ts <parsed-slug> [kind] [expectedNonCrit]'
  );
  process.exit(1);
}
...
if (!hits.length) {
  console.log('no popups of that kind — nothing to test.');
  process.exit(0);
}

const RATIO_LO = 1.4,
  RATIO_HI = 1.6; // crit band around ×1.5 (base +50% crit dmg)
...
if (flagged.length) {
  ...
  console.log(
    `[flags] ${flagged.length}/${hits.length} popups colour-classified: ` +
  ...
  const nonCritVals = flagged
    .filter((h) => !h.crit)
    .map((h) => h.value)
    .sort((a, b) => a - b);
  const modal = nonCritVals.length
    ? nonCritVals[Math.floor(nonCritVals.length / 2)]
    : undefined;
  if (modal && crits) {
    const critMean =
      flagged.filter((h) => h.crit).reduce((s, h) => s + h.value, 0) / crits;
    console.log(
      `[flags] median non-crit ${modal.toLocaleString()}, mean crit ${Math.round(critMean).toLocaleString()} ` +
...
const critBand = hits.filter(
  (h) => h.value >= base * RATIO_LO && h.value <= base * RATIO_HI
);
const nearBase = hits.filter(
  (h) => h.value >= base * 0.92 && h.value <= base * 1.08
);
console.log(
...
console.log(
  `[values] near reference (±8%): ${nearBase.length}   in crit band (×1.40–1.60): ${critBand.length}` +
    `   other: ${hits.length - nearBase.length - critBand.length}`
);
if (nearBase.length) {
  const impliedRate = critBand.length / (nearBase.length + critBand.length);
  console.log(
    `[values] implied crit rate (band / (base+band)): ${(impliedRate * 100).toFixed(1)}%`
  );
}
...
for (let i = 0; i < BINS; i++) {
  ...
  console.log(
    `  ${Math.round(binLo).toString().padStart(10)}  ${bar} ${hist[i]}`
  );
}
