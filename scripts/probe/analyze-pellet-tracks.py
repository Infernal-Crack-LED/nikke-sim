...
def _print_mislock_identity(reports):
    ...
    print("\n⚑ CONTROL POPULATION, checked rather than asserted. On all four PRODUCTION dumps this "
          ...
          "mislock-rate-slice.json, which\n  cannot supply a crosshair at a band-PLATEAU frame (a "
          "representative frame is generally not one of\n  t0+8..11). The only template-located "
          "DUMPS of that clip are `groundtruth-f811-shot04-tmpl` /\n  `groundtruth-f811-v6-landed-"
          "tmpl`, a different localization run, so the clip reads 10 mislocked of\n  37 here "
          "against §20's committed 6 of 37 -- §20's six are a strict SUBSET. Same detector, same "
          "160 px\n  threshold, same t0+8..11 offsets: only the second lock's provenance differs, "
