...
if (!shot || !existsSync(shot)) {
  console.error('usage: read-battle-records.ts <screenshot> [--comp a,b,c] [--expect-total N] ' +
...
const comp = flags.comp && flags.comp !== 'true' ? flags.comp.split(',').map((s) => s.trim()) : null;
const tolerance = Number(flags.tolerance ?? 1);
const outDir = flags.out ?? `${process.env.CLAUDE_SCRATCH ?? '/tmp'}/battle-records`;

let cumulativeTotal: number | null =
  flags['expect-total'] && flags['expect-total'] !== 'true' ? Number(flags['expect-total']) : null;
if (cumulativeTotal == null && flags['total-damage'] && flags['total-damage'] !== 'true') {
  ...
  const vals = td.reads.filter((r) => r.totalDamage != null && r.totalDamage > 0);
  cumulativeTotal = vals.length ? (vals[vals.length - 1].totalDamage as number) : null;
}
...
interface Unit {
  slot: number; slug?: string; name: string | null; level: number | null;
  combatPower: number | null; totalDamage: number; damageTaken: number | null; healing: number | null;
}

async function ask(prompt: string, b64: string): Promise<Record<string, unknown>> {
  ...
  if (Array.isArray(content)) content = content.map((c) => (c as { text?: string }).text ?? '').join('');
...
for (let attempt = 0; attempt < 3; attempt++) {
  try { parsed = await ask(PROMPT, b64); break; } catch (e) {
    if (attempt === 2) { console.error(`  FAILED — ${(e as Error).message}`); process.exit(1); }
    await new Promise((r) => setTimeout(r, 1000));
  }
}

const warnings: string[] = [];
const rows = Array.isArray(parsed.units) ? (parsed.units as Record<string, unknown>[]) : [];
if (!rows.length) { console.error('VLM returned no rows — check the endpoint and the screenshot'); process.exit(1); }
...
const units: Unit[] = rows.map((r, i) => {
  ...
  return {
    slot,
    slug: comp?.[slot - 1],
    name: typeof r.name === 'string' && r.name.trim() && !/empty/i.test(r.name) ? r.name.trim() : null,
    level: num(r.level),
...
const deltaPct = cumulativeTotal ? Math.round(((sum - cumulativeTotal) / cumulativeTotal) * 10000) / 100 : null;
const pass = deltaPct == null ? null : Math.abs(deltaPct) <= tolerance;
if (cumulativeTotal == null)
  warnings.push('NO CHECKSUM — pass --expect-total or --total-damage. Without it this is an ' +
    'unconfirmed VLM read: report it as a survey, never as a measured per-unit total.');
else if (!pass)
  warnings.push(`CHECKSUM FAILED — per-unit sum ${sum.toLocaleString()} vs cumulative ` +
...
if (occupied.some((u) => u.level != null && u.level !== 400))
  warnings.push('a row reads LV != 400 — scope-lock recordings are sync 400, so the level column was misread');
const cps = occupied.map((u) => u.combatPower).filter((v): v is number => v != null);
if (cps.length > 1 && new Set(cps).size === 1)
  warnings.push('every Combat Power is identical — that is the signature of ATK, not CP, so the ⚔ column was misread');
for (const u of occupied)
  if (u.combatPower != null && u.combatPower > 200000)
    warnings.push(`slot ${u.slot} Combat Power ${u.combatPower} is implausibly large — likely a damage value read into the ⚔ column`);
if (comp && comp.length !== occupied.length)
  warnings.push(`--comp lists ${comp.length} slugs but ${occupied.length} rows are occupied — slot mapping is unreliable`);

const result = {
  screenshot: shot,
  model,
  fieldMap: {
    crossedSwords: 'Combat Power (NOT ATK — ATK is class-based, see data/reference-stats.json)',
    redBar: 'total damage dealt', shield: 'damage taken', asterisk: 'healing done',
  },
  units,
...
writeFileSync(`${outDir}/battle-records.json`, JSON.stringify(result, null, 2) + '\n');

console.log(`\nwrote ${outDir}/battle-records.json`);
for (const u of occupied)
  console.log(`  slot ${u.slot}  ${(u.slug ?? u.name ?? '?').padEnd(24)} dmg ${u.totalDamage.toLocaleString().padStart(14)}  taken ${u.damageTaken}  heal ${u.healing}  CP ${u.combatPower}`);
console.log(`  sum ${sum.toLocaleString()}` +
  (cumulativeTotal != null ? `  vs cumulative ${cumulativeTotal.toLocaleString()}  Δ ${deltaPct}%  ${pass ? 'CHECKSUM PASS' : 'CHECKSUM FAIL'}` : '  (no checksum)'));
