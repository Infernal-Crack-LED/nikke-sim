...
const CATALOG = fileURLToPath(
  new URL('../../docs/probe-data/catalog.json', import.meta.url)
);
...
const catalog: CatalogEntry[] = existsSync(CATALOG)
...
console.log(
  `\n${catalogued}/${media.length} catalogued · ${parsedCount} parsed` +
    (stale.length
      ? ` · ${stale.length} stale entries (file missing): ${stale.map((s) => s.video).join(', ')}`
      : '')
);
