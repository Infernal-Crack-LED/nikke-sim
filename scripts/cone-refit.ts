...
const COREPX: Record<string, number> = {
  near: 31,
  mid: 28,
  midfar: 21,
  far: 17,
...
const CELLS: Cell[] = [
  // ── AR (scarlet HR0 SPAWN re-count COUNT-1; blanc HR39 near/mid per-frame + far SPAWN COUNT-2; Label HR23 spawn) ──
  {
    ...
    n: 20,
    method: 'spawn',
    note: 'scarlet COUNT-1 spawn 0.25 ⚠fade-stack',
  },
  {
    w: 'AR',
    band: 'mid',
    ...
    n: 12,
    method: 'spawn',
    note: 'scarlet COUNT-1 spawn 0.33 ⚠ (non-monotone noise)',
  },
  {
    w: 'AR',
    band: 'midfar',
    ...
    n: 16,
    method: 'spawn',
    note: 'scarlet COUNT-1 spawn 0.19 ⚠',
  },
  {
    w: 'AR',
    band: 'far',
    ...
    n: 36,
    method: 'spawn',
    note: 'scarlet COUNT-1 spawn 0.08 ✅ clean',
  },
  ...
  { w: 'AR', band: 'midfar', hr: 39.24, k: 7, n: 24, method: 'perframe' },
  {
    ...
    n: 180,
    method: 'spawn',
    note: 'blanc far spawn 0.111 n=180',
  },
  {
    w: 'AR',
    band: 'near',
    ...
    method: 'perframe',
    interval: [0.9, 1.0],
    note: 'jill near saturation (far excluded) — owner-weighted',
  },
  // ── SMG (chisato HR0/HR22 near; quency HR61 all bands n_eff=pulls; LM HR15) ──
  {
    ...
    n: 109,
    method: 'spawn',
    note: 'chisato-off, cleanest HR0 SMG near',
  },
  {
    w: 'SMG',
    band: 'near',
  ...
  },
  {
    ...
    method: 'perframe',
    interval: [0.35, 0.583],
    note: 'quency near biased-high, n_eff=pulls',
  },
  {
    w: 'SMG',
    band: 'mid',
  ...
  },
  {
    ...
    n: 8,
    method: 'perframe',
    note: 'quency midfar 0.267 n_eff~8',
  },
  {
    w: 'SMG',
    band: 'far',
    ...
    n: 18,
    method: 'perframe',
    note: 'quency far 0.143 n_eff~18',
  },
  {
    w: 'SMG',
    band: 'near',
    ...
    n: 122,
    method: 'spawn',
    note: 'LM 0.238 (whites undercounted → high)',
  },
  {
    w: 'SMG',
    band: 'mid',
  ...
  // ── SG (dorothy HR0 all bands; noir ▲60 all bands; noir ▲98 near ~1) ──
  {
    ...
    n: 48,
    method: 'spawn',
    note: 'dorothy 0.083 (~7 shots pellets)',
  },
  ...
  },
  {
    ...
    method: 'perframe',
    interval: [0.9, 1.0],
    note: 'noir ▲98 in-burst ~1 (owner-attested, weighted)',
  },
];
...
const sigma = (w: string, hr: number, s: number, floor: number) =>
  ((CIRCLE_PX_K * SCALE[w]) / 2 / K_SIGMA) *
  Math.max(floor, 1 - s * Math.max(0, hr));
const delta = (d0: number, hr: number, H: number) =>
  d0 * Math.max(0, 1 - Math.max(0, hr) / H);
const predCore = (w: string, band: string, hr: number, p: P) =>
  Math.min(
    1,
    offsetCoreProb(
      COREPX[band] / 2,
      sigma(w, hr, p.s, p.floor),
      delta(p.d0[w], hr, p.H)
    )
  );
...
function nll(p: P): number {
  ...
  for (const a of LANDING) {
    const lf = pelletLandFrac(
      BAND_SG_HIT_FRAC[a.band],
      sigma('SG', a.hr, p.s, p.floor)
    );
...
for (let iter = 0; iter < 8; iter++) {
  ...
  for (const v of grids.floor) {
    const t = { ...best, floor: v };
    const l = nll(t);
    if (l < bestL) {
      bestL = l;
      best = t;
...
console.log(
  `  δ0:  AR ${best.d0.AR}  SMG ${best.d0.SMG}  SG ${best.d0.SG}  (px @2622)`
);
console.log(
  `  H:   ${best.H}   s: ${best.s}   S_FLOOR: ${best.floor}   (K_SIGMA 2.53 held)`
);
...
for (const c of CELLS) {
  ...
  const ok = c.interval
    ? pr >= c.interval[0] - 0.03 && pr <= c.interval[1] + 0.03
    : Math.abs(pr - meas) < 0.12;
  console.log(
    `  ${c.w.padEnd(4)} ${c.band.padEnd(7)} ${String(c.hr).padStart(5)}  ${meas.toFixed(3)}  ${String(c.n).padStart(4)}  ${pr.toFixed(3)}  ${c.method.padEnd(9)} ${ok ? '✓' : '✗'} ${c.interval ? `[${c.interval[0]},${c.interval[1]}]` : ''}`
  );
}
console.log('\n=== SG LANDING anchors ===');
for (const a of LANDING)
...
console.log(
  `  soda-tb SG ▲38.91  near ${predCore('SG', 'near', 38.91, best).toFixed(3)}  mid ${predCore('SG', 'mid', 38.91, best).toFixed(3)}  midfar ${predCore('SG', 'midfar', 38.91, best).toFixed(3)}  far ${predCore('SG', 'far', 38.91, best).toFixed(3)}`
);
console.log(
  `  chisato SMG midfar HR22.37: ${predCore('SMG', 'midfar', 22.37, best).toFixed(3)}`
);
console.log(
  `  AR near curve: HR0 ${predCore('AR', 'near', 0, best).toFixed(3)} → HR23 ${predCore('AR', 'near', 23, best).toFixed(3)} → HR39 ${predCore('AR', 'near', 39, best).toFixed(3)} → HR80 ${predCore('AR', 'near', 80, best).toFixed(3)}`
);
...
for (const s of [0.004, 0.006, 0.008, 0.01, 0.012]) {
  const row = [0.1, 0.15, 0.2, 0.3].map((f) =>
    (nllAt(s, f) - base0).toFixed(1).padStart(7)
  );
  console.log(
    `  s=${s.toFixed(3)}  ΔNLL@floor[.10 .15 .20 .30]: ${row.join(' ')}`
  );
}
