...
The judge does the discriminating work of this whole test: separating an _expected_ divergence (a kit
...
- `GOTCHA` — the reconstruction (derived faithfully from code) diverges from the real kit in a way that
  ...
  - `ENGINE` — the override encodes it fine, but the engine routes/executes it so that the _behavior_
