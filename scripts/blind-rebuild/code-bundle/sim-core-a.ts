...
function dealDamage(
  ...
  if (opts.crit) {
    const critRate = Math.min(
      1,
      Math.max(0, (u.critRate + stat(u, 'critRatePct', frame)) / 100)
    );
    const critBonus =
  ...
  if (opts.core && cfg.coreHitRate > 0) {
    ...
    const hr =
      HRCORE || PELLET_GAUSS || CONE_DELTA ? stat(u, 'hitRatePct', frame) : 0;
...
function resolveTargets(t: TargetDef, ownerIdx: number): UnitState[] {
  switch (t.kind) {
    ...
    case 'alliesOfElement':
      return units.filter(
        (u) =>
          u.char.element === t.element && (!t.excludeSelf || u.idx !== ownerIdx)
      );
    case 'alliesOfClass':
      return units.filter(
