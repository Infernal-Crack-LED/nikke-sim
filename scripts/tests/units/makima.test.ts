...
const COMP = { slugs: [...SLUGS], bossElement: 'Fire' as const, focusSlug: 'ada' };
...
const hasGainPierce = (b: any) => b.effects.some((e: any) => e.kind === 'gainPierce');
...
const injectPierceProbe = (ov: any) => {
  ov.skill1 = [
    ...(ov.skill1 ?? []),
    {
      ...
      target: { kind: 'self' },
      effects: [{ kind: 'buff', stat: 'pierceDamagePct', value: 50, durationSec: 180 }],
    },
  ];
};
...
const makimaS1Phantom = withPatchedOverride('makima', (ov) => {
  ov.skill1 = [
    ...(ov.skill1 ?? []),
    {
      ...
      trigger: { kind: 'passive' },
      target: { kind: 'allies' },
      effects: [{ kind: 'buff', stat: 'reloadSpeedPct', value: 36.96, durationSec: 180 }],
    },
  ];
});
...
const makimaCasts = (evs: SimEvent[]): BurstCast[] =>
  evs.filter((e): e is BurstCast => e.kind === 'burstCast' && e.slug === 'makima');
const makimaNormals = (evs: SimEvent[]): Damage[] =>
  evs.filter(
    (e): e is Damage => e.kind === 'damage' && e.slug === 'makima' && e.bucket === 'normal'
  );
const probeFirings = (evs: SimEvent[]): BuffApply[] =>
  evs.filter(
    (e): e is BuffApply =>
      e.kind === 'buffApply' && e.casterIdx === MAKIMA && e.stat === 'defPct' && e.value === 1
  );
...
describe('makima — kit spec', () => {
  describe('M0 — fixture guard: sole-B2 comp, she casts every FB cycle', () => {
    it('casts her burst at stage 2, repeatedly (not starved by a second B2)', () => {
      ...
      expect(casts.length, 'no makima burst casts — fixture is starved').toBeGreaterThanOrEqual(6);
  ...
  describe('M1 — L5 burst "Gain Pierce for 10 sec": timed self pierce tag, inert at scope lock', () => {
    ...
    it('tags her normal hits for 10s after each cast (probe: pierceDamagePct feed goes live in-window only)', () => {
      ...
      for (const c of measurable) {
        const inShipped = windowSum(pierceProbed.events, c.frame, c.frame + 10 * FPS);
        const inNoTag = windowSum(pierceProbeNoTag.events, c.frame, c.frame + 10 * FPS);
        expect(
          inShipped,
    ...
    it('expires at 10s: out-of-window hits are byte-identical with and without the tag', () => {
      ...
      for (let i = 0; i + 1 < casts.length; i++) {
        ...
        if (to - from < FPS) continue; // no clean gap (casts abnormally close)
        expect(
      ...
      expect(checked, 'no inter-cast gap was measurable').toBeGreaterThanOrEqual(3);
    });
  });
  ...
  describe('M2 — L6 burst "Recover 34.02% of attack damage as HP OVER 10 SEC": a 10s recovery window', () => {
    const firingsFor = (evs: SimEvent[], cast: BurstCast) =>
      probeFirings(evs).filter((b) => b.frame >= cast.frame && b.frame <= cast.frame + 11 * FPS);
  ...
  describe('M3 — L1/L2/L3/L4/L7 are UNMODELED: the whole kit is damage-neutral at scope lock', () => {
    ...
    it('structural: no S1/S2 blocks, burst is exactly [gainPierce 10s self, heal HoT self]', () => {
      ...
      expect(ov.skill1, 'S1 must stay unmodeled (attacked-count trigger is out of domain)').toEqual([]);
      expect(
        ov.skill2,
      ...
      expect(heal.effects).toEqual([{ kind: 'heal', ticks: 10, intervalSec: 1 }]);
    ...
    it('structural: the unmodeled record carries every skipped line (no silent drops)', () => {
      ...
      expect(un.skill1.some((s: string) => s.includes('attacked 20 times'))).toBe(true);
      expect(un.skill1.some((s: string) => s.includes('Reload Speed ▲ 36.96%'))).toBe(true);
      expect(un.skill2.some((s: string) => s.includes('Taunt all enemies'))).toBe(true);
      expect(un.skill2.some((s: string) => s.includes('indomitability for 7 sec'))).toBe(true);
      expect(un.skill2.some((s: string) => s.includes('Cooldown of Burst Skill ▼ 11.58 sec.'))).toBe(true);
      expect(un.burst.some((s: string) => s.includes('Incoming healing ▲ 41.02%'))).toBe(true);
    });
  });
  ...
  describe('M4 — rotation integrity: no cast lands closer than her unmodified 20s CD', () => {
    ...
    it('her cast count equals the bare-kit count (the rotation is undisturbed by the kit)', () => {
      expect(makimaCasts(base.events).length).toBe(makimaCasts(bare.events).length);
    });
  });
});
