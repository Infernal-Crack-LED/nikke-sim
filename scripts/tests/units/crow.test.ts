...
const crowCasts = (evs: SimEvent[]) =>
  evs.filter(
    (e): e is BurstCast => e.kind === 'burstCast' && e.slug === 'crow'
  );
...
const crowWithS1Debuff = withPatchedOverride('crow', (ov) => {
  ov.skill1.push({
    ...
    target: { kind: 'enemy' },
    effects: [{ kind: 'buff', stat: 'atkPct', value: -19.93, durationSec: 10 }],
  });
});
...
describe('crow — kit spec', () => {
  ...
  describe('C1 — S2 rider: 89.09% of final ATK when the LAST bullet hits', () => {
    ...
    const emptyShots = crowShots(base.events).filter((s) => s.ammoAfter === 0);
    ...
    it('enacting it anyway moves NOTHING (enemy ATK▼ is dropped at dispatch — no incoming-damage model)', () => {
      ...
      expect(
        buffs(withS1Debuff.events).filter(
          (b) => b.casterIdx === CROW && b.stat === 'atkPct' && b.value < 0
        ),
