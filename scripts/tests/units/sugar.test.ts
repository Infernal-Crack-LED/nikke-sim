...
import { controlComp, data, runComp, totals, withPatchedOverride } from '../lib/harness.js';
...
const removeEffect = (slot: 'skill1' | 'skill2' | 'burst', stat: string) => (ov: any) => {
  let removed = false;
  for (const b of ov[slot]) {
    const n = b.effects.length;
    b.effects = b.effects.filter((e: any) => !(e.kind === 'buff' && e.stat === stat));
    if (b.effects.length !== n) removed = true;
  }
  ov[slot] = ov[slot].filter((b: any) => b.effects.length > 0);
  if (!removed) throw new Error(`sugar ${slot} ${stat} effect missing — fixture is stale`);
};
const removeAdvantageVs = (ov: any) => {
  const before = ov.skill1.length;
  ov.skill1 = ov.skill1.filter((b: any) => !b.effects.some((e: any) => e.kind === 'advantageVs'));
  if (ov.skill1.length === before) throw new Error('sugar skill1 advantageVs block missing — fixture is stale');
};
/** Nearest-wrong for G1: the continuous passive re-gated to a part-time Full-Burst window. */
const s1l4ToFbGated = (ov: any) => {
  const b = ov.skill1.find((x: any) => x.effects.some((e: any) => e.stat === 'attackDamagePct'));
  if (!b) throw new Error('sugar skill1 attackDamagePct block missing — fixture is stale');
  b.trigger = { kind: 'fullBurstEnter' };
  b.effects.find((e: any) => e.stat === 'attackDamagePct').durationSec = 10;
};
const setTrigger = (slot: 'skill2' | 'burst', stat: string, kind: string) => (ov: any) => {
  let hit = false;
  for (const b of ov[slot]) if (b.effects.some((e: any) => e.kind === 'buff' && e.stat === stat)) { b.trigger = { kind }; hit = true; }
  if (!hit) throw new Error(`sugar ${slot} ${stat} block missing — fixture is stale`);
};
const setTarget = (slot: 'skill2' | 'burst', stat: string, target: any) => (ov: any) => {
  let hit = false;
  for (const b of ov[slot]) if (b.effects.some((e: any) => e.kind === 'buff' && e.stat === stat)) { b.target = target; hit = true; }
  if (!hit) throw new Error(`sugar ${slot} ${stat} block missing — fixture is stale`);
};
...
const s2AmmoUnscoped = run(setTarget('skill2', 'maxAmmoPct', { kind: 'allies' }));
const noS2Elem = run(removeEffect('skill2', 'elemAdvantageDamagePct'));
const s2ElemUnscoped = run(setTarget('skill2', 'elemAdvantageDamagePct', { kind: 'allies' }));
...
const bElemUnscoped = run(setTarget('burst', 'elemAdvantageDamagePct', { kind: 'allies' }));

// ---- readers ----------------------------------------------------------------------------------
const buffs = (evs: SimEvent[]) => evs.filter((e): e is BuffApply => e.kind === 'buffApply');
const fbCount = (evs: SimEvent[]) => evs.filter((e): e is FbStart => e.kind === 'fullBurstStart').length;
const sugarCasts = (evs: SimEvent[]) =>
  evs.filter((e): e is BurstCast => e.kind === 'burstCast' && e.slug === 'sugar').length;
...
describe('sugar (Treasure) — kit spec', () => {
  describe('G1 — S1 cover-intact Attack Damage ▲19.98% is a continuous passive (self)', () => {
    ...
    it('DISCRIMINATING: a Full-Burst-gated 19.98%/10s is part-time and deals less', () => {
      ...
      expect(sugarBuffs(s1l4Fb.events, 'attackDamagePct').some((b) => b.frame === 0)).toBe(false);
    });
  });
  ...
  describe('G4 — S2 Full-Burst-enter self ATK ▲25.01% for 10 sec', () => {
    const applied = sugarBuffs(base.events, 'atkPct').filter((b) => b.value === 25.01);
  ...
  describe('G5 — S2 Full-Burst-enter Max Ammunition ▲83.8% for 15 sec, scoped to SHOTGUN allies', () => {
    ...
    it('is 83.8%, 15 sec, reaches exactly the shotgun allies (sugar here), once per Full Burst', () => {
      ...
      for (const i of holders(base.events, 'maxAmmoPct')) expect(COMP[i]).toBeTruthy();
    ...
    it('DISCRIMINATING: an unscoped allies buff reaches all four, not just the SG allies', () => {
      expect(holders(s2AmmoUnscoped.events, 'maxAmmoPct')).toEqual(sorted(ALL_IDX));
  ...
  describe('G6 — S2 Full-Burst-enter Elem. Advantage Attack Damage ▲40.02% / 15s, Water+Iron SG allies', () => {
    const applied = sugarBuffs(base.events, 'elemAdvantageDamagePct').filter((b) => b.value === 40.02);

    it('is 40.02%, 15 sec, reaches exactly the Water/Iron shotgun allies (sugar here)', () => {
      expect(applied.length).toBeGreaterThan(0);
      for (const b of applied) expect(b.expiresFrame! - b.frame).toBe(15 * FPS);
      expect(holders(base.events, 'elemAdvantageDamagePct').filter((i) =>
        applied.some((b) => b.targetIdx === i))).toEqual(sorted(waterIronSgIdx));
    ...
    it('DISCRIMINATING: an unscoped allies buff reaches all four, not just Water/Iron SG', () => {
      expect(holders(s2ElemUnscoped.events, 'elemAdvantageDamagePct')).toEqual(sorted(ALL_IDX));
      expect(holders(base.events, 'elemAdvantageDamagePct')).not.toEqual(sorted(ALL_IDX));
    });
  });
  ...
  describe('G9 — burst self ATK ▲20% for 15 sec (burstCast)', () => {
    const applied = sugarBuffs(base.events, 'atkPct').filter((b) => b.value === 20);
  ...
  describe('G10 — burst Elem. Advantage Attack Damage ▲60.01% / 15s, Water+Iron SG allies (burstCast)', () => {
    const applied = sugarBuffs(base.events, 'elemAdvantageDamagePct').filter((b) => b.value === 60.01);
    ...
    it('is 60.01%, 15 sec, reaches exactly the Water/Iron shotgun allies, once per sugar burst cast', () => {
      ...
      expect(holders(base.events, 'elemAdvantageDamagePct').filter((i) =>
        applied.some((b) => b.targetIdx === i))).toEqual(sorted(waterIronSgIdx));
      expect(applied.length).toBe(sugarCasts(base.events) * waterIronSgIdx.length);
    });
    ...
    it('DISCRIMINATING: an unscoped allies buff reaches all four, not just Water/Iron SG', () => {
      expect(holders(bElemUnscoped.events, 'elemAdvantageDamagePct')).toEqual(sorted(ALL_IDX));
      expect(holders(base.events, 'elemAdvantageDamagePct')).not.toEqual(sorted(ALL_IDX));
    });
  });
});
