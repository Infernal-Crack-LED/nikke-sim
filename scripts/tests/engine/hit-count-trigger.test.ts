...
describe('hitCount trigger', () => {
  ...
  it('DISCRIMINATING: the meter advances by hitsPerShot per PULL, not by one', () => {
    ...
    expect(
      procs.length,
      `one activation per pull at count == hitsPerShot (${hps})`
    ).toBe(shots.length);
  ...
  it('DISCRIMINATING: one pull can fire the block several times, and the remainder carries over', () => {
    ...
    expect(
      Math.max(...counts),
      'no pull ever fired the block more than 3 times'
    ).toBe(4);
    expect(
      counts,
      'activations per pull should alternate 3/4 as the remainder accrues'
    ).toEqual([3, 4]);
  });
  ...
  it('DISCRIMINATING: countInFb lowers the threshold in Full Burst and the accrued meter CARRIES OVER', () => {
    ...
    const carry = 'ada';
    const { procs, shots, events } = probe(carry, {
      kind: 'hitCount',
      count: 100_000,
      countInFb: 1,
    });
    ...
    expect(
      firstInFbShot,
      'the carry never fired inside a Full Burst window'
    ).toBeDefined();
    const accrued =
      shots.filter((s) => s.frame <= firstInFbShot!.frame).length *
      hitsPerShot(carry);
    ...
    expect(
      accrued,
      'nothing had accrued before the first in-FB pull — the arm is vacuous'
    ).toBeGreaterThan(2);
    const atThatFrame = procs.filter(
      (p) => p.frame === firstInFbShot!.frame
    ).length;
    expect(
      atThatFrame,
    ...
    expect(
      procs.filter((p) => p.frame < firstInFbShot!.frame),
      'the 100k threshold leaked'
    ).toEqual([]);
  });
});
