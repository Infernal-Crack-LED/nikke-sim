  ...
  const privaty = withPatchedOverride('privaty', (ov) => {
    ...
    ov.skill1 = applyEffects.length
      ? [
          {
            slot: 'skill1',
            trigger: o.applyEvery
              ? { kind: 'hitCount', count: o.applyEvery }
              : { kind: 'shotFired' },
...
describe('named target-status gate (targetStatus / requiresTargetStatus)', () => {
  ...
  const matched = run({
    applied: 'Designated Target',
    gated: 'Designated Target',
  });
  ...
  it('P2 mechanism fires when the status is live and the gate names it', () => {
    expect(
      matched.skill,
      'gate never opened despite the status being applied'
    ).toBeGreaterThan(0);
  });
  ...
  it('P4 DISCRIMINATING: concurrent statuses do not cross-satisfy and stay name-isolated', () => {
    ...
    expect(
      bothB.skill,
      'P4b: the second concurrent status did not open its own gate'
    ).toBeGreaterThan(0);
    expect(
      bothC.skill,
  ...
  it('P7 engine ignores block.target for targetStatus (implicit enemy)', () => {
    ...
    const alliesTargeted = run({
      applied: 'Designated Target',
      gated: 'Designated Target',
      statusTarget: 'allies',
    });
    expect(
      alliesTargeted.skill,
      'block.target changed the status registry'
    ).toBe(matched.skill);
  });

  it('P6 is deterministic — the seedless run reproduces', () => {
    expect(
      run({ applied: 'Designated Target', gated: 'Designated Target' }).skill
    ).toBe(matched.skill);
  });
});
