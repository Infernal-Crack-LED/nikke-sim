...
function describeError(err: unknown): {
  ...
  if (err instanceof ApiError) {
    switch (err.status) {
      case 400:
        return {
          msg: "That doesn't look like a valid link or open id.",
          unprivate: false,
        };
      case 401:
        return {
          msg: 'Please log in with Discord to sync your roster.',
          unprivate: false,
        };
      case 429:
        return {
      ...
      case 500:
        return {
          msg: 'Roster sync is temporarily unavailable. Please try again later.',
          unprivate: false,
        };
      default:
        return {
          msg: `Something went wrong (${err.status}).`,
          unprivate: false,
        };
    }
  }
      ...
      {open && (
        <div className='roster-unprivate-body'>
          ...
          <ol>
            ...
            <li>
              Click your <strong>profile card</strong>.
            </li>
            <li>
              Click the <strong>padlock icon</strong> in the top-right
              (highlighted below).
            </li>
            <li>
...
export function RosterSyncPage({
  ...
  const runSync = async (openid: string, refresh: boolean) => {
    ...
    if (!id) {
      setError({
        msg: 'Paste your blablalink profile link (or open id) first.',
        unprivate: false,
      });
      return;
    }
  ...
  const rosterCount = roster
    ? (roster.count ??
      (Array.isArray(roster.characters) ? roster.characters.length : 0))
    : 0;
