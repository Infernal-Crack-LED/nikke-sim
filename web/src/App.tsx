...
// metaScoringFor (enikk meta-popularity per weakness) and prydwenScoreOf (the
// element-agnostic bossing-tier seed score) now live in ./genCalc, so the roster
...
// main thread apply an identical bonus.
...
const shortfallReasonText = (r: ShortfallReason): string => {
  switch (r.kind) {
    ...
    case 'element':
      return `Every team needs at least one ${r.element} unit for the selected boss weakness — none left.`;
    case 'required-any':
      return `Every team needs a healer (Include Healer is on) — none left.`;
  }
};
...
export function App({ user }: { user: AuthUser | null }) {
  ...
  // Materialize the per-unit loadout resolver into a serializable slug→loadout map
  // covering every generator pool slug — functions can't cross the worker boundary,
  // so the main thread resolves them once and the worker (or main-thread fallback)
  // rebuilds an identical calc from the map (perf plan item 1a; parity gate:
  // scripts/tests/generators/loadouts-parity.test.ts).
  const materializeLoadouts = (): Record<string, UnitOptions> => {
    const resolve = genSynced
      ? (slug: string) => {
          const L = genSynced.get(slug);
          return L
  ...
  // meta/prydwen/synergy/pool from static JSON on the far side). Replaces the old
  // newCalc/newCalcWith: same pool, cfg, blocked list, and materialized loadouts.
  const genParams = (
  ...
  // Explain a short roster run: diagnose what the leftover pool is missing, and
  // whether the "Include Non-OL Units" pill would supply it. Mirrors the search's
  // own rules — the healer requirement relaxes when no healer is eligible at all
  // (buildGenCalc drops an empty requiredAny the same way).
  const shortfallOpts = (eligible: Set<string>) => ({
  ...
  const computeRosterShortfall = (
    ...
    const used = new Set(teams.flatMap((t) => t.slugs));
    ...
    const reasons = diagnoseTeamShortfall(
      ...
      shortfallOpts(new Set(eligible))
    );
  ...
  // Used by the non-generator tools (doll cal, OL sim) that still compute on the
  // main thread — the 30ms yield lets the "calculating…" state paint first.
  ...
  // Async generator run (perf plan item 1a): the roster search now runs in a web
  // worker, so the UI thread never blocks and no setTimeout paint hack is needed —
  // just await the worker result and setState. Falls back to a main-thread run
  // when workers are unavailable (see simClient).
  const runGen = async (fn: () => Promise<void>) => {
    setCalcBusy(true);
    try {
      await fn();
    } finally {
  ...
  const runTopTeams = () =>
    runGen(async () => {
      ...
      if (results.length < 5) {
        setRosterShortfall(computeRosterShortfall(params, results, 5));
      }
    });
          ...
          {rosterGenMode === 'solo' ? (
            <>
              ...
              {rosterResults && (
                <>
                  {rosterShortfall &&
                    rosterShortfall.built < rosterShortfall.requested && (
                      <div className="roster-shortfall">
                        <div className="roster-shortfall-title">
                          Built {rosterShortfall.built} of{' '}
                          {rosterShortfall.requested} teams — the{' '}
                          {rosterShortfall.leftoverCount} eligible units left
                          can't form another:
                        </div>
                        <ul className="roster-shortfall-reasons">
                          {rosterShortfall.reasons.length ? (
                            rosterShortfall.reasons.map((r, i) => (
                              <li key={i}>{shortfallReasonText(r)}</li>
                            ))
                          ) : (
                            <li>
                              Their burst roles, cooldowns, and team rules don't
                              combine into a valid 5-unit team.
                            </li>
                          )}
                        </ul>
                        {rosterShortfall.nonOlFix && (
                          <div className="roster-shortfall-hint">
                            “Include Non-OL Units” adds{' '}
                            {rosterShortfall.nonOlFix.count} more of your synced
                            units
                            {rosterShortfall.nonOlFix.names.length
                              ? ` — ${rosterShortfall.nonOlFix.names.join(', ')} cover what's missing`
                              : ''}{' '}
