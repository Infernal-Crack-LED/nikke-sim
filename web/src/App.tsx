...
// Kit-role archetype tags by slug (data/archetype-tags.json). Used by the Team
// Builder filter (CharacterGrid) and to feed the generators' like-tag synergy bias
// (see SYNERGY_PAIRS below). Generated by scripts/build-archetype-tags.ts.
const archetypeTags = (archetypeTagsJson as { tags: Record<string, string[]> })
  .tags;
...
// Like-tag synergy bias for the team/roster generators (owner ruling 2026-07-23):
// a team that pairs a damage dealer with its matching damage buffer scores a little
// higher — pierce dealer ↔ Pierce ▲ buffer, projectile dealer ↔ Projectile ▲ buffer
// (archetype tags from data/archetype-tags.json). Soft on purpose: the bonus is a
// small multiplier on the damage×meta ranking score (teamcalc.scoreOf), so simulated
// damage and the meta blend still lead and the bias only breaks otherwise-close
// choices. Weight is per satisfied pair (a team can satisfy both at once).
