  ...
  return (
    <div className="matrix-filter">
      <Row
        label="Framework"
        ids={FRAMEWORK_IDS}
        labelOf={(id) => FRAMEWORKS[id].label}
        value={cell.framework}
        onPick={(framework) => onChange({ ...cell, framework })}
      />
      <Row
        label="Element"
        ids={ELEADV_IDS}
        labelOf={(id) => ELEADVS[id].label}
        value={cell.eleadv}
        onPick={(eleadv) => onChange({ ...cell, eleadv })}
      />
      <Row
        label="Core"
        ids={CORE_IDS}
        labelOf={(id) => CORES[id].label}
        value={cell.core}
        onPick={(core) => onChange({ ...cell, core })}
      />
      {!hideInvest && (
        <Row
          label="Investment"
          ids={INVEST_IDS}
          labelOf={(id) => INVESTS[id].label}
          value={cell.invest}
          onPick={(invest) => onChange({ ...cell, invest })}
        />
      )}
    </div>
  );
