...
// One-time removal of the 9 "rare" units (idoll-*, product-*, soldier-*) from
// the Characters page and Team Builder. characters.json carries no rarity
// field to filter these dynamically, and no new ones get added, so a fixed
// exclusion list here is sufficient rather than a general mechanism.
const RARE_UNIT_SLUGS = new Set([
  'idoll-flower',
  'idoll-ocean',
  'idoll-sun',
  'product-08',
...
export function useCharacterFilter({
  ...
  const characters = useMemo(() => {
    const all = visibleCharacters().filter(
      (c) =>
  ...
  const total = visibleCharacters().filter(
    (c) =>
      (!restrict || restrict.has(c.slug)) &&
      (allowUnsupported || c.simSupported)
  ).length;
