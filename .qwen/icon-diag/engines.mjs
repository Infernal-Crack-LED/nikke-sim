...
for (const e of ELEMENTS) {
  ...
  writeFileSync(
    `${OUT}/wh_${e}.svg`,
    src.replace('<svg ', '<svg width="73" height="73" ')
  );
}
  ...
  for (const dpr of [1, 1.5, 2]) {
    ...
    for (const e of ELEMENTS) {
      variants.push({
        id: `${e}/raw`,
        kind: 'img',
        url: toDataUrl(`${ICONS}/code_${e}.svg`, 'image/svg+xml'),
      });
      variants.push({
        id: `${e}/wh`,
        kind: 'img',
        url: toDataUrl(`${OUT}/wh_${e}.svg`, 'image/svg+xml'),
      });
      variants.push({
        id: `${e}/inline`,
        kind: 'inline',
        markup: readFileSync(`${ICONS}/code_${e}.svg`, 'utf8'),
      });
    }
