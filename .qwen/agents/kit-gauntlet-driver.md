...
- STOP-DON'T-WAIT: a NON-dispatch command hangs ~60s, or a tool is blocked → `echo 'STOPPED-AT: <action> — <reason>' >> .gauntlet-progress-<slug>.txt`, then end with a short report. **EXCEPTION — `dispatch-claude.sh`** (see DISPATCH PATIENCE in ENVIRONMENT): run it FOREGROUND with a long shell timeout (~480000–600000 ms); opus S5/S6/S7 take 2–5 min, so the 60s rule does NOT apply (a premature abort forces a needless same-model fallback). NEVER background a dispatch — a backgrounded dispatch can outlive your turn and leak a result JSON after the unit is committed. A dispatch has FAILED only if it errors out or returns no valid JSON after a genuinely long wait; suspect impatience before suspecting the bridge.
